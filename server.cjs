...
function normalizeShops(db){
  ...
  db.shops = db.shops.map(shop=>{
    ...
    return {id, displayId, name, login, password:String(shop.password || SHOP_PASSWORD)};
...
function adminMenu(){ const menuDb=readDb(),msgUnread=adminDirectUnread(menuDb),notifUnread=notificationsCount(menuDb,{admin:true}); return `<a class="mobileBackToCabinet" href="/admin">← Перейти у Кабінет складу</a><aside class="adminMenu"><div class="adminMenuHead"><a class="adminMenuLogo" href="/admin" style="text-decoration:none">Кабінет</a></div><a href="/admin-notifications">Сповіщення${badgeCount(notifUnread)}</a><a href="/admin-settings">🔒 Налаштування магазинів</a><a href="/admin-products">Товар</a><a href="/admin-barcodes">Штрихкоди товарів</a><a href="/admin-new-products">Новинки</a><a href="/admin-applications">Нова заявка</a><a data-unread-key="directMessages" href="/admin-messages">Повідомлення${badgeCount(msgUnread)}</a><a href="/admin-notes">Нотатки</a><a href="/admin-announcements">Оголошення</a><a href="/admin-orders">Замовлення</a><a href="/admin-accounting">🔒 Журнал обліку</a><a href="/admin-kegs">Облік кег</a><a href="/admin-keg-types">🔒 Редагувати список кег</a><a href="/admin-missing-products">Відсутні товари</a><a href="/admin-hidden-products">Приховані позиції</a><a href="/admin-backup">🔒 Резервна копія</a><a href="/admin-logout" class="adminMenuLogout">Вийти</a></aside>`; }
...
function directMessagesHtml(db, shop, canDelete=false, peer='warehouse'){
  ...
  return messages.map(m=>{
    ...
    return `<div class="chatMessage ${isAdmin||mine?'adminMsg':'shopMsg'}" data-message-id="${esc(m.id)}"><div class="chatMeta"><b class="${isAdmin||mine?'adminName':'shopName'}">${esc(name)}</b><span class="muted" style="font-size:11px;margin-left:6px">${esc(m.createdAt||'')}</span>${del}</div><div class="chatText">${esc(m.text||'')}</div></div>`;
  }).join('') || '<div class="chatEmpty">Повідомлень поки немає</div>';
}
function adminCabinetPage(db){
  const items=[
    ...
    ['Облік кег','/admin-kegs','🛢️'],['Редагувати список кег','/admin-keg-types','🔒'],['Журнал обліку','/admin-accounting','🔒'],
...
function shopCabinetPage(db, session){
  ...
  const items=[['Сповіщення','/notifications','🔔',notificationsCount(db,session)],['Каталог','/catalog','📦'],['Кошик','/cart','🛒',(session.cart||[]).reduce((a,i)=>a+Number(i.qty||0),0)],['Кеги',session.shop==='Склад'?'/warehouse-kegs':'/kegs','🛢️'],['Журнал обліку','/cabinet/accounting','📒'],['Повідомлення','/messages','✉️',unread.directMessages],['Оголошення','/about','📢',unread.announcements]];
...
function shopMessagesPage(db, session){
  const shop=session.shop;
  return `<section class="messagesPage hasPeer"><div class="messagesLayout messagesWarehouseOnly"><div class="card chatBox"><div class="mobileChatTop"><span></span><b>Склад</b><span></span></div><div class="chatHeader"><div><h2>Повідомлення складу</h2><p class="muted">Тут можна написати лише на склад і отримати відповідь.</p></div></div><div class="chatMessages" data-direct-messages="1">${directMessagesHtml(db,shop,false,'warehouse')}</div><form class="form chatForm" method="post" action="/messages/send"><input type="hidden" name="recipient" value="warehouse"><label>Ваше повідомлення<textarea name="text" required placeholder="Напишіть повідомлення на склад..."></textarea></label><button>Надіслати</button></form></div></div></section>`;
}
...
function layout(title, content, session={cart:[]}){
  ...
  return `<!doctype html><html lang="pl"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>${esc(title)} · TARANKA MAGAZINE</title>
...
.adminOrderSummaryCard{cursor:pointer}
.adminOrderSummaryCard:focus-visible{outline:3px solid rgba(37,99,235,.28);outline-offset:3px}
...
@media(max-width:700px){.shopSettingRow{grid-template-columns:1fr;gap:10px;padding:14px 54px 14px 13px}.shopSettingName{grid-column:auto}.shopSettingDelete{top:9px;right:9px}}
...
function filterProducts(){
  ...
  catalogSearchItems().forEach(function(item){
    ...
    if(show&&item.isCard)visibleCards++;
  });
  const empty=document.getElementById('searchEmpty');
  if(empty)empty.style.display=visibleCards?'none':'block';
}
...
async function startBarcodeCamera(){
  ...
  try{
    ...
    box.innerHTML='';box.appendChild(video);box.insertAdjacentHTML('beforeend','<div class="barcodeGuide"></div><div id="barcodeProductName" class="barcodeProductName"></div><div class="barcodeHint">Наведіть задню камеру на штрихкод</div>');
...
function missingProductRows(db){
  const rows=[];
  const clearedAtMs=Number(db.missingProductsClearedAtMs||0);
  (db.orders||[]).forEach(o=>{
    if(!o || !o.pickingFinalizedAt) return;
    const finalizedMs=Number(o.pickingFinalizedMs||o.createdMs||0);
    if(clearedAtMs && finalizedMs<=clearedAtMs) return;
    (o.items||[]).forEach((item,index)=>{
      if(String(item&&item.pickingStatus||'')!=='absent') return;
      const unit=inferResultUnit(item);
      const orderedQty=item.orderedQty!==undefined?item.orderedQty:item.qty;
      rows.push({
        key:String(o.id||'')+'-'+String(item.id||index),
        productName:productDisplayName(item),
        shop:String(o.shop||''),
        orderNo:String(o.orderNo||o.id||''),
        quantity:productResultText(item,orderedQty),
        createdAt:String(o.pickingFinalizedAt||''),
        createdMs:Number(o.pickingFinalizedMs||o.createdMs||0),
        unit
      });
    });
  });
  return rows.sort((a,b)=>b.createdMs-a.createdMs || a.productName.localeCompare(b.productName,'uk'));
}
function adminMissingProductsPage(db){
  const rows=missingProductRows(db);
  const clearButton=rows.length?`<form method="post" action="/admin-missing-products/clear" onsubmit="return confirm('Очистити всю історію відсутніх товарів? Нові відсутні позиції після очищення знову будуть додаватися автоматично.')" style="margin:0"><button type="submit" class="secondary">Очистити історію</button></form>`:'';
  const content=rows.length?`<div style="display:grid;gap:10px">${rows.map(r=>`<div style="display:grid;grid-template-columns:minmax(0,1fr) auto;gap:10px 16px;align-items:center;padding:14px 16px;border:1px solid #e5e7eb;border-radius:14px;background:#fff"><div style="min-width:0"><div style="display:flex;align-items:flex-start;gap:9px"><span style="flex:0 0 auto;width:30px;height:30px;border-radius:9px;background:#fee2e2;color:#991b1b;display:grid;place-items:center;font-weight:900">!</span><div style="min-width:0"><b style="display:block;font-size:16px;overflow-wrap:anywhere">${esc(r.productName)}</b><div class="muted" style="margin-top:4px;font-size:13px">Магазин: <b>${esc(r.shop||'—')}</b> · Замовлення №${esc(r.orderNo)}</div></div></div></div><div style="text-align:right;white-space:nowrap"><div style="font-weight:800;color:#991b1b">${esc(r.quantity)}</div><div class="muted" style="font-size:12px;margin-top:4px">${esc(r.createdAt)}</div></div></div>`).join('')}</div>`:'<div class="center muted" style="padding:30px">Відсутніх товарів поки немає</div>';
  return `<div class="adminShell">${adminMenu()}<section><div class="actions" style="justify-content:space-between;align-items:flex-start;gap:12px;margin-bottom:16px"><div><h1 style="margin-bottom:5px">Відсутні товари</h1><p class="muted" style="margin:0">Сюди автоматично потрапляють позиції, позначені знаком «−» під час формування замовлення.</p></div>${clearButton}</div><div class="card" style="padding:16px">${content}</div></section></div>`;
}
...
function adminOrderCard(o, products){
  ...
  const editable=!o.invoicePrintedAt && !finalized;
  const openUrl=`/admin-order-picking?id=${encodeURIComponent(o.id)}`;
  return `<div class="card order adminOrderGridCard adminOrderSummaryCard" data-order-id="${esc(o.id)}" role="link" tabindex="0" onclick="if(!event.target.closest('a,button,form,input'))location.href='${openUrl}'" onkeydown="if((event.key==='Enter'||event.key===' ')&&!event.target.closest('a,button,form,input')){event.preventDefault();location.href='${openUrl}'}">
...
function addProductToOrderPicking(o, product, actualTotal, barcode=''){
  ...
  if(idx<0){
    ...
    o.items.push({id,productId:product.id,name:product.name,productName:product.name,category:product.category||'',weight:product.weight||String(format),format,packQty:format,packUnit:displayUnit,resultUnit:displayUnit,qty,unitType:unit,unitsPerScan:format,barcode:barcode||''});
    idx=o.items.length-1;
  }else{
    const current=o.items[idx]||{};
    o.items[idx]={...current,productId:product.id,name:current.name||product.name,productName:current.productName||product.name,category:current.category||product.category||'',weight:product.weight||current.weight||String(format),format,packQty:format,packUnit:displayUnit,resultUnit:displayUnit,unitType:unit,unitsPerScan:format,barcode:barcode||current.barcode||''};
  }
...
function adminOrderPickingCard(o, products){
  ...
  const rows=(o.items||[]).map((i,idx)=>{
    const key=String(i.id||idx);
    const saved=picking[key]||{};
  ...
  const checked=finalized?total:(o.items||[]).reduce((n,i,idx)=>{const x=picking[String(i.id||idx)];return n+(x&&(x.status==='present'||x.status==='absent')?1:0)},0);
...
function appXlsx(app, options={}){ const db=options.db||null; const rows=[['Numer zamówienia',app.number,'','',''],['Data',app.completedAt||app.createdAt,'','',''],['Sklep',app.shopName,'','',''],markPlain(['','','','','']),markHeader(['Lp.','Nazwa pozycji','Ilość','Jednostka','Kod kreskowy'])]; let n=0; (app.pallets||[]).forEach(p=>(p.items||[]).forEach(i=>{n++;rows.push([n,i.productName,appOrderValue(i,db),appUnitLabel(i.unitType),i.barcode])})); let logo=null;try{logo=fs.readFileSync(path.join(__dirname,'taranka-header-logo.png'))}catch(e){try{logo=fs.readFileSync(path.join(__dirname,'taranka-logo.png'))}catch(_){}} return genericXlsx('Zamówienie',rows,[14,46,24,18,18],'portrait',logo);}
...
async function appPdf(app, options={}){
  ...
  return new Promise((resolve,reject)=>{
    try{
      ...
      doc.font('AppSans').fontSize(10).text('Magazyn: ____________________',36,y);
      doc.text('Sklep: ____________________',330,y);
      doc.end();
    }catch(e){reject(e);}
  });
}
...
<div class="layout2">
  ...
  <section>
    ...
    <div id="prodGrid" class="prodGrid" style="display:grid">${products.map(p=>productCard(p,session)).join('')}</div>
  ...
  if(req.method==='POST' && url.pathname==='/admin/order-picking-finalize'){ if(!requireAdmin(req,res,session)) return; const b=await body(req); const o=(db.orders||[]).find(x=>String(x.id)===String(b.id)); if(!o){res.writeHead(404,{'Content-Type':'application/json; charset=utf-8'});return res.end(JSON.stringify({ok:false,error:'Замовлення не знайдено'}));} if(o.pickingFinalizedAt){res.writeHead(200,{'Content-Type':'application/json; charset=utf-8'});return res.end(JSON.stringify({ok:true,html:adminOrderPickingCard(o,db.products)}));} const picking=o.pickingItems&&typeof o.pickingItems==='object'?o.pickingItems:{}; const complete=(o.items||[]).every((i,idx)=>{const x=picking[String(i.id||idx)];return x&&(x.status==='present'||x.status==='absent')}); if(!complete){res.writeHead(400,{'Content-Type':'application/json; charset=utf-8'});return res.end(JSON.stringify({ok:false,error:'Спочатку позначте всі позиції'}));} o.items=(o.items||[]).map((i,idx)=>{const key=String(i.id||idx),x=picking[key]||{},format=Math.max(0.000001,productFormatValue(itemWithQuantityFields(i))); const unit=inferResultUnit(i),actual=normalizeQuantityForUnit(Math.max(0,Number(x.actualTotal)||0),unit); return {...i,orderedQty:i.orderedQty!==undefined?i.orderedQty:i.qty,qty:Math.round((actual/format)*1000)/1000,pickingStatus:x.status,actualTotal:actual};}); o.pickingFinalizedAt=warsawTime(); o.pickingFinalizedMs=nowMs(); o.status='Сформовано для друку'; o.kegItems=captureOrderKegItems(db,o.items); applyOrderStock(db,o); writeDb(db); res.writeHead(200,{'Content-Type':'application/json; charset=utf-8'}); return res.end(JSON.stringify({ok:true,html:adminOrderPickingCard(o,db.products)})); }
  ...
  if(req.method==='POST' && url.pathname==='/admin-missing-products/clear'){ if(!requireAdmin(req,res,session)) return; db.missingProductsClearedAtMs=nowMs(); writeDb(db); return redirect(res,'/admin-missing-products'); }
