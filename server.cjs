...
async function toggleFavorite(event,button){
  ...
  try{
    ...
    if(location.search.indexOf('favorites=1')>=0&&!active){
      document.querySelectorAll('.prodCard[data-product-id="'+CSS.escape(id)+'"], #prodList tr[data-product-id="'+CSS.escape(id)+'"]').forEach(function(el){el.remove()});
      const hasCards=document.querySelector('#prodGrid .prodCard'); if(!hasCards){location.reload();}
    }
...
async function handleRequest(req,res){ try{ const url=new URL(req.url, `http://${req.headers.host}`); const session=getSession(req,res); let db=readDb();
 ...
 if(session.shop && isValidShopInDb(db, session.shop)){ touchPresence(db, session); db=readDb(); }
  ...
  if(req.method==='GET' && url.pathname==='/catalog'){
    ...
    const favoriteIds=shopFavoriteIds(db,session.shop);
    const products=db.products.filter(p=>!p.hidden&&(!cat||p.category===cat)&&(!onlyNew||p.isNew)&&(!onlyFavorites||favoriteIds.has(String(p.id))));
    const unread=unreadCounts(db, session);
...
<div class="layout2">
  ...
  <section>
    ...
    ${!products.length?`<div class="card center" style="padding:36px"><p class="muted">${onlyFavorites?'Ви ще не додали товари в обране':'У цій категорії немає товарів'}</p><a class="btn secondary" href="/catalog">Усі товари</a></div>`:''}
  </section>
</div>
<script>(function(){
  let stopped=false;
