...
async function handleRequest(req,res){ try{ const url=new URL(req.url, `http://${req.headers.host}`); const session=getSession(req,res); let db=readDb();
 ...
 if(session.shop && isValidShopInDb(db, session.shop)) schedulePresenceUpdate(db,session);
  ...
  if(req.method==='GET' && url.pathname==='/catalog/products'){
    ...
    const offset=Math.max(0,Math.min(selection.products.length,parseInt(url.searchParams.get('offset')||'0',10)||0));
    const chunk=selection.products.slice(offset,offset+CATALOG_PAGE_SIZE);
    const nextOffset=offset+chunk.length;
    return sendJson(res,200,{ok:true,html:catalogChunkHtml(chunk,session,db),nextOffset,total:selection.products.length,hasMore:nextOffset<selection.products.length});
  }
  if(req.method==='GET' && url.pathname==='/catalog'){
    ...
    const firstProducts=products.slice(0,CATALOG_PAGE_SIZE);
    const hasMore=firstProducts.length<products.length;
    const unread=unreadCounts(db, session);
...
<div class="layout2">
  <aside class="card side catSideNew">
    <div class="catSideHead">
      <a href="/catalog" class="catAllLink ${!cat&&!onlyNew&&!onlyFavorites?'catAllActive':''}">
    ...
    <div class="catGridNew">
      ...
      <a href="/catalog?favorites=1" class="catCardNew ${onlyFavorites?'catCardActive':''}">
  ...
  <section>
    ...
    <div id="prodGrid" class="prodGrid" style="display:grid">${catalogChunkHtml(firstProducts,session,db)}</div>
    ...
    <div id="catalogLoadMoreWrap" class="center" style="padding:20px 0 8px;${hasMore?'':'display:none'}">
      <button type="button" class="btn secondary" id="catalogLoadMore" onclick="loadMoreCatalogProducts()">Показати ще 40 товарів</button>
      <div class="muted" id="catalogProgress" style="font-size:12px;margin-top:8px">Показано ${firstProducts.length} із ${products.length}</div>
    </div>
  </section>
</div>
<script>(function(){
  let stopped=false;
  let loading=false;
  let offset=${firstProducts.length};
  ...
  const loadWrap=document.getElementById('catalogLoadMoreWrap');
  const loadButton=document.getElementById('catalogLoadMore');
  ...
  function updateCatalogControls(hasMore){
    if(progress)progress.textContent='Показано '+offset+' із '+total;
    if(loadWrap)loadWrap.style.display=hasMore?'':'none';
    if(empty)empty.style.display=total?'none':'';
  }
  function requestParams(nextOffset){
  ...
  async function loadCatalogProducts(reset){
    ...
    loading=true;
    if(loadButton){loadButton.disabled=true;loadButton.textContent='Завантаження…';}
    try{
      const r=await fetch('/catalog/products?'+requestParams(nextOffset).toString(),{cache:'no-store',headers:{'X-Requested-With':'fetch'},signal:currentController.signal});
      const j=await r.json();
      if(!r.ok||!j||!j.ok)throw new Error('catalog');
      if(reset)grid.innerHTML=j.html||'';else grid.insertAdjacentHTML('beforeend',j.html||'');
      offset=Number(j.nextOffset||0);
      total=Number(j.total||0);
      __catalogSearchItems=null;
      applyMissingProductCards();
      if(emptyText&&reset)emptyText.textContent='Товарів за вашим запитом не знайдено';
      updateCatalogControls(!!j.hasMore);
    }catch(e){
      if(e&&e.name!=='AbortError')toast('Не вдалося завантажити товари');
    }finally{
      if(requestController===currentController){loading=false;if(loadButton){loadButton.disabled=false;loadButton.textContent='Показати ще 40 товарів';}}
    }
  }
  window.loadMoreCatalogProducts=function(){loadCatalogProducts(false)};
  window.refreshCatalogProducts=function(){loadCatalogProducts(true)};
  window.catalogSearchChanged=function(){
    clearTimeout(searchTimer);
    searchTimer=setTimeout(function(){
      const pageUrl=new URL(location.href);
      const q=String((document.getElementById('search')||{}).value||'').trim();
      if(q)pageUrl.searchParams.set('q',q);else pageUrl.searchParams.delete('q');
      pageUrl.searchParams.delete('offset');
      history.replaceState(null,'',pageUrl.pathname+(pageUrl.searchParams.toString()?'?'+pageUrl.searchParams.toString():''));
  ...
  if(req.method==='POST' && url.pathname==='/cart/add'){ if(!requireShop(req,res,session)) return; const b=await body(req); const p=db.products.find(x=>String(x.id)===String(b.id)&&!x.hidden); const requestedDelta=Number(b.delta===undefined||b.delta===''?1:b.delta),delta=[1,5,10].includes(requestedDelta)?requestedDelta:1; let itemQty=0; if(p){ const item=session.cart.find(x=>String(x.id)===String(p.id)); if(item)item.qty=Number(item.qty||0)+delta; else session.cart.push({...copyProductFields(p), qty:delta}); itemQty=(session.cart.find(x=>String(x.id)===String(p.id))||{}).qty||0; saveCart(session); } const count=session.cart.reduce((a,i)=>a+Number(i.qty||0),0); if(req.headers['x-requested-with']==='fetch'){ res.writeHead(200, {'Content-Type':'application/json; charset=utf-8'}); return res.end(JSON.stringify({ok:true,count,positions:session.cart.length,id:String(b.id),itemQty,result:productResultText(p,itemQty),catalogTotal:productCatalogTotalText(p,itemQty)})); } return redirect(res, req.headers.referer || '/catalog'); }
