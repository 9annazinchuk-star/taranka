...
const MISSING_ALERT_POLL_INTERVAL_MS = 5 * 60 * 1000;
const ORDER_PAGE_SIZE = 20;
const ADMIN_PRODUCT_PAGE_SIZE = 50;
const MESSAGE_POLL_INTERVAL_MS = 30 * 1000;
...
const DEPOSIT_TYPES = ['puszka','PET'];
function canHaveDeposit(category){ return DEPOSIT_CATEGORIES.includes(String(category||'')); }
function normalizeDepositType(value){ const raw=String(value||'').trim().toLowerCase(); if(raw==='puszka'||raw==='can'||raw==='банка')return 'puszka'; if(raw==='pet'||raw==='plastic'||raw==='пет')return 'PET'; return ''; }
function productDepositType(product){ return String(product&&product.category||'')==='Алкоголь' ? normalizeDepositType(product&&product.depositType) : ''; }
const CAT_SVG_ICONS = {
...
const CART_BACKUP_INTERVAL_MS=60*1000;
let lastDbBackupAt=0;
let cartBackupDirty=false;
let cartBackupTimer=null;
function refreshDbBackupFromCurrent(){
  if(!fs.existsSync(DB_FILE))return;
  const content=fs.readFileSync(DB_FILE);
  JSON.parse(content.toString('utf8'));
  atomicReplaceFile(DB_FILE+'.bak',content,{keepBackup:false});
  lastDbBackupAt=nowMs();
  cartBackupDirty=false;
}
function scheduleCartBackup(){
  cartBackupDirty=true;
  if(cartBackupTimer)return;
  const delay=Math.max(250,CART_BACKUP_INTERVAL_MS-Math.max(0,nowMs()-lastDbBackupAt));
  cartBackupTimer=setTimeout(()=>{
    cartBackupTimer=null;
    if(!cartBackupDirty)return;
    enqueueDbWrite(()=>{
      try{refreshDbBackupFromCurrent();}
      catch(error){console.error('[cart backup]',error);scheduleCartBackup();}
    });
  },delay);
  if(cartBackupTimer&&typeof cartBackupTimer.unref==='function')cartBackupTimer.unref();
}
...
function ensureDb(){
  ...
  if(!fs.existsSync(DB_FILE)) writeDb({products:[], orders:[], accountingReports:[], kegAdjustments:[], notes:[], announcements:[], chatMembers:[], chatMessages:[], directMessages:[], presence:{}, readState:{}, carts:{}, favorites:{}, sessions:{}, shops: defaultShops()});
}
function readDb(){
  const requestState=requestDbContext.getStore();
  if(requestState&&requestState.db) return requestState.db;
  ensureDb();
  try {
    ...
    db.carts=db.carts||{};
    db.favorites=db.favorites&&typeof db.favorites==='object'?db.favorites:{};
    ...
    if(requestState) requestState.db=db;
    return db;
  } catch(error) {
...
function writeDb(db,{cartWrite=false}={}){
  normalizeShops(db);
  normalizeChat(db);
  sortProductsInCategories(db);
  const content=Buffer.from(JSON.stringify(db,null,2),'utf8');
  // Зміни кошика завжди спочатку атомарно потрапляють в основну базу. Їхню
  // резервну копію робимо окремим відкладеним знімком уже нового стану.
  const keepBackup=!cartWrite;
  atomicReplaceFile(DB_FILE,content,{keepBackup});
  if(cartWrite)scheduleCartBackup();
  else {lastDbBackupAt=nowMs();cartBackupDirty=false;}
  const requestState=requestDbContext.getStore();
  if(requestState) requestState.db=db;
}
...
function renameShopReferences(db, oldName, newName){
  ...
  if(db.favorites&&Object.prototype.hasOwnProperty.call(db.favorites,oldName)){ db.favorites[newName]=db.favorites[oldName]; delete db.favorites[oldName]; }
...
function adminMenu(active=''){ const menuDb=readDb(),msgUnread=adminDirectUnread(menuDb),notifUnread=notificationsCount(menuDb,{admin:true}),activeClass=key=>active===key?' class="active" aria-current="page"':''; return `<a class="mobileBackToCabinet" href="/admin">← Перейти у Кабінет складу</a><aside class="adminMenu"><div class="adminMenuHead"><a class="adminMenuLogo" href="/admin" style="text-decoration:none">Кабінет</a></div><a href="/admin-notifications">Сповіщення${badgeCount(notifUnread)}</a><a href="/admin-settings">🔒 Налаштування магазинів</a><a href="/admin-products">Товар</a><a href="/admin-statistics"${activeClass('statistics')}>📊 Статистика</a><a href="/admin-market"${activeClass('market')}>📋 Ринок</a><a href="/admin-barcodes">Штрихкоди товарів</a><a href="/admin-new-products">Новинки</a><a href="/admin-applications">Нова заявка</a><a data-unread-key="directMessages" href="/admin-messages">Повідомлення${badgeCount(msgUnread)}</a><a href="/admin-notes">Нотатки</a><a href="/admin-announcements">Оголошення</a><a href="/admin-orders">Замовлення</a><a href="/admin-accounting">🔒 Журнал обліку</a><a href="/admin-work-hours">⏱️ Робочі години</a><a href="/admin-kegs">Облік кег</a><a href="/admin-keg-types">🔒 Редагувати список кег</a><a href="/admin-missing-products">Відсутні товари</a><a href="/admin-hidden-products">Приховані позиції</a><a href="/admin-backup">🔒 Резервна копія</a><a href="/admin-logout" class="adminMenuLogout">Вийти</a></aside>`; }
...
const pendingPresenceUpdates=new Map();
function schedulePresenceUpdate(db, session){
  if(!session || !session.shop) return;
  db.presence=db.presence||{};
  const shop=String(session.shop),now=nowMs();
  const previous=Math.max(Number((db.presence[shop]||{}).lastSeenMs||0),Number(pendingPresenceUpdates.get(shop)||0));
  if(previous>0 && now-previous<PRESENCE_UPDATE_INTERVAL_MS) return;
  pendingPresenceUpdates.set(shop,now);
  enqueueDbWrite(()=>{
    try{
      const latest=readDb();
      if(!isValidShopInDb(latest,shop)) return;
      latest.presence=latest.presence||{};
      const saved=Number((latest.presence[shop]||{}).lastSeenMs||0);
      if(saved>0 && now-saved<PRESENCE_UPDATE_INTERVAL_MS) return;
      latest.presence[shop]={lastSeenMs:now,lastSeenAt:warsawTime()};
      writeDb(latest);
    }finally{
      if(Number(pendingPresenceUpdates.get(shop)||0)===now) pendingPresenceUpdates.delete(shop);
    }
  });
}
...
function adminCabinetPage(db){
  const items=[
    ...
    ['Замовлення','/admin-orders','🧾'],['Нова заявка','/admin-applications','📲'],['Товари','/admin-products','📦'],['Статистика','/admin-statistics','📊'],['Ринок','/admin-market','📋'],['Штрихкоди товарів','/admin-barcodes','▥',(db.productBarcodes||[]).length],
...
const STATISTICS_PERIODS=new Set(['7','30','60','90','current_month','previous_month','custom','all']);
const STATISTICS_SORTS=new Set(['name','quantity','orders','shops','last','change']);
function statisticsDate(ms){
  if(!Number.isFinite(Number(ms)))return '—';
  return new Intl.DateTimeFormat('uk-UA',{timeZone:'Europe/Warsaw',day:'2-digit',month:'2-digit',year:'numeric'}).format(new Date(Number(ms)));
}
function statisticsDateKey(ms=nowMs()){
...
function statisticsDayStart(dateKey){
  const m=String(dateKey||'').match(/^(\d{4})-(\d{2})-(\d{2})$/);
  if(!m)return NaN;
  const target=Date.UTC(Number(m[1]),Number(m[2])-1,Number(m[3]),0,0,0,0);
  let guess=target;
  for(let i=0;i<2;i++){
    const parts=new Intl.DateTimeFormat('en-CA',{timeZone:'Europe/Warsaw',year:'numeric',month:'2-digit',day:'2-digit',hour:'2-digit',minute:'2-digit',second:'2-digit',hourCycle:'h23'}).formatToParts(new Date(guess));
    const v=Object.fromEntries(parts.map(p=>[p.type,p.value]));
    const represented=Date.UTC(Number(v.year),Number(v.month)-1,Number(v.day),Number(v.hour),Number(v.minute),Number(v.second));
    guess-=represented-target;
  }
  return guess;
}
function statisticsNextDateKey(dateKey,days=1){
  const m=String(dateKey||'').match(/^(\d{4})-(\d{2})-(\d{2})$/);
  if(!m)return '';
  return new Date(Date.UTC(Number(m[1]),Number(m[2])-1,Number(m[3])+Number(days))).toISOString().slice(0,10);
}
function statisticsMonthRange(offset=0){
  const today=statisticsDateKey();
  const [year,month]=today.split('-').map(Number);
  const first=new Date(Date.UTC(year,month-1+offset,1)).toISOString().slice(0,10);
  const next=new Date(Date.UTC(year,month+offset,1)).toISOString().slice(0,10);
  return {from:statisticsDayStart(first),to:statisticsDayStart(next)-1,fromKey:first,toKey:statisticsNextDateKey(next,-1)};
}
function statisticsRange(url){
  const raw=String(url.searchParams.get('period')||'all');
  const period=STATISTICS_PERIODS.has(raw)?raw:'all';
  const now=nowMs(),day=86400000;
  let requestedFrom=0,requestedTo=now,fromKey='',toKey='';
  if(['7','30','60','90'].includes(period))requestedFrom=now-Number(period)*day;
  else if(period==='current_month'){const r=statisticsMonthRange(0);requestedFrom=r.from;requestedTo=Math.min(now,r.to);fromKey=r.fromKey;toKey=statisticsDateKey();}
  else if(period==='previous_month'){const r=statisticsMonthRange(-1);requestedFrom=r.from;requestedTo=r.to;fromKey=r.fromKey;toKey=r.toKey;}
  else if(period==='custom'){
    const requestedFromKey=String(url.searchParams.get('from')||''),requestedToKey=String(url.searchParams.get('to')||'');
    const customFrom=statisticsDayStart(requestedFromKey),customTo=statisticsDayStart(statisticsNextDateKey(requestedToKey,1))-1;
    if(Number.isFinite(customFrom)&&Number.isFinite(customTo)&&customTo>=customFrom){requestedFrom=customFrom;requestedTo=Math.min(customTo,now);fromKey=requestedFromKey;toKey=requestedToKey;}
    else {requestedFrom=now-30*day;requestedTo=now;}
  }
  const from=requestedFrom,to=requestedTo;
  const duration=Math.max(1,requestedTo-requestedFrom+1),compareTo=requestedFrom-1,compareFrom=compareTo-duration+1;
  const comparisonAvailable=period!=='all'&&compareFrom>=0&&to>=from;
  return {period,from,to,requestedFrom,requestedTo,compareFrom,compareTo,comparisonAvailable,partial:false,fromKey,toKey};
}
function statisticsOrderMs(order){
  const createdMs=Number(order&&order.createdMs);
  if(Number.isFinite(createdMs)&&createdMs>0)return createdMs;
  const idMs=Number(order&&order.id);
  return Number.isFinite(idMs)&&idMs>1000000000000?idMs:0;
}
function statisticsOrderFinalized(order){
  return !!(order&&order.pickingFinalizedAt);
}
function statisticsWarsawDateMs(value){
  const m=String(value||'').match(/^\s*(\d{1,2})[.]\s*(\d{1,2})[.]\s*(\d{4})\D+(\d{1,2}):(\d{2})(?::(\d{2}))?/);
  if(!m)return 0;
  const target=Date.UTC(Number(m[3]),Number(m[2])-1,Number(m[1]),Number(m[4]),Number(m[5]),Number(m[6]||0));
  let guess=target;
  for(let i=0;i<2;i++){
    const parts=new Intl.DateTimeFormat('en-CA',{timeZone:'Europe/Warsaw',year:'numeric',month:'2-digit',day:'2-digit',hour:'2-digit',minute:'2-digit',second:'2-digit',hourCycle:'h23'}).formatToParts(new Date(guess));
    const v=Object.fromEntries(parts.map(p=>[p.type,p.value]));
...
function statisticsApplicationMs(application){
  const completedMs=Number(application&&application.completedMs);
  if(Number.isFinite(completedMs)&&completedMs>0)return completedMs;
  const completedAt=statisticsWarsawDateMs(application&&application.completedAt);
  if(completedAt>0)return completedAt;
  const createdMs=Number(application&&application.createdMs);
  if(Number.isFinite(createdMs)&&createdMs>0)return createdMs;
  const createdAt=statisticsWarsawDateMs(application&&application.createdAt);
  if(createdAt>0)return createdAt;
  const idMatch=String(application&&application.id||'').match(/^app_(\d{13})_/);
  return idMatch?Number(idMatch[1]):0;
}
function statisticsApplicationOrder(application){
  return {
    id:`application:${String(application&&application.id||application&&application.number||'')}`,
    shop:String(application&&application.shopName||''),
    createdMs:statisticsApplicationMs(application),
    items:(application&&Array.isArray(application.pallets)?application.pallets:[]).flatMap(p=>Array.isArray(p.items)?p.items:[]).map(item=>({...item,name:item.productName||item.name,actualTotal:item.totalUnits}))
  };
}
function statisticsProductId(db,item,productMap){
  const candidates=[item&&item.productId,item&&item.id];
  for(const candidate of candidates){const id=String(candidate||'');if(id&&productMap.has(id))return id;}
  const resolved=resolveCatalogProductId(db,item||{});
  return resolved&&productMap.has(String(resolved))?String(resolved):'';
}
function statisticsAggregate(db,orders,products){
  const productMap=new Map(products.map(p=>[String(p.id),p]));
  const map=new Map(products.map(p=>[String(p.id),{quantity:0,orderCount:0,shops:new Set(),orderIds:new Set(),lastMs:0}]));
  for(const order of orders){
    const orderProducts=new Set();
    for(const item of Array.isArray(order.items)?order.items:[]){
      if(String(item&&item.pickingStatus||'')==='absent')continue;
      const id=statisticsProductId(db,item,productMap),row=map.get(id);
      if(!row)continue;
      const savedActual=Number(item.actualTotal);
      const quantity=Number(item.qty);
      const total=Number.isFinite(savedActual)&&savedActual>=0?savedActual:productFormatValue(itemWithQuantityFields(item))*quantity;
      if(!Number.isFinite(total)||total<=0)continue;
      row.quantity=Math.round((row.quantity+total)*1000)/1000;
      row.shops.add(String(order.shop||''));
      row.orderIds.add(String(order.id||''));
      row.lastMs=Math.max(row.lastMs,statisticsOrderMs(order));
      orderProducts.add(id);
    }
    orderProducts.forEach(id=>{const row=map.get(id);if(row)row.orderCount++;});
  }
  return map;
}
function statisticsBuild(db,url){
  const range=statisticsRange(url),products=(db.products||[]).slice();
  const shop=String(url.searchParams.get('shop')||''),category=String(url.searchParams.get('category')||''),unitFilter=String(url.searchParams.get('unit')||''),search=String(url.searchParams.get('q')||'').trim().toLocaleLowerCase('uk'),statusFilter=String(url.searchParams.get('status')||'');
  const finalizedOrders=(db.orders||[]).filter(statisticsOrderFinalized);
  const completedApplications=(db.applications||[]).filter(a=>String(a&&a.status||'')==='completed').map(statisticsApplicationOrder);
  const eligible=finalizedOrders.concat(completedApplications).filter(o=>!shop||String(o.shop||'')===shop);
  const currentOrders=eligible.filter(o=>{const ms=statisticsOrderMs(o);return ms>=range.from&&ms<=range.to;});
  const compareOrders=range.comparisonAvailable?eligible.filter(o=>{const ms=statisticsOrderMs(o);return ms>=range.compareFrom&&ms<=range.compareTo;}):[];
  const current=statisticsAggregate(db,currentOrders,products),previous=statisticsAggregate(db,compareOrders,products),lifetime=statisticsAggregate(db,eligible,products);
  let rows=products.map(product=>{
    const id=String(product.id),cur=current.get(id),prev=previous.get(id),life=lifetime.get(id),unit=normalizeUnit(product.resultUnit||product.packUnit||inferResultUnit(product));
    const change=range.comparisonAvailable?(prev.quantity>0?((cur.quantity-prev.quantity)/prev.quantity)*100:(cur.quantity>0?null:0)):null;
    let trendKey='insufficient',trendLabel='Недостатньо даних';
    if(life.orderCount===0){trendKey='never';trendLabel='Ще не замовляли';}
    else if(cur.orderCount===0){trendKey='no_period';trendLabel='Не замовляли за період';}
    else if(!range.comparisonAvailable){trendKey='ordered';trendLabel='Замовляють';}
    else if(prev.quantity===0&&cur.quantity>0){trendKey='new';trendLabel='Новий попит';}
    else if(change>10){trendKey='growth';trendLabel='Попит зростає';}
    else if(change<-10){trendKey='decline';trendLabel='Попит знижується';}
    else {trendKey='stable';trendLabel='Стабільний попит';}
    return {product,id,name:String(product.name||''),category:String(product.category||''),unit,quantity:cur.quantity,orderCount:cur.orderCount,shops:cur.shops,orderIds:cur.orderIds,lastMs:cur.lastMs,lifetimeLastMs:life.lastMs,lifetimeOrders:life.orderCount,average:cur.orderCount?cur.quantity/cur.orderCount:0,change,trendKey,trendLabel,activityKey:'none',activityLabel:'Без замовлень'};
  });
  const positive=rows.filter(r=>r.orderCount>0).map(r=>r.orderCount).sort((a,b)=>a-b);
  const low=positive.length?positive[Math.floor((positive.length-1)*0.25)]:0,high=positive.length?positive[Math.floor((positive.length-1)*0.75)]:0;
  rows.forEach(r=>{if(!r.orderCount)return;if(r.orderCount<=low){r.activityKey='rare';r.activityLabel='Рідко замовляють';}else if(r.orderCount>=high){r.activityKey='frequent';r.activityLabel='Часто замовляють';}else{r.activityKey='regular';r.activityLabel='Замовляють';}});
  rows=rows.filter(r=>(!category||r.category===category)&&(!unitFilter||r.unit===unitFilter)&&(!search||(`${r.name} ${r.category}`).toLocaleLowerCase('uk').includes(search))&&(!statusFilter||r.trendKey===statusFilter||r.activityKey===statusFilter));
  const sort=STATISTICS_SORTS.has(String(url.searchParams.get('sort')||''))?String(url.searchParams.get('sort')):'orders',dir=String(url.searchParams.get('dir')||'desc')==='asc'?'asc':'desc',sign=dir==='asc'?1:-1;
  const value=row=>sort==='name'?row.name.toLocaleLowerCase('uk'):sort==='quantity'?row.quantity:sort==='shops'?row.shops.size:sort==='last'?row.lastMs:sort==='change'?(row.change===null?-Infinity:row.change):row.orderCount;
  rows.sort((a,b)=>{const av=value(a),bv=value(b);const cmp=typeof av==='string'?av.localeCompare(bv,'uk'):av-bv;return sign*cmp||a.name.localeCompare(b.name,'uk');});
  const orderIds=new Set(),activeShops=new Set(); rows.forEach(r=>{r.orderIds.forEach(x=>orderIds.add(x));r.shops.forEach(x=>{if(x)activeShops.add(x);});});
  return {range,rows,shop,category,unitFilter,search,statusFilter,sort,dir,totalProducts:rows.length,orderedProducts:rows.filter(r=>r.orderCount>0).length,neverProducts:rows.filter(r=>r.lifetimeOrders===0).length,orderCount:orderIds.size,activeShopCount:activeShops.size,growthCount:rows.filter(r=>r.trendKey==='growth').length,declineCount:rows.filter(r=>r.trendKey==='decline').length};
}
function statisticsQuery(url,changes={}){
  const q=new URLSearchParams(url.searchParams);
  Object.entries(changes).forEach(([key,value])=>{if(value===null||value===undefined||value==='')q.delete(key);else q.set(key,String(value));});
  return q.toString();
}
function statisticsPercent(value){
  if(value===null||value===undefined||!Number.isFinite(Number(value)))return '—';
  const n=Math.round(Number(value)*10)/10;
  return `${n>0?'+':''}${fmtNum(n)}%`;
}
function statisticsStatusOptions(selected){
  return [['','Усі статуси'],['frequent','Часто замовляють'],['regular','Замовляють'],['rare','Рідко замовляють'],['none','Без замовлень за період'],['never','Ще не замовляли'],['no_period','Замовляли раніше'],['new','Новий попит'],['growth','Попит зростає'],['decline','Попит знижується'],['stable','Стабільний попит']].map(([v,l])=>`<option value="${v}" ${selected===v?'selected':''}>${l}</option>`).join('');
}
function statisticsPage(db,url){
  const data=statisticsBuild(db,url),page=Math.max(1,Number.parseInt(url.searchParams.get('page')||'1',10)||1),perPage=50,totalPages=Math.max(1,Math.ceil(data.rows.length/perPage)),safePage=Math.min(page,totalPages),displayRows=data.rows.slice((safePage-1)*perPage,safePage*perPage);
  const categories=[...new Set((db.products||[]).map(p=>String(p.category||'')).filter(Boolean))].sort((a,b)=>a.localeCompare(b,'uk'));
  const units=[...new Set((db.products||[]).map(p=>normalizeUnit(p.resultUnit||p.packUnit||inferResultUnit(p))).filter(Boolean))].sort();
  const periods=[['all','За весь час'],['7','Останні 7 днів'],['30','Останні 30 днів'],['60','Останні 60 днів'],['90','Останні 90 днів'],['current_month','Поточний місяць'],['previous_month','Попередній місяць'],['custom','Власний період']];
  const sortLink=(key,label)=>{const active=data.sort===key,next=active&&data.dir==='desc'?'asc':'desc';return `<a href="/admin-statistics?${statisticsQuery(url,{sort:key,dir:next,page:1})}">${label}${active?(data.dir==='desc'?' ↓':' ↑'):''}</a>`;};
  const requestedRank=['quantity','orders','shops'].includes(String(url.searchParams.get('rank')||''))?String(url.searchParams.get('rank')):'orders';
  const rank=requestedRank==='quantity'&&!data.unitFilter?'orders':requestedRank;
  const rankValue=r=>rank==='quantity'?r.quantity:(rank==='shops'?r.shops.size:r.orderCount);
  const maxTop=Math.max(1,...data.rows.map(rankValue));
  const top=data.rows.filter(r=>r.orderCount>0).slice().sort((a,b)=>rankValue(b)-rankValue(a)||b.orderCount-a.orderCount||a.name.localeCompare(b.name,'uk')).slice(0,10);
  const empty=data.rows.filter(r=>r.orderCount===0).slice(0,10);
  const pages=Array.from({length:totalPages},(_,i)=>i+1).filter(n=>n===1||n===totalPages||Math.abs(n-safePage)<=2);
  let previousPage=0;const pagination=pages.map(n=>{const gap=previousPage&&n-previousPage>1?'<span class="statsPageGap">…</span>':'';previousPage=n;return gap+`<a class="${n===safePage?'active':''}" href="/admin-statistics?${statisticsQuery(url,{page:n})}">${n}</a>`;}).join('');
  const rowsHtml=displayRows.map((r,index)=>{const image=safeProductImagePath(r.product.image),last=r.lastMs?r.lastMs:r.lifetimeLastMs,days=last?Math.max(0,Math.floor((nowMs()-last)/86400000)):null;return `<tr><td class="statsNum">${(safePage-1)*perPage+index+1}</td><td><div class="statsProductCell">${image?`<img src="${esc(image)}" alt="" loading="lazy">`:'<span class="statsNoPhoto">📦</span>'}<div><b>${esc(r.name)}</b><small>${esc(r.category)}</small></div></div></td><td>${esc(r.unit)}</td><td class="statsNumber">${esc(fmtNum(r.quantity))}</td><td class="statsNumber">${r.orderCount}</td><td class="statsNumber">${r.shops.size}</td><td class="statsNumber">${esc(fmtNum(r.average))}</td><td>${r.lastMs?esc(statisticsDate(r.lastMs)):'Ще не замовляли'}</td><td class="statsNumber">${days===null?'—':days}</td><td class="statsNumber ${r.change>0?'statsUp':r.change<0?'statsDown':''}">${esc(statisticsPercent(r.change))}</td><td><span class="statsStatus statsStatus-${esc(r.trendKey)}">${esc(r.trendLabel)}</span><small class="statsActivity">${esc(r.activityLabel)}</small></td></tr>`;}).join('');
  const cards=[['Усього товарів',data.totalProducts,'📦'],['Замовляли',data.orderedProducts,'✓'],['Ще не замовляли',data.neverProducts,'○'],['Сформованих замовлень',data.orderCount,'🧾'],['Активних магазинів',data.activeShopCount,'🏪'],['Попит зростає',data.growthCount,'↗'],['Попит знижується',data.declineCount,'↘']];
  const topHtml=top.length?top.map((r,i)=>`<div class="statsTopRow"><span class="statsRank">${i+1}</span><div class="statsTopInfo"><div><b>${esc(r.name)}</b><span>${r.orderCount} замовл. · ${fmtNum(r.quantity)} ${esc(r.unit)} · ${r.shops.size} магаз.</span></div><small>${esc(r.category)} · останнє: ${esc(statisticsDate(r.lastMs))}</small><div class="statsBar"><i style="width:${Math.max(3,Math.round(rankValue(r)/maxTop*100))}%"></i></div></div></div>`).join(''):'<div class="statsEmpty">Сформованих замовлень ще немає.</div>';
  const emptyHtml=empty.length?empty.map(r=>`<div class="statsEmptyProduct"><b>${esc(r.name)}</b><span>${esc(r.category)} · ${esc(r.unit)}</span></div>`).join(''):'<div class="statsEmpty">Усі товари з вибірки замовляли.</div>';
  const compareNotice=data.range.period!=='all'&&!data.range.comparisonAvailable?'<div class="statsNotice statsNoticeNeutral">Ще недостатньо даних для порівняння з попереднім періодом.</div>':'';
  const rankLabel=rank==='quantity'?'замовленою кількістю':(rank==='shops'?'кількістю магазинів':'кількістю замовлень');
  const rankNotice=requestedRank==='quantity'&&!data.unitFilter?'<p class="statsRankNotice">Щоб порівнювати кількість коректно, спочатку виберіть одну одиницю вимірювання. Зараз рейтинг показано за кількістю замовлень.</p>':'';
  return `<div class="adminShell">${adminMenu('statistics')}<section class="statisticsPage"><div class="statsHeader"><div><h1>Статистика товарів</h1><p class="muted">Аналітика сформованих замовлень і завершених заявок усіх магазинів без урахування наявності.</p><small>Враховується вся збережена історія без обмеження датою початку статистики.</small></div><a class="btn secondary" href="/admin-statistics.xlsx?${statisticsQuery(url,{page:null})}">⬇ Експорт Excel</a></div>${compareNotice}<form class="card statsFilters" method="get" action="/admin-statistics"><label>Період<select name="period" onchange="toggleStatsCustom(this.value)">${periods.map(([v,l])=>`<option value="${v}" ${data.range.period===v?'selected':''}>${l}</option>`).join('')}</select></label><label>Магазин<select name="shop"><option value="">Усі магазини</option>${getShops(db).map(s=>`<option value="${esc(s.name)}" ${data.shop===s.name?'selected':''}>${esc(s.name)}</option>`).join('')}</select></label><label>Категорія<select name="category"><option value="">Усі категорії</option>${categories.map(c=>`<option value="${esc(c)}" ${data.category===c?'selected':''}>${esc(c)}</option>`).join('')}</select></label><label>Одиниця<select name="unit"><option value="">Усі одиниці</option>${units.map(u=>`<option value="${esc(u)}" ${data.unitFilter===u?'selected':''}>${esc(u)}</option>`).join('')}</select></label><label>Рейтинг<select name="rank"><option value="orders" ${requestedRank==='orders'?'selected':''}>За замовленнями</option><option value="quantity" ${requestedRank==='quantity'?'selected':''}>За кількістю</option><option value="shops" ${requestedRank==='shops'?'selected':''}>За магазинами</option></select></label><label>Статус<select name="status">${statisticsStatusOptions(data.statusFilter)}</select></label><label class="statsSearch">Товар<input name="q" value="${esc(url.searchParams.get('q')||'')}" placeholder="Пошук за назвою"></label><div class="statsCustomDates ${data.range.period==='custom'?'':'isHidden'}" id="statsCustomDates"><label>Від<input type="date" name="from" value="${esc(url.searchParams.get('from')||data.range.fromKey||statisticsDateKey(data.range.from))}"></label><label>До<input type="date" name="to" value="${esc(url.searchParams.get('to')||data.range.toKey||statisticsDateKey(data.range.to))}"></label></div><div class="statsFilterActions"><button type="submit">Застосувати</button><a class="btn secondary" href="/admin-statistics">Скинути фільтри</a></div></form><div class="statsCards">${cards.map(c=>`<div class="card statsMetric"><span>${c[2]}</span><div><small>${c[0]}</small><strong>${c[1]}</strong></div></div>`).join('')}</div><div class="statsOverviewGrid"><div class="card statsPanel"><div class="statsPanelHead"><div><h2>Найчастіше замовляють</h2><p class="muted">ТОП-10 за ${rankLabel}</p></div></div>${rankNotice}${topHtml}</div><div class="card statsPanel"><div class="statsPanelHead"><div><h2>Без замовлень</h2><p class="muted">Перші 10 товарів у поточній вибірці</p></div><a href="/admin-statistics?${statisticsQuery(url,{status:'none',page:1})}">Показати всі</a></div><div class="statsEmptyList">${emptyHtml}</div></div></div><div class="card statsTableCard"><div class="statsTableHead"><div><h2>Усі товари</h2><p class="muted">Знайдено: ${data.rows.length} · сторінка ${safePage} з ${totalPages}</p></div></div><div class="statsTableWrap"><table class="statsTable"><thead><tr><th>№</th><th>${sortLink('name','Товар')}</th><th>Од.</th><th>${sortLink('quantity','Кількість')}</th><th>${sortLink('orders','Замовлень')}</th><th>${sortLink('shops','Магазинів')}</th><th>Середнє</th><th>${sortLink('last','Останнє')}</th><th>Днів без замовл.</th><th>${sortLink('change','Зміна')}</th><th>Статус</th></tr></thead><tbody>${rowsHtml||'<tr><td colspan="11" class="statsEmpty">За вибраними фільтрами товарів немає.</td></tr>'}</tbody></table></div>${totalPages>1?`<nav class="statsPagination" aria-label="Сторінки">${safePage>1?`<a href="/admin-statistics?${statisticsQuery(url,{page:safePage-1})}">←</a>`:''}${pagination}${safePage<totalPages?`<a href="/admin-statistics?${statisticsQuery(url,{page:safePage+1})}">→</a>`:''}</nav>`:''}</div><script>function toggleStatsCustom(value){var box=document.getElementById('statsCustomDates');if(box)box.classList.toggle('isHidden',value!=='custom');}</script></section></div>`;
}
function statisticsXlsx(db,url){
  const data=statisticsBuild(db,url),rows=[['Товар','Категорія','Одиниця','Замовлена кількість','Кількість замовлень','Кількість магазинів','Середня кількість','Останнє замовлення','Днів без замовлення','Зміна','Статус','Активність']];
  data.rows.forEach(r=>{const last=r.lastMs||r.lifetimeLastMs,days=last?Math.max(0,Math.floor((nowMs()-last)/86400000)):'';rows.push([r.name,r.category,r.unit,r.quantity,r.orderCount,r.shops.size,Math.round(r.average*1000)/1000,r.lastMs?statisticsDate(r.lastMs):'Ще не замовляли',days,statisticsPercent(r.change),r.trendLabel,r.activityLabel]);});
  return genericXlsx('Статистика',rows,[42,22,10,19,18,18,18,20,20,14,24,22],'landscape');
}

/* ── Ринок: ізольований шаблон, щотижневі знімки та автозбереження ── */
const MARKET_UNITS=new Set(['kg','szt','L','ml','g']);
function marketUid(prefix){return `${prefix}_${nowMs()}_${crypto.randomBytes(4).toString('hex')}`;}
function marketState(db){
  if(!db.market||typeof db.market!=='object'||Array.isArray(db.market))db.market={};
  const m=db.market;
  m.template=Array.isArray(m.template)?m.template:[];
  m.sellers=Array.isArray(m.sellers)?m.sellers:[];
...
function marketUnit(value){return MARKET_UNITS.has(String(value||''))?String(value):'kg';}
function marketParsePriceCents(value){
...
function marketMoney(cents){return `${(Math.max(0,Number(cents)||0)/100).toFixed(2).replace('.',',')} zł`;}
function marketPriceInput(cents){return (Math.max(0,Number(cents)||0)/100).toFixed(2).replace('.',',');}
function marketParseQuantityMilli(value,unit){
  const raw=String(value??'').trim();
  if(raw==='')return {ok:true,value:null};
  const s=raw.replace(',','.');
  if(!/^\d+(?:\.\d{0,3})?$/.test(s))return {ok:false,error:'Введіть число не більш як із трьома знаками після коми'};
  const [whole='0',fraction='']=s.split('.');
  const milli=Number(whole)*1000+Number((fraction+'000').slice(0,3));
  if(!Number.isSafeInteger(milli)||milli<0)return {ok:false,error:'Некоректне значення'};
  if(!['kg','L'].includes(marketUnit(unit))&&milli%1000!==0)return {ok:false,error:'Для цієї одиниці введіть ціле число'};
  return {ok:true,value:milli};
}
function marketQuantity(milli,unit){
  if(milli===null||milli===undefined)return '';
  const n=Math.max(0,Number(milli)||0);
  if(['kg','L'].includes(marketUnit(unit)))return (n/1000).toFixed(3).replace('.',',');
  return String(Math.round(n/1000));
}
function marketProductSnapshot(item){
  return {id:String(item.id),name:String(item.name||''),priceCents:Math.max(0,Math.round(Number(item.priceCents)||0)),unit:marketUnit(item.unit),versions:{name:0,price:0,unit:0},createdAt:item.createdAt||warsawTime(),updatedAt:item.updatedAt||warsawTime()};
}
function marketSellerRows(market,sellerId){
  market.rows=market.rows&&typeof market.rows==='object'?market.rows:{};
  market.rows[sellerId]=market.rows[sellerId]&&typeof market.rows[sellerId]==='object'?market.rows[sellerId]:{};
  return market.rows[sellerId];
}
function marketEnsureRows(market,sellers){
  market.rows=market.rows&&typeof market.rows==='object'?market.rows:{};
  for(const seller of sellers){
    const rows=marketSellerRows(market,String(seller.id));
    for(const product of market.products||[]){
      if(!rows[product.id])rows[product.id]={accepted:null,returned:null,versions:{accepted:0,returned:0},updatedAt:''};
      rows[product.id].versions=rows[product.id].versions&&typeof rows[product.id].versions==='object'?rows[product.id].versions:{accepted:0,returned:0};
    }
  }
}
function marketCreateRecord(state,date){
  const products=state.template.map(marketProductSnapshot);
  const record={id:marketUid('market'),date,createdAt:warsawTime(),createdMs:nowMs(),updatedAt:warsawTime(),products,rows:{},sellerSnapshots:state.sellers.map(s=>({id:String(s.id),name:String(s.name||'')}))};
  marketEnsureRows(record,record.sellerSnapshots);
  return record;
}
function marketRecord(state,id){
  if(id&&state.active&&String(state.active.id)===String(id))return state.active;
  if(id)return state.history.find(x=>String(x.id)===String(id))||null;
  return state.active||null;
}
function marketSeller(record,id){return (record&&record.sellerSnapshots||[]).find(s=>String(s.id)===String(id))||null;}
function marketProduct(record,id){return (record&&record.products||[]).find(p=>String(p.id)===String(id))||null;}
function marketCalc(record,sellerId,productId){
  const product=marketProduct(record,productId),row=record&&record.rows&&record.rows[sellerId]&&record.rows[sellerId][productId];
  if(!product||!row)return {sold:null,sumCents:null};
  const hasAccepted=row.accepted!==null&&row.accepted!==undefined,hasReturned=row.returned!==null&&row.returned!==undefined;
  if(!hasAccepted||!hasReturned)return {sold:null,sumCents:null};
  const accepted=Number(row.accepted),returned=Number(row.returned),sold=Math.max(0,accepted-returned);
  return {sold,sumCents:Math.round(sold*Number(product.priceCents||0)/1000)};
}
function marketSellerTotal(record,sellerId){return (record&&record.products||[]).reduce((sum,p)=>sum+(marketCalc(record,sellerId,p.id).sumCents||0),0);}
function marketHasRowData(record,productId){
  return (record&&record.sellerSnapshots||[]).some(s=>{const row=record.rows&&record.rows[s.id]&&record.rows[s.id][productId];return row&&(row.accepted!==null&&row.accepted!==undefined||row.returned!==null&&row.returned!==undefined);});
}
function marketStyles(){return `<style>
.marketPage{width:100%;max-width:1280px;min-width:0}.marketTop,.marketTemplateHead,.marketHistoryHead{display:grid;grid-template-columns:minmax(0,1fr) auto;align-items:center;gap:16px}.marketTop{margin-bottom:14px}.marketTop h1,.marketHistoryHead h1{margin:0}.marketTopActions,.marketPrintActions,.marketTabs{display:flex;gap:8px;align-items:center;flex-wrap:wrap}.marketTopActions{justify-content:flex-end}.marketTopActions form{display:grid;grid-template-columns:minmax(145px,1fr) auto;gap:8px;margin:0}.marketTopActions input,.marketTopActions button{height:40px;margin:0}.marketTabs{margin:12px 0}.marketTabs a{padding:9px 14px;border:1px solid var(--line);border-radius:10px;text-decoration:none;color:var(--d);font-weight:800;background:#fff}.marketTabs a.active{background:var(--b);border-color:var(--b);color:#fff}.marketTemplate{padding:16px;margin-bottom:14px}.marketTemplateHead strong{font-size:16px}.marketSavedBadge{display:inline-block;color:#047857;background:#ecfdf5;border:1px solid #a7f3d0;border-radius:999px;padding:5px 9px;font-size:11px;font-weight:900}.marketForms{display:grid;grid-template-columns:minmax(220px,1.6fr) minmax(110px,.65fr) minmax(82px,.42fr) minmax(150px,auto);gap:10px;margin-top:12px;align-items:end}.marketForms label{min-width:0;font-size:11px;font-weight:800;color:var(--mut)}.marketForms input,.marketForms select,.marketForms button{width:100%;height:40px;margin-top:4px}.marketFormula{padding:11px 14px;background:#eff6ff;border:1px solid #bfdbfe;color:#1e40af;border-radius:12px;font-weight:800;font-size:13px;margin-bottom:12px}.marketStatus{min-height:22px;color:#64748b;font-size:12px;font-weight:800}.marketStatus.saving{color:#b45309}.marketStatus.saved{color:#047857}.marketStatus.error{color:#b91c1c}.marketTableCard{width:100%;min-width:0;overflow:hidden}.marketTableWrap{width:100%;min-width:0;overflow-x:auto}.marketTable{width:100%;border-collapse:collapse;table-layout:fixed;min-width:960px}.marketTable col.marketColName{width:24%}.marketTable col.marketColPrice{width:17%}.marketTable col.marketColAccepted,.marketTable col.marketColReturned{width:13%}.marketTable col.marketColSold{width:14%}.marketTable col.marketColSum{width:13%}.marketTable col.marketColActions{width:6%}.marketTable.marketHistoryTable col.marketColName{width:27%}.marketTable.marketHistoryTable col.marketColPrice{width:18%}.marketTable.marketHistoryTable col.marketColAccepted,.marketTable.marketHistoryTable col.marketColReturned,.marketTable.marketHistoryTable col.marketColSold{width:14%}.marketTable.marketHistoryTable col.marketColSum{width:13%}.marketTable th{background:#f8fafc;color:#475569;font-size:12px;text-align:left;padding:11px 8px;border-bottom:1px solid var(--line)}.marketTable td{padding:8px;border-bottom:1px solid #edf1f7;vertical-align:middle;min-width:0;overflow:hidden}.marketTable input,.marketTable select{box-sizing:border-box;width:100%;min-width:0;height:38px;border:1.5px solid #cbd5e1;border-radius:9px;padding:7px 9px;font:inherit;background:#fff}.marketPriceUnit{display:grid;grid-template-columns:minmax(0,1fr) 58px;gap:6px;align-items:center;min-width:0}.marketPriceUnit select{padding-left:6px;padding-right:4px;text-align:center}.marketCalculated{display:flex;align-items:center;background:#ecfdf5;color:#047857;font-weight:900;font-variant-numeric:tabular-nums;border-radius:9px;padding:8px;min-height:38px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.marketTable td:last-child form{display:flex;justify-content:center;margin:0}.marketDelete{display:inline-flex;align-items:center;justify-content:center;width:38px;height:38px;min-height:38px!important;padding:0!important;background:#fff1f2!important;color:#be123c!important;border:1px solid #fecdd3!important;box-shadow:none!important}.marketResultBox{margin-top:14px;padding:15px}.marketResultBox>button{min-width:190px}.marketResultValues{display:grid;grid-template-columns:minmax(0,420px);gap:10px;margin-top:10px}.marketResultValues[hidden]{display:none!important}.marketResultValue{background:#f8fafc;border:1px solid var(--line);border-radius:12px;padding:12px}.marketResultValue strong{display:block;font-size:22px;color:#1d4ed8}.marketHistoryGrid{display:grid;gap:10px;margin-top:14px}.marketHistoryCard{padding:16px;display:grid;grid-template-columns:minmax(0,1fr) auto;gap:16px;align-items:center}.marketHistoryMeta{display:flex;gap:12px;flex-wrap:wrap;color:var(--mut);font-size:12px;margin-top:5px}.marketHistoryActions{display:grid;grid-template-columns:repeat(4,minmax(105px,auto));gap:7px;align-items:stretch}.marketHistoryActions form{display:contents;margin:0}.marketHistoryActions .btn,.marketHistoryActions button{display:flex;align-items:center;justify-content:center;width:100%;height:40px;margin:0;white-space:nowrap}.marketPrintMenu,.marketPrintActions{display:grid;grid-template-columns:minmax(170px,1fr) auto auto;gap:8px;align-items:center}.marketPrintMenu>*,.marketPrintActions>*{height:40px;margin:0}.marketEmpty{padding:34px;text-align:center;color:var(--mut)}
@media(max-width:980px){.marketTop,.marketHistoryHead{grid-template-columns:1fr;align-items:stretch}.marketTopActions{justify-content:flex-start}.marketPrintMenu,.marketPrintActions{grid-template-columns:1fr 1fr}.marketHistoryCard{grid-template-columns:1fr}.marketHistoryActions{grid-template-columns:repeat(2,minmax(0,1fr))}}
@media(max-width:760px){.marketTop,.marketTemplateHead,.marketHistoryHead{align-items:stretch;grid-template-columns:1fr}.marketTopActions{display:grid;grid-template-columns:1fr}.marketTopActions form{grid-template-columns:1fr 1fr}.marketTopActions>*{width:100%}.marketTabs{overflow-x:auto;flex-wrap:nowrap;padding-bottom:4px}.marketTabs a{white-space:nowrap}.marketForms{grid-template-columns:minmax(0,1fr) 88px}.marketForms label:first-child{grid-column:1/-1}.marketForms button{grid-column:1/-1;width:100%}.marketFormula{display:none}.marketStatus{min-height:18px}.marketTableCard{border-radius:12px}.marketTableWrap{overflow-x:hidden}.marketTable{min-width:0;table-layout:auto}.marketTable thead,.marketTable colgroup{display:none}.marketTable,.marketTable tbody,.marketTable tr,.marketTable td{display:block;width:100%}.marketTable tr{display:grid;grid-template-columns:repeat(11,minmax(0,1fr)) 36px;grid-auto-flow:row;gap:3px 6px;padding:6px;border-bottom:1px solid var(--line);background:#fff}.marketTable tr:last-child{border-bottom:0}.marketTable td{border:0;padding:0;min-width:0;overflow:visible}.marketTable td:before{content:attr(data-label);display:block;font-size:8.5px;line-height:10px;color:var(--mut);font-weight:900;margin:0 0 1px;white-space:nowrap}.marketTable input,.marketTable select{height:32px;border-radius:8px;padding:4px 6px;font-size:13px;line-height:20px}.marketTable td:nth-child(1){grid-column:1/7;grid-row:1}.marketTable td:nth-child(2){grid-column:7/12;grid-row:1}.marketTable td:nth-child(3){grid-column:1/7;grid-row:2}.marketTable td:nth-child(4){grid-column:7/13;grid-row:2}.marketTable td:nth-child(5){grid-column:1/7;grid-row:3}.marketTable td:nth-child(6){grid-column:7/13;grid-row:3}.marketTable:not(.marketHistoryTable) td:nth-child(7){grid-column:12/13;grid-row:1;align-self:end}.marketTable:not(.marketHistoryTable) td:nth-child(7):before{display:none}.marketHistoryTable td:nth-child(2){grid-column:7/13}.marketTable .marketEmpty{grid-column:1/13;grid-row:auto;padding:18px}.marketPriceUnit{grid-template-columns:minmax(0,1fr) 46px;gap:3px}.marketPriceUnit select{padding-left:3px;padding-right:2px;font-size:12px}.marketCalculated{min-height:32px;height:32px;border-radius:8px;padding:4px 6px;font-size:12.5px;line-height:20px}.marketTable:not(.marketHistoryTable) td:last-child form{height:32px;align-items:center;justify-content:flex-end}.marketDelete{width:32px;height:32px;min-height:32px!important;font-size:13px;border-radius:8px!important}.marketResultValues{grid-template-columns:1fr}.marketHistoryActions{grid-template-columns:1fr 1fr}.marketPrintMenu,.marketPrintActions{grid-template-columns:1fr}.marketPrintMenu>*,.marketPrintActions>*{width:100%}}
</style>`;}
function marketActivePage(db,url){
  const state=marketState(db),record=state.active;
  const status=String(url.searchParams.get('status')||'');
  const notice=status==='created'?'<div class="successMsg">Новий ринок створено. Попередній збережено в історії.</div>':status==='product-added'?'<div class="successMsg">Товар додано до постійного шаблону.</div>':status==='seller-added'?'<div class="successMsg">Продавця додано.</div>':status==='product-deleted'?'<div class="successMsg">Позицію видалено з постійного шаблону.</div>':status==='invalid'?'<div class="error">Перевірте введені дані.</div>':'';
  const today=statisticsDateKey();
  if(!record){
    return `<div class="adminShell">${adminMenu('market')}<section class="marketPage">${marketStyles()}<div class="marketTop"><div><h1>Ринок</h1><p class="muted">Створіть перший ринок. Постійний шаблон товарів і продавців зберігатиметься для наступних тижнів.</p></div><a class="btn secondary" href="/admin-market/history">Історія ринків</a></div>${notice}<div class="card marketTemplate"><div class="marketTemplateHead"><div><strong>Постійний шаблон</strong> <span class="marketSavedBadge">Зберігається постійно</span><p class="muted">${state.template.length} товарів · ${state.sellers.length} продавців</p></div></div>${marketSetupForms(state)}</div><form class="card" method="post" action="/admin-market/new" style="padding:20px;max-width:520px" onsubmit="return confirm('Створити новий ринок?')"><h2>Створити перший ринок</h2><label style="display:block;margin:12px 0">Дата ринку<input type="date" name="date" value="${today}" required></label><button>+ Новий ринок</button></form></section></div>`;
  }
  marketEnsureRows(record,record.sellerSnapshots||[]);
  const sellerId=String(url.searchParams.get('seller')||((record.sellerSnapshots||[])[0]||{}).id||''),seller=marketSeller(record,sellerId)||((record.sellerSnapshots||[])[0]||null),activeSellerId=seller?String(seller.id):'';
  const rows=marketSellerRows(record,activeSellerId);
  const rowHtml=activeSellerId?(record.products||[]).map(product=>{const row=rows[product.id]||{accepted:null,returned:null,versions:{accepted:0,returned:0}},calc=marketCalc(record,activeSellerId,product.id),versions=row.versions||{},pVersions=product.versions||{};return `<tr data-market-row data-product-id="${esc(product.id)}" data-unit="${esc(product.unit)}"><td data-label="Назва товару"><input class="marketName" data-market-input data-field="name" data-version="${Number(pVersions.name)||0}" value="${esc(product.name)}" autocomplete="off"></td><td data-label="Ціна"><div class="marketPriceUnit"><input data-market-input data-field="price" data-version="${Number(pVersions.price)||0}" value="${esc(marketPriceInput(product.priceCents))}" inputmode="decimal"><select data-market-input data-field="unit" data-version="${Number(pVersions.unit)||0}" aria-label="Одиниця вимірювання">${[...MARKET_UNITS].map(u=>`<option value="${u}" ${u===product.unit?'selected':''}>${u}</option>`).join('')}</select></div></td><td data-label="Прийняв"><input data-market-input data-field="accepted" data-version="${Number(versions.accepted)||0}" value="${esc(marketQuantity(row.accepted,product.unit))}" inputmode="decimal" placeholder="Порожньо"></td><td data-label="Здав"><input data-market-input data-field="returned" data-version="${Number(versions.returned)||0}" value="${esc(marketQuantity(row.returned,product.unit))}" inputmode="decimal" placeholder="Порожньо"></td><td data-label="Продано"><div class="marketCalculated" data-sold>${calc.sold===null?'':esc(marketQuantity(calc.sold,product.unit)+' '+product.unit)}</div></td><td data-label="Сума"><div class="marketCalculated" data-sum>${calc.sumCents===null?'':esc(marketMoney(calc.sumCents))}</div></td><td data-label="Дії"><form method="post" action="/admin-market/product-delete" onsubmit="return marketDeleteProduct(this,${marketHasRowData(record,product.id)?'true':'false'})"><input type="hidden" name="id" value="${esc(product.id)}"><button class="marketDelete" title="Видалити позицію" aria-label="Видалити позицію">🗑️</button></form></td></tr>`;}).join(''):'';
  const tabs=(record.sellerSnapshots||[]).map(s=>`<a class="${String(s.id)===activeSellerId?'active':''}" href="/admin-market?seller=${encodeURIComponent(s.id)}" onclick="return marketNavigate(this)">${esc(s.name)}</a>`).join('');
  const queryBase=`marketId=${encodeURIComponent(record.id)}&seller=${encodeURIComponent(activeSellerId)}`;
  return `<div class="adminShell">${adminMenu('market')}<section class="marketPage">${marketStyles()}<div class="marketTop"><div><h1>Ринок</h1><p class="muted">Дата: <b>${esc(marketDateLabel(record.date))}</b></p></div><div class="marketTopActions"><a class="btn secondary" href="/admin-market/history" onclick="return marketNavigate(this)">Історія ринків</a><form class="marketFlushForm" method="post" action="/admin-market/new" onsubmit="return marketNewSubmit(this)"><input type="date" name="date" value="${today}" required><button>+ Новий ринок</button></form></div></div>${notice}<div class="card marketTemplate"><div class="marketTemplateHead"><div><strong>Шаблон товарів: ${state.template.length} позицій</strong> <span class="marketSavedBadge">Збережено постійно</span><p class="muted">Товари й актуальні ціни автоматично потраплять у наступний ринок.</p></div></div>${marketSetupForms(state)}</div><div class="marketTabs">${tabs||'<span class="muted">Спочатку додайте продавця</span>'}</div><div class="marketFormula">Продано = Прийняв − Здав &nbsp; • &nbsp; Сума = Продано × Ціна. Результат з’явиться після заповнення обох значень.</div><div id="marketSaveStatus" class="marketStatus saved">Збережено</div><div class="card marketTableCard"><div class="marketTableWrap"><table class="marketTable"><colgroup><col class="marketColName"><col class="marketColPrice"><col class="marketColAccepted"><col class="marketColReturned"><col class="marketColSold"><col class="marketColSum"><col class="marketColActions"></colgroup><thead><tr><th>Назва товару</th><th>Ціна</th><th>Прийняв</th><th>Здав</th><th>Продано</th><th>Сума</th><th>Дії</th></tr></thead><tbody>${rowHtml||`<tr><td colspan="7" class="marketEmpty">${activeSellerId?'Додайте перший товар до шаблону.':'Спочатку додайте продавця.'}</td></tr>`}</tbody></table></div></div><div class="card marketResultBox"><button type="button" class="secondary" aria-expanded="false" onclick="marketToggleResult(this)">Показати результат</button><div id="marketResultValues" class="marketResultValues" hidden><div class="marketResultValue"><small>Сума продавця ${esc(seller?seller.name:'')}</small><strong data-seller-total>${esc(marketMoney(marketSellerTotal(record,activeSellerId)))}</strong></div></div></div><div class="card marketResultBox"><div class="marketTemplateHead"><div><h2>Друк і PDF</h2><p class="muted">Друкується повна таблиця у вертикальному форматі A4.</p></div><div class="marketPrintActions"><select id="marketExportScope" aria-label="Продавці"><option value="seller">Поточний продавець</option><option value="all">Усі продавці</option></select><button type="button" class="secondary" onclick="marketExport('/admin-market/print','${queryBase}',true)">🖨️ Друк</button><button type="button" onclick="marketExport('/admin-market/pdf','${queryBase}',false)">⬇ PDF</button></div></div></div>${marketClientScript(record.id,activeSellerId)}</section></div>`;
}
function marketSetupForms(state){
  return `<div class="marketForms"><form method="post" action="/admin-market/product-add" style="display:contents"><label>Назва товару<input name="name" required placeholder="Назва товару"></label><label>Ціна<input name="price" value="0,00" inputmode="decimal" required></label><label>Одиниця<select name="unit">${[...MARKET_UNITS].map(u=>`<option value="${u}">${u}</option>`).join('')}</select></label><button>+ Додати товар</button></form></div><div class="marketForms" style="grid-template-columns:1fr auto"><form method="post" action="/admin-market/seller-add" style="display:contents"><label>Новий продавець<input name="name" required placeholder="Ім’я продавця"></label><button>+ Додати продавця</button></form></div>`;
}
function marketClientScript(marketId,sellerId){return `<script>
(()=>{const marketId=${JSON.stringify(String(marketId))},sellerId=${JSON.stringify(String(sellerId))},fields=new Map(),status=document.getElementById('marketSaveStatus');
function setStatus(kind,text){if(!status)return;status.className='marketStatus '+kind;status.textContent=text}
function fieldKey(input){const row=input.closest('[data-market-row]');return row?row.dataset.productId+':'+input.dataset.field:''}
function fieldState(input){const key=fieldKey(input);let state=fields.get(key);if(!state){state={key,input,row:input.closest('[data-market-row]'),desired:input.value,dirty:false,timer:null,running:null,error:''};fields.set(key,state)}return state}
function refreshStatus(){const states=[...fields.values()],failed=states.find(state=>state.error);if(failed)return setStatus('error',failed.error);if(states.some(state=>state.dirty||state.timer||state.running))return setStatus('saving','Збереження…');setStatus('saved','Збережено')}
function localNumber(v){const s=String(v||'').trim().replace(',','.');return /^\\d+(?:\\.\\d{0,3})?$/.test(s)?Number(s):null}
function updateLocal(row){const a=row.querySelector('[data-field="accepted"]'),r=row.querySelector('[data-field="returned"]'),p=row.querySelector('[data-field="price"]'),unit=row.dataset.unit;const av=localNumber(a&&a.value),rv=localNumber(r&&r.value),pv=localNumber(p&&p.value),hasA=a&&a.value.trim()!=='',hasR=r&&r.value.trim()!=='';let sold='';let sum='';if(hasA&&hasR&&av!==null&&rv!==null&&rv<=av){const s=av-rv;sold=(unit==='kg'||unit==='L'?s.toFixed(3).replace('.',','):String(Math.round(s)))+' '+unit;if(pv!==null)sum=(Math.round(s*pv*100)/100).toFixed(2).replace('.',',')+' zł'}row.querySelector('[data-sold]').textContent=sold;row.querySelector('[data-sum]').textContent=sum}
async function sendField(state,value){const input=state.input,row=state.row,data=new URLSearchParams({marketId,sellerId,productId:row.dataset.productId,field:input.dataset.field,value,expectedVersion:input.dataset.version||'0'}),response=await fetch('/admin-market/update',{method:'POST',headers:{'Content-Type':'application/x-www-form-urlencoded','X-Requested-With':'fetch'},body:data.toString()}),out=await response.json();if(!response.ok)throw Object.assign(new Error(out.error||'Не вдалося зберегти'),{conflict:out.conflict});input.dataset.version=String(out.fieldVersion||0);if(out.normalized!==undefined&&state.desired===value&&document.activeElement!==input){input.value=out.normalized;state.desired=String(out.normalized)}if(input.dataset.field==='unit')row.dataset.unit=out.normalized;updateLocal(row);const total=document.querySelector('[data-seller-total]');if(total)total.textContent=out.sellerTotal}
async function drainField(state){if(state.running)return state.running;state.running=(async()=>{while(state.dirty){state.dirty=false;const value=state.desired;try{await sendField(state,value);state.error='';inputError(state.input,false)}catch(error){state.dirty=true;state.error=error.conflict?'Дані цього поля вже змінено. Оновіть сторінку.':'Помилка збереження. Повторити';inputError(state.input,true);throw error}}})().finally(()=>{state.running=null;refreshStatus()});refreshStatus();return state.running}
function inputError(input,show){input.classList.toggle('inputError',show)}
function scheduleField(input,immediate){const state=fieldState(input);state.desired=input.value;state.dirty=true;state.error='';inputError(input,false);if(state.timer){clearTimeout(state.timer);state.timer=null}if(immediate){drainField(state).catch(()=>{})}else state.timer=setTimeout(()=>{state.timer=null;drainField(state).catch(()=>{})},400);refreshStatus()}
document.querySelectorAll('[data-market-input]').forEach(input=>{fieldState(input);input.addEventListener('input',()=>{const row=input.closest('[data-market-row]');updateLocal(row);scheduleField(input,false)});input.addEventListener('blur',()=>{const state=fieldState(input);if(state.dirty||state.timer)scheduleField(input,true)})});
window.marketFlush=async()=>{for(;;){for(const state of fields.values()){if(state.timer){clearTimeout(state.timer);state.timer=null}if(state.dirty&&!state.running)drainField(state).catch(()=>{})}const active=[...fields.values()].map(state=>state.running).filter(Boolean);if(active.length)await Promise.allSettled(active);const failed=[...fields.values()].find(state=>state.error);if(failed)throw new Error(failed.error);if(![...fields.values()].some(state=>state.dirty||state.timer||state.running))break}refreshStatus()};
window.marketNavigate=a=>{marketFlush().then(()=>location.href=a.href).catch(()=>{});return false};
window.marketNewSubmit=form=>{if(form.dataset.ready==='1')return true;if(!confirm('Створити новий ринок? Поточний ринок буде збережено в історії.'))return false;marketFlush().then(()=>{form.dataset.ready='1';form.submit()}).catch(()=>{});return false};
window.marketDeleteProduct=(form,hasData)=>{const text=hasData?'Ця позиція має заповнені значення. Видалити її з постійного шаблону? У поточному ринку та історії дані залишаться.':'Видалити цю позицію з постійного шаблону?';if(!confirm(text))return false;marketFlush().then(()=>form.submit()).catch(()=>{});return false};
window.marketToggleResult=button=>{const box=document.getElementById('marketResultValues');if(!box)return;const show=box.hidden;box.hidden=!show;button.setAttribute('aria-expanded',String(show));button.textContent=show?'Приховати результат':'Показати результат'};
window.marketExport=(path,base,newTab)=>{const scope=document.getElementById('marketExportScope').value;marketFlush().then(()=>{const url=path+'?'+base+'&mode=full&scope='+encodeURIComponent(scope);if(newTab)window.open(url,'_blank');else location.href=url}).catch(()=>{})};
})();
</script>`;}
function marketHistoryPage(db,url){
  const state=marketState(db),viewId=String(url.searchParams.get('id')||''),record=viewId?marketRecord(state,viewId):null;
  if(record){
    marketEnsureRows(record,record.sellerSnapshots||[]);
    const sellerId=String(url.searchParams.get('seller')||((record.sellerSnapshots||[])[0]||{}).id||''),seller=marketSeller(record,sellerId)||((record.sellerSnapshots||[])[0]||null),sid=seller?String(seller.id):'';
    const tabs=(record.sellerSnapshots||[]).map(s=>`<a class="${String(s.id)===sid?'active':''}" href="/admin-market/history?id=${encodeURIComponent(record.id)}&seller=${encodeURIComponent(s.id)}" onclick="return marketNavigate(this)">${esc(s.name)}</a>`).join('');
    const rows=(record.products||[]).map(p=>{const row=record.rows&&record.rows[sid]&&record.rows[sid][p.id]||{accepted:null,returned:null,versions:{accepted:0,returned:0}},calc=marketCalc(record,sid,p.id),versions=row.versions||{},pVersions=p.versions||{};return `<tr data-market-row data-product-id="${esc(p.id)}" data-unit="${esc(p.unit)}"><td data-label="Назва товару"><input class="marketName" data-market-input data-field="name" data-version="${Number(pVersions.name)||0}" value="${esc(p.name)}" autocomplete="off"></td><td data-label="Ціна"><div class="marketPriceUnit"><input data-market-input data-field="price" data-version="${Number(pVersions.price)||0}" value="${esc(marketPriceInput(p.priceCents))}" inputmode="decimal"><select data-market-input data-field="unit" data-version="${Number(pVersions.unit)||0}" aria-label="Одиниця вимірювання">${[...MARKET_UNITS].map(u=>`<option value="${u}" ${u===p.unit?'selected':''}>${u}</option>`).join('')}</select></div></td><td data-label="Прийняв"><input data-market-input data-field="accepted" data-version="${Number(versions.accepted)||0}" value="${esc(marketQuantity(row.accepted,p.unit))}" inputmode="decimal" placeholder="Порожньо"></td><td data-label="Здав"><input data-market-input data-field="returned" data-version="${Number(versions.returned)||0}" value="${esc(marketQuantity(row.returned,p.unit))}" inputmode="decimal" placeholder="Порожньо"></td><td data-label="Продано"><div class="marketCalculated" data-sold>${calc.sold===null?'':esc(marketQuantity(calc.sold,p.unit)+' '+p.unit)}</div></td><td data-label="Сума"><div class="marketCalculated" data-sum>${calc.sumCents===null?'':esc(marketMoney(calc.sumCents))}</div></td></tr>`;}).join('');
    const base=`marketId=${encodeURIComponent(record.id)}&seller=${encodeURIComponent(sid)}`;
    return `<div class="adminShell">${adminMenu('market')}<section class="marketPage">${marketStyles()}<div class="marketHistoryHead"><div><a href="/admin-market/history" onclick="return marketNavigate(this)">← Історія ринків</a><h1>Ринок за ${esc(marketDateLabel(record.date))}</h1><p class="muted">Історичні значення можна редагувати. Зміни не впливають на шаблон або активний ринок.</p></div><div class="marketPrintMenu"><select id="marketExportScope"><option value="seller">Поточний продавець</option><option value="all">Усі продавці</option></select><button type="button" class="secondary" onclick="marketExport('/admin-market/print','${base}',true)">Друк</button><button type="button" onclick="marketExport('/admin-market/pdf','${base}',false)">PDF</button></div></div><div class="marketTabs">${tabs}</div><div class="marketFormula">Продано й сума з’являться після заповнення полів «Прийняв» і «Здав».</div><div id="marketSaveStatus" class="marketStatus saved">Збережено</div><div class="card marketTableCard"><div class="marketTableWrap"><table class="marketTable marketHistoryTable"><colgroup><col class="marketColName"><col class="marketColPrice"><col class="marketColAccepted"><col class="marketColReturned"><col class="marketColSold"><col class="marketColSum"></colgroup><thead><tr><th>Назва товару</th><th>Ціна</th><th>Прийняв</th><th>Здав</th><th>Продано</th><th>Сума</th></tr></thead><tbody>${rows}</tbody></table></div></div><div class="card marketResultBox"><button type="button" class="secondary" aria-expanded="false" onclick="marketToggleResult(this)">Показати результат</button><div id="marketResultValues" class="marketResultValues" hidden><div class="marketResultValue"><small>Сума продавця ${esc(seller?seller.name:'')}</small><strong data-seller-total>${esc(marketMoney(marketSellerTotal(record,sid)))}</strong></div></div></div>${marketClientScript(record.id,sid)}</section></div>`;
  }
  const status=String(url.searchParams.get('status')||''),notice=status==='deleted'?'<div class="successMsg">Історію ринку видалено.</div>':'';
  const cards=state.history.slice().reverse().map(record=>`<div class="card marketHistoryCard"><div><h2>Ринок за ${esc(marketDateLabel(record.date))}</h2><div class="marketHistoryMeta"><span>${(record.sellerSnapshots||[]).length} продавців</span><span>${(record.products||[]).length} позицій</span></div></div><div class="marketHistoryActions"><a class="btn" href="/admin-market/history?id=${encodeURIComponent(record.id)}">Редагувати</a><a class="btn secondary" target="_blank" href="/admin-market/print?marketId=${encodeURIComponent(record.id)}&mode=full&scope=all">Друк</a><a class="btn secondary" href="/admin-market/pdf?marketId=${encodeURIComponent(record.id)}&mode=full&scope=all">PDF</a><form method="post" action="/admin-market/history-delete" onsubmit="return confirm('Видалити історію ринку за ${esc(marketDateLabel(record.date))}? Цю дію неможливо скасувати.')"><input type="hidden" name="id" value="${esc(record.id)}"><button class="danger">Видалити історію</button></form></div></div>`).join('');
  return `<div class="adminShell">${adminMenu('market')}<section class="marketPage">${marketStyles()}<div class="marketHistoryHead"><div><h1>Історія ринків</h1><p class="muted">Завершені щотижневі ринки не змінюються разом із шаблоном.</p></div><a class="btn secondary" href="/admin-market">До активного ринку</a></div>${notice}<div class="marketHistoryGrid">${cards||'<div class="card marketEmpty">Історія поки порожня.</div>'}</div></section></div>`;
}
function marketPrintRows(record,seller,mode){
  const rows=marketSellerRows(record,String(seller.id));
  return (record.products||[]).map((p,index)=>{const row=rows[p.id]||{},calc=marketCalc(record,String(seller.id),p.id);return `<tr><td>${index+1}</td><td>${esc(p.name)}</td><td>${esc(marketMoney(p.priceCents))}</td><td>${esc(p.unit)}</td>${mode==='full'?`<td>${esc(marketQuantity(row.accepted,p.unit))}</td><td>${esc(marketQuantity(row.returned,p.unit))}</td><td>${calc.sold===null?'':esc(marketQuantity(calc.sold,p.unit))}</td><td>${calc.sumCents===null?'':esc(marketMoney(calc.sumCents))}</td>`:''}</tr>`;}).join('');
}
function marketPrintPage(record,{mode='full',scope='seller',sellerId='',auto=false}={}){
  mode=mode==='names'?'names':'full';const sellers=scope==='all'?(record.sellerSnapshots||[]):[marketSeller(record,sellerId)||((record.sellerSnapshots||[])[0])].filter(Boolean);
  const sections=sellers.map(s=>`<section class="seller"><h2>Продавець: ${esc(s.name)}</h2><table><thead><tr><th>№</th><th>Назва товару</th><th>Ціна</th><th>Од.</th>${mode==='full'?'<th>Прийняв</th><th>Здав</th><th>Продано</th><th>Сума</th>':''}</tr></thead><tbody>${marketPrintRows(record,s,mode)}</tbody>${mode==='full'?`<tfoot><tr><td colspan="7">Результат продавця</td><td>${esc(marketMoney(marketSellerTotal(record,s.id)))}</td></tr></tfoot>`:''}</table></section>`).join('');
  return `<!doctype html><html lang="uk"><head><meta charset="utf-8"><title>Ринок ${esc(marketDateLabel(record.date))}</title><style>@page{size:A4 portrait;margin:10mm}*{box-sizing:border-box}body{font-family:Arial,sans-serif;color:#111;margin:0}.tools{position:fixed;right:12px;top:12px}.tools button{padding:10px 16px}h1{margin:0 0 4px}header{margin-bottom:14px}header p{margin:0;color:#555}.seller{page-break-after:always}.seller:last-child{page-break-after:auto}.seller h2{font-size:14px;margin:0 0 8px}table{width:100%;border-collapse:collapse;table-layout:fixed;font-size:${mode==='full'?'8.5':'10'}px}th,td{border:1px solid #777;padding:${mode==='full'?'4':'6'}px;text-align:left;overflow-wrap:anywhere}th{background:#eef3f8}th:first-child,td:first-child{width:5%}th:nth-child(2),td:nth-child(2){width:${mode==='full'?'29':'55'}%}tfoot td{font-weight:bold;background:#f6f8fb}@media print{.tools{display:none}}</style></head><body><div class="tools"><button onclick="window.print()">Друк / зберегти PDF</button></div><header><h1>Ринок</h1><p>Дата: ${esc(marketDateLabel(record.date))}</p></header>${sections||'<p>Продавців немає.</p>'}${auto?'<script>setTimeout(()=>window.print(),250)</script>':''}</body></html>`;
}
function marketPdfFontFiles(){
  const first=candidates=>candidates.find(file=>file&&fs.existsSync(file));let packageRegular='',packageBold='';
  try{const pkg=require.resolve('dejavu-fonts-ttf/package.json'),dir=path.join(path.dirname(pkg),'ttf');packageRegular=path.join(dir,'DejaVuSans.ttf');packageBold=path.join(dir,'DejaVuSans-Bold.ttf');}catch(_){}
  const regular=first([packageRegular,'/usr/share/fonts/truetype/dejavu/DejaVuSans.ttf','/usr/share/fonts/truetype/liberation2/LiberationSans-Regular.ttf']);
  const bold=first([packageBold,'/usr/share/fonts/truetype/dejavu/DejaVuSans-Bold.ttf','/usr/share/fonts/truetype/liberation2/LiberationSans-Bold.ttf',regular]);return {regular,bold};
}
function marketPdf(record,{mode='full',scope='seller',sellerId=''}={}){
  mode=mode==='names'?'names':'full';const sellers=scope==='all'?(record.sellerSnapshots||[]):[marketSeller(record,sellerId)||((record.sellerSnapshots||[])[0])].filter(Boolean);
  return new Promise((resolve,reject)=>{try{const fonts=marketPdfFontFiles();if(!fonts.regular)throw new Error('PDF font with Cyrillic support was not found');const doc=new PDFDocument({size:'A4',layout:'portrait',margins:{top:34,bottom:34,left:28,right:28},info:{Title:`Ринок ${marketDateLabel(record.date)}`}}),chunks=[];doc.on('data',c=>chunks.push(c));doc.on('end',()=>resolve(Buffer.concat(chunks)));doc.on('error',reject);doc.registerFont('MarketSans',fonts.regular);doc.registerFont('MarketBold',fonts.bold||fonts.regular);doc.font('MarketBold').fontSize(18).text('Ринок');doc.font('MarketSans').fontSize(10).text(`Дата: ${marketDateLabel(record.date)}`);const headers=mode==='full'?['№','Назва товару','Ціна','Од.','Прийняв','Здав','Продано','Сума']:['№','Назва товару','Ціна','Од.'];const pageWidth=doc.page.width-doc.page.margins.left-doc.page.margins.right;const widths=mode==='full'?[24,pageWidth-24-57-31-56-56-61-72,57,31,56,56,61,72]:[28,pageWidth-28-90-45,90,45];const rowH=24;const drawRow=(values,bold=false)=>{let x=doc.page.margins.left,y=doc.y;for(let i=0;i<values.length;i++){doc.rect(x,y,widths[i],rowH).stroke('#77808d');doc.font(bold?'MarketBold':'MarketSans').fontSize(mode==='full'?7.2:8.5).fillColor('#111').text(String(values[i]??''),x+3,y+7,{width:widths[i]-6,height:rowH-9,ellipsis:true});x+=widths[i];}doc.y=y+rowH;};const drawHeader=()=>drawRow(headers,true);sellers.forEach((seller,sellerIndex)=>{if(sellerIndex)doc.addPage();doc.moveDown(.8).font('MarketBold').fontSize(12).text(`Продавець: ${seller.name}`);doc.moveDown(.4);drawHeader();const rows=marketSellerRows(record,String(seller.id));(record.products||[]).forEach((p,index)=>{if(doc.y+rowH>doc.page.height-doc.page.margins.bottom){doc.addPage();drawHeader();}const row=rows[p.id]||{},calc=marketCalc(record,String(seller.id),p.id),values=[index+1,p.name,marketMoney(p.priceCents),p.unit];if(mode==='full')values.push(marketQuantity(row.accepted,p.unit),marketQuantity(row.returned,p.unit),calc.sold===null?'':marketQuantity(calc.sold,p.unit),calc.sumCents===null?'':marketMoney(calc.sumCents));drawRow(values);});if(mode==='full'){doc.moveDown(.5);const totalY=doc.y;doc.font('MarketBold').fontSize(11).text(`Результат продавця: ${marketMoney(marketSellerTotal(record,seller.id))}`,doc.page.margins.left,totalY,{width:pageWidth,align:'right'});}});doc.end();}catch(error){reject(error);}});
}
...
function originalOrderAsApplication(o,db=null){
  const items=(Array.isArray(o.items)?o.items:[]).map((i,idx)=>{
    ...
    return {
      ...
      depositType:orderItemDepositType(db,item),
      depositCount:totalUnits,
...
function orderItemDepositType(db,item){ const direct=productDepositType(item); if(direct)return direct; if(!db)return ''; const productId=String(item&&item.productId||item&&item.id||''); const p=(db.products||[]).find(x=>String(x.id)===productId); return productDepositType(p); }
...
function ensureOrderNumbers(db,persist=true){
  db.orders = db.orders || [];
  const used = new Set();
  let changed = false;
...
function copyProductFields(p){ const e=enrichProduct(p); const depositType=productDepositType(e); const hasDeposit=(!!e.hasDeposit||!!depositType) && canHaveDeposit(e.category); return {id:e.id, productId:e.id, name:e.name, category:e.category, weight:String(e.weight||e.format||''), format:e.format, productType:'universal', packQty:e.packQty, packUnit:normalizeUnit(e.resultUnit||e.packUnit), resultUnit:normalizeUnit(e.resultUnit||e.packUnit), hasDeposit, depositType, displayWeight:hasDeposit?'кауція':''}; }
...
function shopOrdersFor(db, shop){
  return (db.orders||[]).filter(o=>String(o.shop||'')===String(shop||'')).sort((a,b)=>(Number(b.id)||0)-(Number(a.id)||0));
}
function shopOrderDetailsHtml(o){
  return `<div class="historyItemsTable"><div class="historyItemsHead"><span>Назва товару</span><span>Замовлена кількість</span></div>${(o.items||[]).map(i=>`<div class="historyItemRow"><div class="historyItemName">${esc(productDisplayName(i))}</div><div class="historyItemQty">${esc(productTotalDisplay(i,i.qty))}</div></div>`).join('')}</div>${o.comment?`<div class="orderComment"><div class="orderCommentLabel">Коментар:</div>${esc(o.comment)}</div>`:''}`;
}
function shopOrderHistoryCard(o){
  const editable=canShopEditOrder(o);
  return `<details class="card historyOrder historyOrderGridCard" data-order-id="${esc(o.id)}" ontoggle="loadLazyOrderDetails(this)"><summary class="historyOrderHead historyOrderSummary"><div class="historyOrderSummaryMain"><span class="historyOrderArrow" aria-hidden="true">▸</span><div><h3>Замовлення №${o.orderNo||o.id}</h3><div class="historyMeta">${esc(o.createdAt||'')} · ${esc(o.status||'Нове')} · ${(o.items||[]).length} позицій</div></div></div>${editable?`<a class="btn historyEditBtn" href="/order-edit?id=${encodeURIComponent(o.id)}" onclick="event.stopPropagation()" aria-label="Редагувати замовлення">✏️ <span>Редагувати</span></a>`:`<span class="historyLockedBadge">🔒 Накладну надруковано</span>`}</summary><div class="historyOrderDetails" data-order-lazy data-url="/orders/history-details?id=${encodeURIComponent(o.id)}"><div class="muted center" style="padding:18px">Відкрийте замовлення, щоб завантажити позиції</div></div></details>`;
}
function shopOrderHistoryHtml(db, shop){
  const orders=shopOrdersFor(db,shop),first=orders.slice(0,ORDER_PAGE_SIZE),hasMore=first.length<orders.length;
  const list=first.map(shopOrderHistoryCard).join('');
  return `<section class="orderHistory"><div class="historyTitleRow"><h2>Історія попередніх замовлень</h2></div><div id="orderHistoryList">${list||'<div class="card historyEmpty">Попередніх замовлень ще немає</div>'}</div><div class="center" data-order-load-wrap style="padding:10px 0 20px;${hasMore?'':'display:none'}"><button type="button" class="btn secondary" data-url="/orders/history-chunk" data-target="orderHistoryList" data-offset="${first.length}" onclick="loadMoreOrderCards(this)">Показати ще 20 замовлень</button><div class="muted" data-order-progress style="font-size:12px;margin-top:8px">Показано ${first.length} із ${orders.length}</div></div></section>`;
}
...
function saveCart(session){ const db=readDb(); db.carts=db.carts||{}; db.carts[cartKey(session)]=session.cart||[]; writeDb(db,{cartWrite:true}); }
const processedCartOperations=new Map();
const CART_OPERATION_TTL_MS=60*60*1000;
function validCartOperationId(value){return /^[A-Za-z0-9._:-]{8,128}$/.test(String(value||''));}
function cartOperationKey(session,operationId){return cartKey(session)+':'+String(operationId||'');}
function cleanupCartOperations(){
  const cutoff=nowMs()-CART_OPERATION_TTL_MS;
  for(const [key,value] of processedCartOperations){if(Number(value||0)<cutoff)processedCartOperations.delete(key);}
  while(processedCartOperations.size>10000)processedCartOperations.delete(processedCartOperations.keys().next().value);
}
function cartQuantityValue(value){
  if(value===undefined||value===null||value==='')return null;
  const n=Number(value);
  return Number.isFinite(n)&&Number.isInteger(n)&&n>=0&&n<=1000000?n:null;
}
function cartPayload(session,product,id,operationId='',extra={}){
  const item=(session.cart||[]).find(x=>String(x.id)===String(id));
  const itemQty=item?Number(item.qty||0):0;
  const displayItem=item||product||{};
  return {ok:true,count:(session.cart||[]).reduce((a,i)=>a+Number(i.qty||0),0),positions:(session.cart||[]).length,id:String(id),itemQty,result:productResultText(displayItem,itemQty),catalogTotal:productCatalogTotalText(displayItem,itemQty),operationId:String(operationId||''),saved:true,...extra};
}
function applyCartTargetUpdate(db,session,bodyData){
  const id=String(bodyData.id||'');
  const product=(db.products||[]).find(x=>String(x.id)===id&&!x.hidden);
  if(!product)return {status:404,data:{ok:false,error:'Товар не знайдено',id,operationId:String(bodyData.operationId||''),saved:false}};
  const operationId=String(bodyData.operationId||'');
  const expectedQty=cartQuantityValue(bodyData.expectedQty),targetQty=cartQuantityValue(bodyData.targetQty);
  if(!validCartOperationId(operationId)||expectedQty===null||targetQty===null)return {status:400,data:{ok:false,error:'Некоректна зміна кількості',id,operationId,saved:false}};
  cleanupCartOperations();
  const operationKey=cartOperationKey(session,operationId);
  if(processedCartOperations.has(operationKey))return {status:200,data:cartPayload(session,product,id,operationId,{deduplicated:true})};
  const item=(session.cart||[]).find(x=>String(x.id)===id);
  const actualQty=item?Number(item.qty||0):0;
  if(!Number.isFinite(actualQty)||actualQty<0)return {status:409,data:{ok:false,conflict:true,error:'Некоректна поточна кількість',id,operationId,actualQty:0,saved:false}};
  // Повтор після втрати відповіді (зокрема після перезапуску процесу): якщо
  // ціль уже записана, операція завершена і вдруге кількість не змінюємо.
  if(actualQty===targetQty){processedCartOperations.set(operationKey,nowMs());return {status:200,data:cartPayload(session,product,id,operationId,{deduplicated:true})};}
  if(actualQty!==expectedQty)return {status:409,data:{ok:false,conflict:true,error:'Кошик змінився на іншому пристрої',id,operationId,actualQty,count:(session.cart||[]).reduce((a,i)=>a+Number(i.qty||0),0),positions:(session.cart||[]).length,saved:false}};
  if(targetQty<=0){session.cart=(session.cart||[]).filter(x=>String(x.id)!==id);}
  else if(item){item.qty=targetQty;}
  else{session.cart.push({...copyProductFields(product),qty:targetQty});}
  saveCart(session);
  processedCartOperations.set(operationKey,nowMs());
  return {status:200,data:cartPayload(session,product,id,operationId)};
}
...
.cartSaveStatus{position:fixed;z-index:900;top:72px;right:14px;padding:6px 10px;border-radius:999px;background:#eff6ff;border:1px solid #bfdbfe;color:#1d4ed8;font-size:11px;font-weight:800;line-height:1.2;box-shadow:0 4px 12px rgba(37,99,235,.12);opacity:0;visibility:hidden;pointer-events:none;transition:opacity .18s ease,visibility .18s ease}
.cartSaveStatus.visible{opacity:1;visibility:visible}
.cartSaveStatus.error{background:#fff7ed;border-color:#fdba74;color:#9a3412}
@media(max-width:700px){.cartSaveStatus{top:70px;left:14px;right:auto}}
...
@media(min-width:701px){
  .adminProductsSection .adminProductsTable{width:100%;min-width:980px;table-layout:fixed}
  .adminProductsSection .adminProductsTable th:nth-child(1){width:52px}
  .adminProductsSection .adminProductsTable th:nth-child(3){width:170px}
  .adminProductsSection .adminProductsTable th:nth-child(4){width:132px}
  .adminProductsSection .adminProductsTable th:nth-child(5){width:108px}
  .adminProductsSection .adminProductsTable th:nth-child(6){width:88px}
  .adminProductsSection .adminProductsTable th:nth-child(7){width:52px}
  .adminProductsSection .adminProductsTable th:nth-child(8){width:46px}
  .adminProductsSection .adminProductsTable th,
  .adminProductsSection .adminProductsTable td{box-sizing:border-box;vertical-align:middle}
  .adminProductsSection .adminProductsTable .mainCell{min-width:0;overflow-wrap:anywhere}
  .adminProductsSection .adminProductsTable tr.isProductEditing td{background:#f8fbff}
  .adminProductsSection .adminProductsTable tr.isProductEditing .mainCell,
  .adminProductsSection .adminProductsTable tr.isProductEditing .weight{vertical-align:top}
  .adminProductsSection .adminProductsTable .editInlineInput{height:38px;min-width:0!important;max-width:100%;margin:0}
  .adminProductsSection .adminProductsTable input[name="editName"]{display:block;width:100%!important}
  .adminProductsSection .adminProductsTable .editMeasureWrap{display:grid!important;grid-template-columns:minmax(0,1fr) 64px;align-items:center;gap:6px!important;width:100%}
  .adminProductsSection .adminProductsTable .editMeasureWrap input{width:100%!important}
  .adminProductsSection .adminProductsTable .editMeasureWrap select{width:64px!important}
  .adminProductsSection .adminProductsTable tr.isProductEditing .editIconBtn{display:grid;place-items:center;width:38px;height:38px;min-height:38px;margin:0 auto;padding:0}
}
...
@media(max-width:700px){
  ...
  .adminProductsTable tr.isProductEditing{align-items:start}
  .adminProductsTable .editMeasureWrapMobile{display:grid!important;grid-template-columns:minmax(0,1fr) 74px;align-items:center;gap:6px!important;width:100%;min-width:0}
  .adminProductsTable .editMeasureWrapMobile .editInlineInput{width:100%!important;min-width:0!important;height:40px;margin:0}
}
@media(max-width:380px){
...
.missingProductsList{display:grid;gap:10px}.missingProductRow{display:grid;grid-template-columns:34px minmax(0,1fr) 150px 132px;gap:14px;align-items:center;padding:14px 16px;border:1px solid #e5e7eb;border-radius:14px;background:#fff}.missingProductSelect{display:grid;place-items:center}.missingProductCheck{appearance:none;-webkit-appearance:none;width:24px;height:24px;margin:0;border:2px solid #94a3b8;border-radius:7px;background:#fff;cursor:pointer;position:relative}.missingProductCheck:checked{background:#16a34a;border-color:#16a34a}.missingProductCheck:checked:after{content:'✓';position:absolute;inset:0;display:grid;place-items:center;color:#fff;font-size:16px;font-weight:900;line-height:1}.missingProductInfo{min-width:0;display:flex;align-items:flex-start;gap:9px}.missingProductIcon{flex:0 0 auto;width:30px;height:30px;border-radius:9px;background:#fee2e2;color:#991b1b;display:grid;place-items:center;font-weight:900}.missingProductText{min-width:0}.missingProductName{display:block;font-size:16px;overflow-wrap:anywhere}.missingProductMeta{margin-top:4px;font-size:13px}.missingProductTimer{text-align:center;white-space:nowrap}.missingProductCountdown{font-weight:800;color:#dc2626;font-variant-numeric:tabular-nums}.missingProductTimerLabel{font-size:12px;margin-top:4px}.missingProductAction{margin:0}.missingProductAction button{width:100%;white-space:nowrap;background:#16a34a;border-color:#16a34a}.missingBatchBar{display:flex;align-items:center;gap:9px;flex-wrap:wrap;margin:0 0 12px}.missingBatchBar button{background:#16a34a;border-color:#16a34a}.missingBatchCount{font-size:12px;font-weight:800;color:#64748b}
@media(max-width:700px){.missingProductsList{gap:8px}.missingProductRow{grid-template-columns:28px minmax(0,1fr) 86px 102px;gap:7px;padding:10px 8px;border-radius:11px;align-items:center}.missingProductCheck{width:22px;height:22px;border-radius:6px}.missingProductInfo{gap:7px;align-items:center}.missingProductIcon{width:25px;height:25px;border-radius:7px;font-size:13px}.missingProductName{font-size:13px;line-height:1.2}.missingProductMeta{font-size:10px;line-height:1.25;margin-top:3px}.missingProductTimer{width:86px;text-align:center}.missingProductCountdown{font-size:11px;line-height:1.2}.missingProductTimerLabel{font-size:9px;margin-top:2px}.missingProductAction{width:102px}.missingProductAction button{width:102px;min-height:36px;padding:6px 5px;font-size:10px;border-radius:9px}.missingBatchBar{position:sticky;top:6px;z-index:8;background:#fff;padding:8px;border:1px solid #e5e7eb;border-radius:12px;box-shadow:0 4px 14px rgba(15,23,42,.08)}}
@media(max-width:390px){.missingProductRow{grid-template-columns:26px minmax(0,1fr) 76px 92px;gap:5px;padding:9px 6px}.missingProductTimer{width:76px}.missingProductAction{width:92px}.missingProductAction button{width:92px;padding:5px 3px;font-size:9px}.missingProductIcon{display:none}.missingProductInfo{gap:0}.missingProductName{font-size:12px}.missingProductMeta{font-size:9px}.missingProductCheck{width:20px;height:20px}}
...
/* Android / small-phone compact barcode binding sheet */
@media(max-width:700px){#bindModal{padding:4px!important;align-items:stretch!important;overflow:hidden!important}#bindModal .appModalBox{height:calc(100dvh - 8px)!important;max-height:calc(100dvh - 8px)!important;border-radius:14px!important;padding:9px 10px calc(8px + env(safe-area-inset-bottom))!important}#bindModal .appModalBox>h2{font-size:18px!important;line-height:1.15!important;margin:0 0 2px!important}#bindModal .appModalBox>p{font-size:11px!important;line-height:1.2!important;margin:1px 0 4px!important}#bindModal .appBindSearchGrid{grid-template-columns:1fr 1fr!important;gap:6px!important}#bindModal label{font-size:11px!important}#bindModal .appBindSearchGrid label,#bindModal .appBindGrid label{margin:2px 0!important}#bindModal select,#bindModal input{min-height:36px!important;height:36px!important;padding:6px 8px!important;font-size:14px!important}#bindModal .bindProductList{min-height:70px!important;margin:5px 0!important;border-radius:9px!important}#bindModal .bindProductRow{min-height:40px!important;padding:7px 9px!important;font-size:12px!important;gap:7px!important}#bindModal .bindProductRow small{font-size:10px!important;padding:2px 6px!important}#bindModal .appBindGrid{grid-template-columns:1fr 1fr!important;gap:6px!important}#bindModal .appBindFieldTitle{font-size:10px!important}#bindModal .actions{grid-template-columns:88px 1fr!important;gap:6px!important;margin-top:4px!important;padding-top:5px!important}#bindModal .actions button{min-height:40px!important;height:40px!important;padding:6px 7px!important;font-size:11px!important;border-radius:9px!important}}
@media(max-width:390px){#bindModal .appBindSearchGrid{grid-template-columns:1fr!important}#bindModal .appBindGrid{grid-template-columns:1fr 1fr!important}#bindModal .appModalBox>h2{font-size:17px!important}#bindModal .actions{grid-template-columns:82px 1fr!important}}
...
/* Shop favorites: unobtrusive star on the photo and the 8th catalog tile */
.favoriteStar{position:absolute;right:10px;bottom:8px;z-index:8;width:34px;height:34px;border:0!important;border-radius:50%;padding:0!important;display:flex;align-items:center;justify-content:center;background:rgba(255,255,255,.70)!important;color:#64748b!important;font-size:25px!important;line-height:1!important;box-shadow:0 2px 8px rgba(15,23,42,.14)!important;backdrop-filter:blur(3px);-webkit-backdrop-filter:blur(3px);cursor:pointer;transition:transform .16s ease,color .16s ease,background .16s ease,box-shadow .16s ease}
.favoriteStar:hover{transform:scale(1.08);background:rgba(255,255,255,.92)!important}
.favoriteStar:active{transform:scale(.94)}
.favoriteStar.isFavorite{color:#f59e0b!important;background:rgba(255,251,235,.94)!important;box-shadow:0 3px 11px rgba(245,158,11,.32)!important}
.favoriteStar.isBusy{pointer-events:none;opacity:.65}
.catFavoriteIcon{font-size:28px;color:#f59e0b;line-height:1}
@media(max-width:700px){.favoriteStar{right:8px;bottom:7px;width:31px;height:31px;font-size:23px!important}}

/* Shop catalog pagination; card layout remains unchanged. */
.catalogPagination{padding:20px 0 8px;text-align:center;max-width:100%}
.catalogPaginationNav{display:flex;align-items:center;justify-content:center;flex-wrap:wrap;gap:8px;max-width:100%}
.catalogPageButton{min-width:40px;height:42px;padding:0 12px;border:1px solid #93c5fd;border-radius:12px;background:#f8fbff;color:#2563eb;font-size:14px;font-weight:800;line-height:40px;text-align:center;text-decoration:none;white-space:nowrap;box-shadow:0 1px 2px rgba(37,99,235,.06)}
.catalogPageButton:hover{background:#eff6ff;border-color:#60a5fa}
.catalogPageButton.isActive{background:#2563eb;color:#fff;border-color:#2563eb;box-shadow:0 5px 14px rgba(37,99,235,.24)}
.catalogPageButton.isDisabled{color:#94a3b8;background:#f8fafc;border-color:#e2e8f0;box-shadow:none;pointer-events:none}
.catalogPageDirection{min-width:92px}
.catalogPageEllipsis{min-width:22px;color:#64748b;font-size:18px;font-weight:800;line-height:42px;text-align:center}
.catalogPaginationProgress{margin-top:10px;color:#64748b;font-size:12px}
@media(max-width:700px){.catalogPagination{padding:16px 0 6px}.catalogPaginationNav{gap:6px}.catalogPageButton{min-width:34px;height:38px;padding:0 9px;border-radius:10px;font-size:13px;line-height:36px}.catalogPageDirection{min-width:74px;padding:0 9px}.catalogPageEllipsis{min-width:16px;line-height:38px}.catalogPaginationProgress{padding:0 6px;line-height:1.35}}
...
/* Shop catalog: quick quantity buttons use the existing cart-add flow. */
.prodGrid{grid-auto-rows:464px}
.prodCard,
.prodCard.hasProductImage{
...
.prodCardQuickQty form{display:block;min-width:0;margin:0}
.prodCardQuickQty .quickQtyBtn{
  display:flex;
  align-items:center;
  justify-content:center;
  width:100%!important;
...
.prodCardQuickQty .quickQtyBtn:hover{background:#dbeafe!important;border-color:#60a5fa!important}
.prodCardQuickQty .quickQtyBtn:active{background:#bfdbfe!important;transform:scale(.98)}
@media(max-width:700px){
  .prodGrid{grid-auto-rows:394px!important}
  .prodCard,
  .prodCard.hasProductImage{
...
/* Warehouse statistics: isolated styles, aligned with the existing cabinet UI. */
.adminMenu a.active{background:#dbeafe;color:#1d4ed8;font-weight:800}
.statisticsPage{min-width:0;overflow-x:hidden;padding-bottom:32px}
.statsHeader{display:flex;align-items:flex-start;justify-content:space-between;gap:18px;margin-bottom:16px}
.statsHeader h1{margin:0 0 5px}
.statsHeader p{margin:0 0 7px}
.statsHeader small{display:block;color:#64748b}
.statsNotice{margin:0 0 12px;padding:11px 14px;border:1px solid #fbbf24;border-radius:12px;background:#fffbeb;color:#92400e;font-size:13px;font-weight:700}
.statsNoticeNeutral{border-color:#bfdbfe;background:#eff6ff;color:#1e40af}
.statsFilters{display:grid;grid-template-columns:repeat(4,minmax(150px,1fr));gap:12px;padding:16px;margin-bottom:16px;align-items:end}
.statsFilters label,.statsCustomDates label{display:grid;gap:6px;min-width:0;color:#475569;font-size:12px;font-weight:800}
.statsFilters input,.statsFilters select{width:100%;min-width:0;height:42px;margin:0;box-sizing:border-box}
.statsSearch{grid-column:span 2}
.statsCustomDates{grid-column:span 2;display:grid;grid-template-columns:repeat(2,minmax(130px,1fr));gap:10px;min-width:0}
.statsCustomDates.isHidden{display:none}
.statsFilterActions{grid-column:span 2;display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:8px;align-items:center}
.statsFilterActions>*{width:100%;min-width:0;min-height:42px;justify-content:center;text-align:center;box-sizing:border-box}
.statsCards{display:grid;grid-template-columns:repeat(7,minmax(120px,1fr));gap:10px;margin-bottom:16px}
.statsMetric{display:flex;align-items:center;gap:10px;min-width:0;min-height:86px;padding:14px;box-sizing:border-box}
.statsMetric>span{display:grid;place-items:center;flex:0 0 38px;width:38px;height:38px;border-radius:12px;background:#eff6ff;color:#2563eb;font-size:19px;font-weight:900}
.statsMetric>div{display:grid;gap:3px;min-width:0}
.statsMetric small{color:#64748b;font-size:11px;font-weight:800;line-height:1.2}
.statsMetric strong{font-size:24px;line-height:1;color:#0f172a}
.statsOverviewGrid{display:grid;grid-template-columns:minmax(0,1.25fr) minmax(300px,.75fr);gap:16px;margin-bottom:16px}
.statsPanel{min-width:0;padding:18px;box-sizing:border-box}
.statsPanelHead,.statsTableHead{display:flex;align-items:flex-start;justify-content:space-between;gap:12px;margin-bottom:14px}
.statsPanelHead h2,.statsTableHead h2{margin:0 0 3px;font-size:19px}
.statsPanelHead p,.statsTableHead p{margin:0;font-size:12px}
.statsPanelHead>a{font-size:12px;font-weight:800;white-space:nowrap}
.statsRankNotice{margin:0 0 12px;padding:9px 11px;border-radius:10px;background:#fff7ed;color:#9a3412;font-size:12px;font-weight:700}
.statsTopRow{display:grid;grid-template-columns:30px minmax(0,1fr);gap:9px;align-items:center;padding:8px 0;border-top:1px solid var(--line)}
.statsTopRow:first-of-type{border-top:0}
.statsRank{display:grid;place-items:center;width:28px;height:28px;border-radius:9px;background:#eff6ff;color:#2563eb;font-size:12px;font-weight:900}
.statsTopInfo{display:grid;gap:6px;min-width:0}
.statsTopInfo>div:first-child{display:flex;align-items:baseline;justify-content:space-between;gap:12px;min-width:0}
.statsTopInfo b{min-width:0;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;font-size:13px}
.statsTopInfo span{flex:none;color:#64748b;font-size:11px;white-space:nowrap}
.statsTopInfo>small{color:#64748b;font-size:10px;line-height:1.25}
.statsBar{height:6px;border-radius:999px;background:#e2e8f0;overflow:hidden}
.statsBar i{display:block;height:100%;border-radius:inherit;background:linear-gradient(90deg,#2563eb,#60a5fa)}
.statsEmptyList{display:grid;gap:7px}
.statsEmptyProduct{display:flex;align-items:center;justify-content:space-between;gap:12px;padding:9px 11px;border:1px solid var(--line);border-radius:10px;background:#f8fafc;min-width:0}
.statsEmptyProduct b{min-width:0;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;font-size:12px}
.statsEmptyProduct span{flex:none;color:#64748b;font-size:11px;white-space:nowrap}
.statsTableCard{min-width:0;padding:18px;overflow:hidden}
.statsTableWrap{width:100%;max-width:100%;overflow-x:auto;border:1px solid var(--line);border-radius:12px;-webkit-overflow-scrolling:touch}
.statsTable{width:100%;min-width:1180px;border-collapse:collapse;font-size:12px;background:#fff}
.statsTable th,.statsTable td{padding:10px 9px;border-bottom:1px solid var(--line);vertical-align:middle;text-align:left}
.statsTable th{position:sticky;top:0;z-index:1;background:#f8fafc;color:#475569;font-size:11px;white-space:nowrap}
.statsTable th a{color:inherit;text-decoration:none}
.statsTable tbody tr:last-child td{border-bottom:0}
.statsTable tbody tr:hover{background:#f8fbff}
.statsNum{color:#94a3b8;text-align:center!important}
.statsNumber{text-align:right!important;white-space:nowrap;font-variant-numeric:tabular-nums}
.statsProductCell{display:flex;align-items:center;gap:9px;min-width:230px;max-width:330px}
.statsProductCell img,.statsNoPhoto{flex:0 0 38px;width:38px;height:38px;border:1px solid var(--line);border-radius:9px;background:#fff;object-fit:contain}
.statsNoPhoto{display:grid;place-items:center;font-size:17px}
.statsProductCell>div{display:grid;gap:3px;min-width:0}
.statsProductCell b{overflow:hidden;text-overflow:ellipsis;white-space:nowrap}
.statsProductCell small{color:#64748b}
.statsStatus{display:inline-flex;padding:5px 7px;border-radius:999px;background:#f1f5f9;color:#475569;font-size:10px;font-weight:900;white-space:nowrap}
.statsStatus-growth,.statsStatus-new{background:#dcfce7;color:#166534}
.statsStatus-decline,.statsStatus-no_period{background:#fee2e2;color:#991b1b}
.statsStatus-stable,.statsStatus-ordered{background:#dbeafe;color:#1e40af}
.statsActivity{display:block;margin-top:4px;color:#64748b;font-size:10px;white-space:nowrap}
.statsUp{color:#15803d;font-weight:900}.statsDown{color:#b91c1c;font-weight:900}
.statsEmpty{padding:28px!important;text-align:center!important;color:#64748b}
.statsPagination{display:flex;align-items:center;justify-content:center;gap:6px;margin-top:16px;flex-wrap:wrap}
.statsPagination a{display:grid;place-items:center;min-width:36px;height:36px;padding:0 8px;border:1px solid var(--line);border-radius:10px;background:#fff;color:#334155;text-decoration:none;font-weight:800;box-sizing:border-box}
.statsPagination a.active{border-color:#2563eb;background:#2563eb;color:#fff}
.statsPageGap{padding:0 3px;color:#94a3b8}
@media(max-width:1250px){
  .statsCards{grid-template-columns:repeat(4,minmax(130px,1fr))}
  .statsFilters{grid-template-columns:repeat(3,minmax(150px,1fr))}
}
@media(max-width:900px){
  .statsOverviewGrid{grid-template-columns:1fr}
  .statsCards{grid-template-columns:repeat(2,minmax(0,1fr))}
  .statsFilters{grid-template-columns:repeat(2,minmax(0,1fr))}
  .statsSearch,.statsCustomDates,.statsFilterActions{grid-column:span 2}
}
@media(max-width:600px){
  .statisticsPage{padding-left:0;padding-right:0}
  .statsHeader{display:grid;grid-template-columns:1fr;gap:12px}
  .statsHeader .btn{width:100%;justify-content:center;box-sizing:border-box}
  .statsFilters{grid-template-columns:1fr;padding:12px;gap:10px}
  .statsSearch,.statsCustomDates,.statsFilterActions{grid-column:span 1}
  .statsCustomDates,.statsFilterActions{grid-template-columns:1fr}
  .statsCards{grid-template-columns:1fr}
  .statsMetric{min-height:72px}
  .statsOverviewGrid{gap:12px}
  .statsPanel,.statsTableCard{padding:12px}
  .statsTopInfo>div:first-child{display:grid;gap:2px}
  .statsTopInfo span{white-space:normal}
  .statsEmptyProduct{align-items:flex-start;display:grid;gap:3px}
  .statsEmptyProduct span{white-space:normal}
}
...
const CART_BATCH_DELAY_MS=250,CART_FORCE_FLUSH_MS=1000;
const __cartStates=new Map();
let __cartServerCount=null,__cartOptimisticCount=null,__cartSendChain=Promise.resolve(),__cartStatusTimer=null;
function cartSaveStatus(text,error){const el=document.getElementById('cartSaveStatus');if(!el)return;clearTimeout(__cartStatusTimer);el.textContent=text||'';el.classList.toggle('error',!!error);el.classList.toggle('visible',!!text);if(text==='Збережено')__cartStatusTimer=setTimeout(()=>el.classList.remove('visible'),1600)}
function cartUuid(){if(window.crypto&&typeof window.crypto.randomUUID==='function')return window.crypto.randomUUID();return Date.now().toString(36)+'-'+Math.random().toString(36).slice(2)+'-'+Math.random().toString(36).slice(2)}
function cartInitialCount(){if(__cartOptimisticCount!==null)return __cartOptimisticCount;const el=document.querySelector('[data-cart-count],[data-cart-summary-total]');const n=Number(el&&el.textContent||0);__cartServerCount=Number.isFinite(n)?n:0;__cartOptimisticCount=__cartServerCount;return __cartOptimisticCount}
function cartSetCount(value){const n=Math.max(0,Number(value)||0);__cartOptimisticCount=n;document.querySelectorAll('[data-cart-count],[data-cart-summary-total]').forEach(el=>el.textContent=String(n))}
function cartQuantityElements(id){return document.querySelectorAll('[data-quantity-id="'+CSS.escape(String(id))+'"]')}
function cartQuantityFromDom(id){const el=document.querySelector('[data-quantity-id="'+CSS.escape(String(id))+'"]');const n=Number(el&&el.dataset.quantity);return Number.isFinite(n)&&n>=0?n:0}
function cartState(id){id=String(id||'');let state=__cartStates.get(id);if(!state){const qty=cartQuantityFromDom(id);state={id:id,confirmedQty:qty,optimisticQty:qty,pendingDelta:0,inFlight:null,timer:null,firstPendingAt:0};__cartStates.set(id,state)}return state}
function cartNumber(value){const n=Math.round((Number(value)||0)*1000)/1000;return Number.isInteger(n)?String(n):String(n).replace(/0+$/,'').replace(/[.]$/,'')}
function cartQuantityText(el,qty){const factor=Number(el.dataset.quantityFactor||1),unit=String(el.dataset.quantityUnit||'szt');let total=(Number.isFinite(factor)&&factor>0?factor:1)*Math.max(0,Number(qty)||0);if(['szt','keg','g','ml'].includes(unit))total=Math.round(total);else total=Math.round(total*1000)/1000;return cartNumber(total)+unit}
function renderCartQuantity(id,qty){cartQuantityElements(id).forEach(el=>{el.dataset.quantity=String(qty);const target=el.querySelector('.shop-work-hours-blue')||el;target.textContent=cartQuantityText(el,qty)})}
function cartOutstandingDelta(){let total=0;__cartStates.forEach(state=>{if(state.inFlight)total+=Number(state.inFlight.delta||0);total+=Number(state.pendingDelta||0)});return total}
function cartRecomputeCount(serverCount){if(Number.isFinite(Number(serverCount)))__cartServerCount=Number(serverCount);if(__cartServerCount===null)cartInitialCount();cartSetCount(Math.max(0,Number(__cartServerCount||0)+cartOutstandingDelta()))}
function cartEffectiveQty(id,fallback){const state=__cartStates.get(String(id||''));return state?state.optimisticQty:Number(fallback||0)}
function syncOptimisticCartDom(){__cartStates.forEach(state=>renderCartQuantity(state.id,state.optimisticQty));if(__cartOptimisticCount!==null)cartSetCount(__cartOptimisticCount)}
function updateCartUI(data){
  if(!data)return;
  if(Number.isFinite(Number(data.count))){__cartServerCount=Number(data.count);cartRecomputeCount(__cartServerCount)}
  if(data.positions!==undefined)document.querySelectorAll('[data-cart-summary-positions]').forEach(el=>el.textContent=data.positions||0);
  if(data.id){
    const id=String(data.id),shownQty=cartEffectiveQty(id,data.itemQty);
    document.querySelectorAll('[data-item-count="'+CSS.escape(id)+'"]').forEach(el=>el.textContent=shownQty||0);
    renderCartQuantity(id,shownQty);
    const state=__cartStates.get(id);document.querySelectorAll('[data-cart-row="'+CSS.escape(id)+'"]').forEach(row=>{if(shownQty<=0&&(!state||(!state.inFlight&&!state.pendingDelta)))row.remove()});
  }
  if(data.cleared){__cartStates.forEach(state=>{clearTimeout(state.timer)});__cartStates.clear();__cartServerCount=0;cartSetCount(0);document.querySelectorAll('[data-item-count]').forEach(el=>el.textContent='0');document.querySelectorAll('[data-catalog-total]').forEach(el=>{el.dataset.quantity='0';const target=el.querySelector('.shop-work-hours-blue')||el;target.textContent=el.dataset.zeroTotal||'0szt'});document.querySelectorAll('[data-cart-row]').forEach(row=>row.remove())}
  updateEmptyCart();
}
function cartHasPending(){for(const state of __cartStates.values()){if(state.inFlight||state.pendingDelta||state.timer)return true}return false}
function cartRetryWait(ms){return new Promise(resolve=>{let done=false;const finish=()=>{if(done)return;done=true;clearTimeout(timer);window.removeEventListener('online',finish);resolve()};const timer=setTimeout(finish,ms);window.addEventListener('online',finish,{once:true})})}
function scheduleCartState(state){
  if(!state.pendingDelta)return;
  if(!state.firstPendingAt)state.firstPendingAt=Date.now();
  clearTimeout(state.timer);
  const delay=Math.max(0,Math.min(CART_BATCH_DELAY_MS,CART_FORCE_FLUSH_MS-(Date.now()-state.firstPendingAt)));
  state.timer=setTimeout(()=>{state.timer=null;flushCartState(state,false)},delay);
}
async function sendCartOperation(state,operation){
  let retryIndex=0;
  for(;;){
    try{
      const payload=new URLSearchParams({id:state.id,expectedQty:String(operation.expectedQty),targetQty:String(operation.targetQty),operationId:operation.operationId});
      const response=await fetch('/cart/add',{method:'POST',body:payload,headers:{'X-Requested-With':'fetch'},keepalive:!!operation.keepalive});
      const data=await response.json().catch(()=>({ok:false}));
      if(response.status===409&&data&&data.conflict){
        const actual=Math.max(0,Number(data.actualQty)||0);
        operation.expectedQty=actual;operation.targetQty=Math.max(0,actual+operation.delta);state.confirmedQty=actual;state.optimisticQty=Math.max(0,operation.targetQty+state.pendingDelta);
        renderCartQuantity(state.id,state.optimisticQty);cartRecomputeCount(data.count);cartSaveStatus('Збереження…',false);continue;
      }
      if(!response.ok||!data||!data.ok){if(response.status>=400&&response.status<500)throw Object.assign(new Error(data.error||'Некоректний запит'),{permanent:true});throw new Error(data.error||'Помилка сервера')}
      state.confirmedQty=Math.max(0,Number(data.itemQty)||0);state.inFlight=null;state.optimisticQty=Math.max(0,state.confirmedQty+state.pendingDelta);renderCartQuantity(state.id,state.optimisticQty);updateCartUI(data);cartRecomputeCount(data.count);
      if(state.pendingDelta)scheduleCartState(state);else{state.firstPendingAt=0;if(!cartHasPending())cartSaveStatus('Збережено',false)}
      return true;
    }catch(error){
      console.error('[cart save]',error);
      if(error&&error.permanent){const oldOptimistic=state.optimisticQty;state.inFlight=null;state.optimisticQty=Math.max(0,state.confirmedQty+state.pendingDelta);renderCartQuantity(state.id,state.optimisticQty);cartSetCount(Math.max(0,cartInitialCount()+(state.optimisticQty-oldOptimistic)));cartSaveStatus('Не збережено',true);toast(error.message||'Не вдалося зберегти кошик');return false}
      cartSaveStatus('Не збережено — повторюємо…',true);const delays=[1000,2000,5000,10000];await cartRetryWait(delays[Math.min(retryIndex++,delays.length-1)]);
    }
  }
}
function flushCartState(state,keepalive){
  if(!state||state.inFlight||!state.pendingDelta)return;
  clearTimeout(state.timer);state.timer=null;
  const delta=state.pendingDelta;state.pendingDelta=0;state.firstPendingAt=0;
  const operation={operationId:cartUuid(),delta:delta,expectedQty:state.confirmedQty,targetQty:Math.max(0,state.confirmedQty+delta),keepalive:!!keepalive};
  state.inFlight=operation;
  const run=()=>sendCartOperation(state,operation);
  __cartSendChain=__cartSendChain.then(run,run);
}
function queueCartDelta(form,delta){
  const input=form&&form.querySelector('[name=id]'),id=input&&String(input.value||'');delta=Number(delta);
  if(!id||!Number.isFinite(delta)||!Number.isInteger(delta)||delta===0)return false;
  const state=cartState(id),oldQty=state.optimisticQty,nextQty=Math.max(0,oldQty+delta),applied=nextQty-oldQty;
  if(!applied)return false;
  state.optimisticQty=nextQty;state.pendingDelta+=applied;cartSetCount(cartInitialCount()+applied);renderCartQuantity(id,nextQty);cartSaveStatus('Збереження…',false);
  if(!state.pendingDelta&&!state.inFlight){clearTimeout(state.timer);state.timer=null;state.firstPendingAt=0;cartSaveStatus('Збережено',false)}else scheduleCartState(state);
  return false;
}
function flushAllCartStates(keepalive){__cartStates.forEach(state=>{if(state.pendingDelta)flushCartState(state,keepalive);if(state.inFlight&&keepalive)state.inFlight.keepalive=true})}
async function flushCartChanges(timeoutMs=15000){
  flushAllCartStates(false);const started=Date.now();
  while(cartHasPending()){if(Date.now()-started>timeoutMs)throw new Error('Не вдалося зберегти кошик. Перевірте інтернет.');await new Promise(resolve=>setTimeout(resolve,50));flushAllCartStates(false)}
  return true;
}
async function cartFetch(form,msg){try{const r=await fetch(form.action,{method:'POST',body:new URLSearchParams(new FormData(form)),headers:{'X-Requested-With':'fetch'}});const data=await r.json();if(!r.ok||!data||!data.ok)throw new Error(data&&data.error||'Помилка дії');if(data.id&&__cartStates.has(String(data.id)))__cartStates.delete(String(data.id));updateCartUI(data);if(msg)toast(msg);return true}catch(e){console.error(e);toast(e.message||'Помилка дії');return false}}
function addToCart(form){const deltaInput=form.querySelector('[name=delta]');return queueCartDelta(form,deltaInput?Number(deltaInput.value||1):1)}
function changeQty(form,delta){if(delta===undefined){const input=form.querySelector('[name=delta]');delta=input?Number(input.value||0):0}return queueCartDelta(form,delta)}
async function removeCart(form){const ok=await niceConfirm('Видалити позицію?','Цей товар буде видалено з кошика.','Так, видалити');if(ok){try{await flushCartChanges();await cartFetch(form,'Видалено')}catch(e){toast(e.message)}}return false}
async function clearCart(form){const ok=await niceConfirm('Очистити кошик?','Усі позиції будуть видалені з кошика.','Так, очистити');if(ok){try{await flushCartChanges();await cartFetch(form,'Кошик очищено')}catch(e){toast(e.message)}}return false}
document.addEventListener('click',async function(event){const link=event.target.closest&&event.target.closest('[data-cart-checkout]');if(!link||link.dataset.busy==='1')return;if(!cartHasPending())return;event.preventDefault();link.dataset.busy='1';link.setAttribute('aria-disabled','true');cartSaveStatus('Збереження…',false);try{await flushCartChanges();location.href=link.href}catch(error){toast(error.message||'Не вдалося зберегти кошик');cartSaveStatus('Не збережено — повторюємо…',true);delete link.dataset.busy;link.removeAttribute('aria-disabled')}},true);
function sendCartKeepaliveSnapshot(){
  __cartStates.forEach(state=>{
    if(state.inFlight){const op=state.inFlight;fetch('/cart/add',{method:'POST',body:new URLSearchParams({id:state.id,expectedQty:String(op.expectedQty),targetQty:String(op.targetQty),operationId:op.operationId}),headers:{'X-Requested-With':'fetch'},keepalive:true}).catch(()=>{})}
    if(state.pendingDelta){const expected=state.inFlight?state.inFlight.targetQty:state.confirmedQty,target=Math.max(0,expected+state.pendingDelta);fetch('/cart/add',{method:'POST',body:new URLSearchParams({id:state.id,expectedQty:String(expected),targetQty:String(target),operationId:cartUuid()}),headers:{'X-Requested-With':'fetch'},keepalive:true}).catch(()=>{})}
  });
}
document.addEventListener('visibilitychange',function(){if(document.hidden)sendCartKeepaliveSnapshot()});
window.addEventListener('pagehide',sendCartKeepaliveSnapshot);
function toggleDepositCheckbox(sel){const form=sel.closest('form');if(!form)return;const wrap=form.querySelector('[data-deposit-wrap]');const typeWrap=form.querySelector('[data-deposit-type-wrap]');const cb=form.querySelector('input[name=hasDeposit]');const type=form.querySelector('select[name=depositType]');const alcohol=sel.value==='Алкоголь',allowed=['Алкоголь','Напої'].includes(sel.value);if(wrap)wrap.style.display=alcohol?'none':(allowed?'':'none');if(typeWrap)typeWrap.style.display=alcohol?'':'none';if(cb&&(!allowed||alcohol))cb.checked=false;if(type&&!alcohol)type.value='';}
...
function startEditProduct(btn){
  ...
  tr.classList.add('isProductEditing');
  nameSpan.outerHTML='<input class="editInlineInput" name="editName" value="'+nameVal.replace(/&/g,'&amp;').replace(/"/g,'&quot;')+'">';
  weightSpan.outerHTML='<span class="editMeasureWrap"><input class="editInlineInput" name="editWeight" value="'+weightVal.replace(/&/g,'&amp;').replace(/"/g,'&quot;')+'"><select class="editInlineInput" name="editUnit">'+['szt','g','kg','L','ml'].map(function(u){return '<option value="'+u+'" '+(u===unitVal?'selected':'')+'>'+u+'</option>';}).join('')+'</select></span>';
  if(mobileWeightSpan){mobileWeightSpan.outerHTML='<span class="editMeasureWrapMobile"><input class="editInlineInput" name="editWeightMobile" value="'+weightVal.replace(/&/g,'&amp;').replace(/"/g,'&quot;')+'"><select class="editInlineInput" name="editUnitMobile">'+['szt','g','kg','L','ml'].map(function(u){return '<option value="'+u+'" '+(u===unitVal?'selected':'')+'>'+u+'</option>';}).join('')+'</select></span>';}
  btn.textContent='✅';
  btn.title='Зберегти';
...
async function loadLazyOrderDetails(details){
  if(!details||!details.open)return;
  const box=details.querySelector('[data-order-lazy]');
  if(!box||box.dataset.loaded==='1'||box.dataset.loading==='1')return;
  box.dataset.loading='1';
  box.innerHTML='<div class="muted center" style="padding:18px">Завантаження позицій…</div>';
  try{
    const r=await fetch(box.dataset.url,{headers:{'X-Requested-With':'fetch'},cache:'no-store'});
    const data=await r.json();
    if(!r.ok||!data||!data.ok)throw new Error('order details');
    box.innerHTML=data.html||'';
    box.dataset.loaded='1';
  }catch(e){
    console.error(e);
    box.innerHTML='<div class="center" style="padding:18px"><span class="muted">Не вдалося завантажити позиції.</span> <button type="button" class="secondary compactBtn" data-order-retry>Повторити</button></div>';
    const retry=box.querySelector('[data-order-retry]');
    if(retry)retry.addEventListener('click',function(){loadLazyOrderDetails(details)});
  }finally{delete box.dataset.loading;}
}
async function loadMoreOrderCards(button){
  if(!button||button.dataset.loading==='1')return;
  const target=document.getElementById(button.dataset.target||'');
  if(!target)return;
  button.dataset.loading='1';button.disabled=true;
  const oldText=button.textContent;button.textContent='Завантаження…';
  try{
    const params=new URLSearchParams({offset:String(Number(button.dataset.offset)||0)});
    const r=await fetch(button.dataset.url+'?'+params.toString(),{headers:{'X-Requested-With':'fetch'},cache:'no-store'});
    const data=await r.json();
    if(!r.ok||!data||!data.ok)throw new Error('orders');
    target.insertAdjacentHTML('beforeend',data.html||'');
    button.dataset.offset=String(data.nextOffset||0);
    const wrap=button.closest('[data-order-load-wrap]');
    const progress=wrap&&wrap.querySelector('[data-order-progress]');
    if(progress)progress.textContent='Показано '+Number(data.nextOffset||0)+' із '+Number(data.total||0);
    if(wrap&&!data.hasMore)wrap.style.display='none';
  }catch(e){console.error(e);toast('Не вдалося завантажити замовлення')}
  finally{delete button.dataset.loading;button.disabled=false;button.textContent=oldText;}
}
...
function initChatAutoRefresh(){
  ...
  window.__chatTimer=setInterval(function(){refreshChatMessages(false)},${MESSAGE_POLL_INTERVAL_MS});
...
async function toggleFavorite(event,button){
  if(event){event.preventDefault();event.stopPropagation();}
  if(!button||button.classList.contains('isBusy'))return false;
  const id=String(button.dataset.favoriteId||''); if(!id)return false;
  const next=!button.classList.contains('isFavorite');
  button.classList.add('isBusy');
  try{
    const r=await fetch('/catalog/favorite',{method:'POST',headers:{'Content-Type':'application/x-www-form-urlencoded;charset=UTF-8','X-Requested-With':'fetch'},body:new URLSearchParams({id:id,active:next?'1':'0'}).toString()});
    const j=await r.json(); if(!r.ok||!j||!j.ok)throw new Error('favorite');
    const active=!!j.active;
    document.querySelectorAll('[data-favorite-id="'+CSS.escape(id)+'"]').forEach(function(el){el.classList.toggle('isFavorite',active);const sp=el.querySelector('span');if(sp)sp.textContent=active?'★':'☆';el.title=active?'Видалити з обраного':'Додати в обране';el.setAttribute('aria-label',el.title);});
    document.querySelectorAll('[data-favorite-count]').forEach(function(el){el.textContent=String(j.count||0)});
    if(location.search.indexOf('favorites=1')>=0&&!active){
      document.querySelectorAll('.prodCard[data-product-id="'+CSS.escape(id)+'"]').forEach(function(el){el.remove()});
      if(typeof window.refreshCatalogProducts==='function')window.refreshCatalogProducts();
      else{const hasCards=document.querySelector('#prodGrid .prodCard');if(!hasCards)location.reload();}
    }
  }catch(e){showToast('Не вдалося змінити обране');}
  finally{button.classList.remove('isBusy');}
  return false;
}

document.addEventListener('DOMContentLoaded',function(){restoreScrollState();try{const p=location.pathname;const nav=document.querySelector('.links');if(nav){let sel='';if(p==='/cabinet'||p.startsWith('/cabinet/'))sel='.mobileCabinet';else if(p==='/messages'||p.startsWith('/messages/'))sel='.mobileMessages';else if(p==='/cart'||p.startsWith('/cart/'))sel='.mobileCart';else if(p==='/about'||p.startsWith('/about/'))sel='.mobileAnnouncements';if(sel){const a=nav.querySelector(sel);if(a)a.classList.add('shopNavActive')}}}catch(e){}document.querySelectorAll('.accountingForm').forEach(calcAccountingForm);initChatAutoRefresh();initFloatingCartVisibility();document.querySelectorAll('select[name=category]').forEach(toggleDepositCheckbox);try{if(location.pathname==='/admin-products'||location.pathname==='/admin-new-products'){const y=sessionStorage.getItem('adminProductsScroll');if(y!==null){sessionStorage.removeItem('adminProductsScroll');setTimeout(function(){window.scrollTo(0,Number(y)||0)},0);}}}catch(e){}});
document.addEventListener('submit',function(e){const f=e.target;if(f && f.matches('form.directDeleteForm')){e.preventDefault();e.stopPropagation();deleteDirectMessage(f);return false}if(f && f.matches('form.chatForm')){e.preventDefault();e.stopPropagation();sendChatMessage(f);return false}if(f && f.matches('form[data-ajax-cart]')){e.preventDefault();e.stopPropagation();const action=f.dataset.action;if(action==='add')addToCart(f);else if(action==='qty')changeQty(f);else if(action==='remove')removeCart(f);else if(action==='clear')clearCart(f);return false}if(f && f.matches('form[data-ajax-admin-order]')){e.preventDefault();e.stopPropagation();if(f.classList.contains('orderSearchAddBox')&&!prepareOrderProductAdd(f))return false;adminOrderFetch(f);return false}if(f && f.matches('form.orderSearchAddBox')){e.preventDefault();e.stopPropagation();addOrderDraftProduct(f);return false}if(f && f.matches('form[data-preserve-admin-scroll]')){saveAdminScroll();return}if(f && f.method && String(f.method).toLowerCase()==='post'){saveScrollState();}},true);
</script></head><body>
<header class="top"><nav class="nav">
  ...
  <div class="links">
    ...
    ${session.shop?`<a class="mobileCabinet" href="/cabinet">Кабінет магазину</a><a class="mobileMessages" data-unread-key="directMessages" href="/messages">Повідомлення${badgeCount(unread.directMessages)}</a><a class="cart mobileCart" href="/cart" data-cart-checkout>🛒 Кошик (<span data-cart-count>${count}</span>)</a><a class="mobileHide" href="/catalog">Каталог</a><a class="mobileHide" href="/catalog?new=1">Новинки${badge(unread.newProducts)}</a><a class="mobileAnnouncements" href="/about">Оголошення${badge(unread.announcements)}</a><a class="mobileLogout" href="/shop-logout">Вийти</a>`:''}
...
${session.shop?'<div id="cartSaveStatus" class="cartSaveStatus" role="status" aria-live="polite"></div>':''}
<div id="toast" class="toast"></div>
</body></html>`;
}


function shopFavoriteIds(db, shop){
  db.favorites=db.favorites&&typeof db.favorites==='object'?db.favorites:{};
  const raw=Array.isArray(db.favorites[String(shop||'')])?db.favorites[String(shop||'')]:[];
  return new Set(raw.map(String));
}
function isShopFavorite(db, shop, productId){ return shopFavoriteIds(db,shop).has(String(productId)); }
function setShopFavorite(db, shop, productId, active){
  db.favorites=db.favorites&&typeof db.favorites==='object'?db.favorites:{};
  const key=String(shop||''), ids=shopFavoriteIds(db,key);
  if(active) ids.add(String(productId)); else ids.delete(String(productId));
  db.favorites[key]=Array.from(ids);
}

function normalizeCatalogSearchText(value){
  return String(value||'').toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g,'').replace(/ł/g,'l').replace(/đ/g,'d').replace(/ß/g,'ss').replace(/æ/g,'ae').replace(/œ/g,'oe').replace(/[^a-z0-9а-яіїєґ]+/gi,' ').trim();
}
function catalogSelection(db, session, url){
  ...
  const favoriteIds=shopFavoriteIds(db,session&&session.shop);
  const products=(db.products||[]).filter(p=>{
    if(p.hidden || (cat&&p.category!==cat) || (onlyNew&&!p.isNew) || (onlyFavorites&&!favoriteIds.has(String(p.id)))) return false;
    if(!tokens.length) return true;
    const hay=normalizeCatalogSearchText(`${p.name||''} ${p.category||''} ${productMetaText(p)} ${p.weight||''}`);
    return tokens.every(token=>hay.includes(token));
  });
  return {cat,onlyNew,onlyFavorites,query,favoriteIds,products};
}
function catalogChunkHtml(products, session, db){
  return (products||[]).map(p=>productCard(p,session,db)).join('');
}
function catalogPageInfo(products,url){
  const total=Array.isArray(products)?products.length:0;
  const totalPages=Math.ceil(total/CATALOG_PAGE_SIZE);
  const rawPage=String(url.searchParams.get('page')||'');
  const rawOffset=String(url.searchParams.get('offset')||'');
  let requestedPage=/^\d+$/.test(rawPage)&&Number(rawPage)>=1?Number(rawPage):1;
  if(!rawPage&&/^\d+$/.test(rawOffset))requestedPage=Math.floor(Number(rawOffset)/CATALOG_PAGE_SIZE)+1;
  const page=totalPages?Math.min(requestedPage,totalPages):1;
  const offset=(page-1)*CATALOG_PAGE_SIZE;
  const items=(products||[]).slice(offset,offset+CATALOG_PAGE_SIZE);
  return {page,pageSize:CATALOG_PAGE_SIZE,total,totalPages,offset,items,shown:items.length,hasPrevious:page>1,hasNext:page<totalPages};
}
function catalogPageHref(url,page){
  const params=new URLSearchParams(url.searchParams);
  params.delete('offset');params.set('page',String(page));
  return '/catalog?'+params.toString();
}
function catalogPaginationHtml(url,info){
  if(!info||!info.total)return '';
  let nav='';
  if(info.totalPages>1){
    const pages=Array.from(new Set([1,info.page-1,info.page,info.page+1,info.totalPages].filter(n=>n>=1&&n<=info.totalPages))).sort((a,b)=>a-b);
    const parts=[];let previous=0;
    for(const number of pages){
      if(previous&&number-previous>1)parts.push('<span class="catalogPageEllipsis" aria-hidden="true">…</span>');
      parts.push(number===info.page?`<span class="catalogPageButton isActive" aria-current="page">${number}</span>`:`<a class="catalogPageButton" href="${esc(catalogPageHref(url,number))}" data-catalog-page="${number}">${number}</a>`);
      previous=number;
    }
    const previousButton=info.hasPrevious?`<a class="catalogPageButton catalogPageDirection" href="${esc(catalogPageHref(url,info.page-1))}" data-catalog-page="${info.page-1}">‹ Назад</a>`:'<span class="catalogPageButton catalogPageDirection isDisabled" aria-disabled="true">‹ Назад</span>';
    const nextButton=info.hasNext?`<a class="catalogPageButton catalogPageDirection" href="${esc(catalogPageHref(url,info.page+1))}" data-catalog-page="${info.page+1}">Далі ›</a>`:'<span class="catalogPageButton catalogPageDirection isDisabled" aria-disabled="true">Далі ›</span>';
    nav=`<nav class="catalogPaginationNav" aria-label="Сторінки каталогу">${previousButton}${parts.join('')}${nextButton}</nav>`;
  }
  return `<div class="catalogPagination">${nav}<div class="catalogPaginationProgress">Показано ${info.shown} із ${info.total} · Сторінка ${info.page} з ${Math.max(1,info.totalPages)}</div></div>`;
}

function productCard(p, session, db){ p=enrichProduct(p); const isMissing=!!activeMissingProductAlert(db,p.id); const favorite=isShopFavorite(db,session&&session.shop,p.id); const cartItem=(session.cart||[]).find(x=>String(x.id)===String(p.id)); const qty=cartItem?cartItem.qty:0; const meta=productMetaText(p); const image=safeProductImagePath(p.image); const catalogTotal=productCatalogTotalText(p,qty); const zeroTotal=productCatalogTotalText(p,0); const title=productDisplayName(p); const factor=productFormatValue(p),unit=inferResultUnit(p); return `<div class="prodCard ${image?'hasProductImage':''} ${isMissing?'is-temporarily-missing':''}" data-product-id="${esc(p.id)}" data-product="${esc((p.name+' '+p.category+' '+meta+' '+p.weight).toLowerCase())}">${image?`<button type="button" class="product-image-zoom" aria-label="Збільшити фото" title="Збільшити фото" data-image="${esc(image)}" data-title="${esc(title)}" onclick="openProductImage(event,this)">🔍</button>`:''}<div class="prodCardTop"><span class="prodCardBadge" title="${esc(p.category)}" aria-label="${esc(p.category)}">${CAT_SVG_ICONS[p.category]||CAT_ICONS[p.category]||'▣'}<span class="prodCardBadgeText">${esc(p.category)}</span></span>${p.isNew?'<span class="prodCardNew">NEW</span>':''}</div><div class="product-image-wrapper ${image?'product-image-background':''}">${image?`<img src="${esc(image)}" alt="${esc(title)}" loading="lazy" decoding="async"><div class="product-image-shade"></div>`:'<div class="product-image-placeholder">Фото відсутнє</div>'}<button type="button" class="favoriteStar ${favorite?'isFavorite':''}" data-favorite-id="${esc(p.id)}" aria-label="${favorite?'Видалити з обраного':'Додати в обране'}" title="${favorite?'Видалити з обраного':'Додати в обране'}" onclick="toggleFavorite(event,this)"><span aria-hidden="true">${favorite?'★':'☆'}</span></button></div><div class="prodCardName ${image?'prodCardNameOnImage':''}"><span>${esc(title)}</span></div><div class="prodCardQuickQty"><form method="post" action="/cart/add" data-ajax-cart data-action="add" onsubmit="event.preventDefault();return addToCart(this)"><input type="hidden" name="id" value="${p.id}"><input type="hidden" name="delta" value="5"><button class="quickQtyBtn" aria-label="Додати 5">+5</button></form><form method="post" action="/cart/add" data-ajax-cart data-action="add" onsubmit="event.preventDefault();return addToCart(this)"><input type="hidden" name="id" value="${p.id}"><input type="hidden" name="delta" value="10"><button class="quickQtyBtn" aria-label="Додати 10">+10</button></form></div><div class="prodCardQty"><form method="post" action="/cart/qty" data-ajax-cart data-action="qty" onsubmit="event.preventDefault();return changeQty(this)"><input type="hidden" name="id" value="${p.id}"><input type="hidden" name="delta" value="-1"><button class="secondary iconBtn minusBtn" aria-label="Мінус">−</button></form><div class="prodCardQtyNum catalogTotalValue" data-catalog-total="${p.id}" data-zero-total="${esc(zeroTotal)}" data-quantity-id="${esc(p.id)}" data-quantity="${esc(qty)}" data-quantity-factor="${esc(factor)}" data-quantity-unit="${esc(unit)}"><span class="shop-work-hours-blue">${esc(catalogTotal)}</span></div><form method="post" action="/cart/add" data-ajax-cart data-action="add" onsubmit="event.preventDefault();return addToCart(this)"><input type="hidden" name="id" value="${p.id}"><button class="iconBtn" aria-label="Додати">+</button></form></div></div>`; }
function adminProductRow(p, n){
  p=enrichProduct(p);
  const meta=productMetaText(p);
  const depositCell=String(p.category||'')==='Алкоголь'
    ? `<form method="post" action="/admin/product-deposit-type" data-preserve-admin-scroll><input type="hidden" name="id" value="${p.id}"><select name="depositType" onchange="saveAdminScroll(); this.form.submit()" style="min-width:112px;padding:7px 8px;border:1px solid #cbd5e1;border-radius:8px"><option value="" ${!productDepositType(p)?'selected':''}>Без кауції</option><option value="puszka" ${productDepositType(p)==='puszka'?'selected':''}>puszka</option><option value="PET" ${productDepositType(p)==='PET'?'selected':''}>PET</option></select></form>`
    : canHaveDeposit(p.category)
...
function adminProductSelection(db,url){
  const cat=String(url.searchParams.get('cat')||'');
  const query=String(url.searchParams.get('q')||'').trim().slice(0,120);
  const tokens=normalizeCatalogSearchText(query).split(/\s+/).filter(Boolean);
  const products=(db.products||[]).filter(p=>{
    if(p.hidden||(cat&&p.category!==cat))return false;
    if(!tokens.length)return true;
    const hay=normalizeCatalogSearchText(`${p.name||''} ${p.category||''} ${p.weight||''} ${p.resultUnit||p.packUnit||''}`);
    return tokens.every(token=>hay.includes(token));
  });
  return {cat,query,products};
}
function adminProductRowsHtml(products,start=0){
  return (products||[]).map((p,index)=>adminProductRow(p,start+index+1)).join('');
}
function adminProductsPage(db,url){
  const {cat,query,products}=adminProductSelection(db,url),first=products.slice(0,ADMIN_PRODUCT_PAGE_SIZE),hasMore=first.length<products.length;
  return `<div class="adminShell">${adminMenu()}<section class="adminProductsSection"><div class="actions" style="align-items:center;justify-content:space-between;margin-bottom:12px"><h1 style="margin:0">Товари</h1></div><div class="card adminProductAddCard" style="padding:16px;margin-bottom:16px"><form class="form adminProductAddForm" method="post" action="/admin/product-add"><label>Назва<input name="name" required placeholder="Напр. Пельмені Пузата Хата 900г 15 szt"></label><label>Кількість/вага<input name="weight" required type="number" min="0" step="0.001" placeholder="15"></label><label>Одиниця виміру<select name="resultUnit">${unitOptionsHtml('szt')}</select></label><label>Категорія<select name="category" onchange="toggleDepositCheckbox(this)">${CATEGORIES.map(c=>`<option value="${esc(c)}" ${c===cat?'selected':''}>${esc(c)}</option>`).join('')}</select></label><div class="adminProductAddActions"><label class="adminProductNewCheck" data-deposit-wrap style="${(cat||'Алкоголь')==='Алкоголь'?'display:none':(['Алкоголь','Напої'].includes(cat)?'':'display:none')}"><input type="checkbox" name="hasDeposit" value="1"> Кауція</label><label data-deposit-type-wrap style="${(cat||'Алкоголь')==='Алкоголь'?'':'display:none'}">Тип кауції<select name="depositType"><option value="">Без кауції</option><option value="puszka">puszka</option><option value="PET">PET</option></select></label><label class="adminProductNewCheck"><input type="checkbox" name="isNew" value="1"> Новинка</label><button>Додати</button></div></form></div><div class="adminProductCats">${CATEGORIES.map(c=>`<span class="adminProductCatExport"><a class="btn ${cat===c?'':'secondary'}" href="/admin-products?cat=${encodeURIComponent(c)}">${CAT_ICONS[c]||''} ${esc(c)}</a>${categoryDownloadIcon(c)}</span>`).join('')}<span class="adminProductCatExport adminProductCatAll"><a class="btn ${!cat?'':'secondary'}" href="/admin-products">Усі</a><span class="categoryDownloadIcon categoryDownloadPlaceholder" aria-hidden="true">⬇️</span></span></div><div class="card adminSearchCard"><div class="adminSearchWrap"><span class="adminSearchIcon">🔎</span><input id="search" value="${esc(query)}" oninput="adminProductSearchChanged()" placeholder="Пошук товарів..." autocomplete="off"></div><div id="searchEmpty" class="adminSearchEmpty" style="${products.length?'display:none':''}">Нічого не знайдено</div></div><div class="listWrap adminProductsTableWrap"><table class="listTable adminProductsTable"><thead><tr><th>№</th><th>Назва</th><th class="weightHead">Кількість/вага</th><th>Кауція</th><th>Дія</th><th>Новинка</th><th>✏️</th><th>×</th></tr></thead><tbody id="adminProductsRows">${adminProductRowsHtml(first,0)}</tbody></table></div><div id="adminProductsLoadWrap" class="center" style="padding:16px 0 8px;${hasMore?'':'display:none'}"><button type="button" class="btn secondary" id="adminProductsLoadMore" onclick="loadMoreAdminProducts()">Показати ще 50 товарів</button><div class="muted" id="adminProductsProgress" style="font-size:12px;margin-top:8px">Показано ${first.length} із ${products.length}</div></div><script>(function(){let offset=${first.length},total=${products.length},loading=false,timer=null,controller=null;const rows=document.getElementById('adminProductsRows'),button=document.getElementById('adminProductsLoadMore'),wrap=document.getElementById('adminProductsLoadWrap'),progress=document.getElementById('adminProductsProgress'),empty=document.getElementById('searchEmpty');function params(nextOffset){const p=new URLSearchParams(location.search),q=String((document.getElementById('search')||{}).value||'').trim();if(q)p.set('q',q);else p.delete('q');p.set('offset',String(nextOffset));return p}function update(hasMore){if(progress)progress.textContent='Показано '+offset+' із '+total;if(wrap)wrap.style.display=hasMore?'':'none';if(empty)empty.style.display=total?'none':''}async function load(reset){if(loading&&!reset)return;if(controller)controller.abort();controller=new AbortController();const current=controller;loading=true;if(button){button.disabled=true;button.textContent='Завантаження…'}try{const r=await fetch('/admin-products/chunk?'+params(reset?0:offset).toString(),{headers:{'X-Requested-With':'fetch'},cache:'no-store',signal:current.signal}),j=await r.json();if(!r.ok||!j||!j.ok)throw new Error('products');if(reset)rows.innerHTML=j.html||'';else rows.insertAdjacentHTML('beforeend',j.html||'');offset=Number(j.nextOffset||0);total=Number(j.total||0);update(!!j.hasMore)}catch(e){if(e&&e.name!=='AbortError')toast('Не вдалося завантажити товари')}finally{if(controller===current){loading=false;if(button){button.disabled=false;button.textContent='Показати ще 50 товарів'}}}}window.loadMoreAdminProducts=function(){load(false)};window.adminProductSearchChanged=function(){clearTimeout(timer);timer=setTimeout(function(){const pageUrl=new URL(location.href),q=String((document.getElementById('search')||{}).value||'').trim();if(q)pageUrl.searchParams.set('q',q);else pageUrl.searchParams.delete('q');pageUrl.searchParams.delete('offset');history.replaceState(null,'',pageUrl.pathname+(pageUrl.searchParams.toString()?'?'+pageUrl.searchParams.toString():''));load(true)},450)};window.addEventListener('pagehide',function(){clearTimeout(timer);if(controller)controller.abort()},{once:true})})();</script></section></div>`;
}
...
function adminMissingProductsPage(db){
  ...
  const batch=rows.length?`<form id="missingBatchForm" class="missingBatchBar" method="post" action="/admin-missing-products/in-stock-batch" onsubmit="const n=document.querySelectorAll('.missingProductCheck:checked').length;if(!n){alert('Спочатку поставте галочки біля товарів.');return false}return confirm('Повернути в наявність вибрані товари: '+n+'? Червоне підсвічування для них буде одразу знято.')"><button type="submit">✓ В наявності</button><span class="missingBatchCount" id="missingBatchCount">Вибрано: 0</span></form>`:'';
  const content=rows.length?`${batch}<div class="missingProductsList">${rows.map(r=>`<div class="missingProductRow" data-missing-product-id="${esc(r.key)}"><div class="missingProductSelect"><input class="missingProductCheck" type="checkbox" form="missingBatchForm" name="productIds" value="${esc(r.key)}" aria-label="Вибрати ${esc(r.productName)}" onchange="updateMissingBatchCount()"></div><div class="missingProductInfo"><span class="missingProductIcon">!</span><div class="missingProductText"><b class="missingProductName">${esc(r.productName)}</b><div class="muted missingProductMeta">Остання позначка: магазин <b>${esc(r.shop||'—')}</b>${r.orderNo?` · замовлення №${esc(r.orderNo)}`:''}</div></div></div><div class="missingProductTimer"><div class="missingProductCountdown" data-missing-countdown="${esc(r.expiresAtMs)}">—</div><div class="muted missingProductTimerLabel">залишилось</div></div><form class="missingProductAction" method="post" action="/admin-missing-products/in-stock" onsubmit="return confirm('Позначити цей товар як наявний? Червоне підсвічування в магазинах буде одразу знято.')"><input type="hidden" name="productId" value="${esc(r.key)}"><button type="submit">✓ В наявності</button></form></div>`).join('')}</div>`:'<div class="center muted" style="padding:30px">Активних відсутніх товарів поки немає</div>';
  const script=`<script>function updateMissingBatchCount(){const el=document.getElementById('missingBatchCount');if(el)el.textContent='Вибрано: '+document.querySelectorAll('.missingProductCheck:checked').length}(function(){function render(){const now=Date.now();document.querySelectorAll('[data-missing-countdown]').forEach(el=>{let ms=Number(el.dataset.missingCountdown||0)-now;if(ms<=0){el.textContent='Завершено';return}const d=Math.floor(ms/86400000);ms%=86400000;const h=Math.floor(ms/3600000);ms%=3600000;const m=Math.floor(ms/60000);const sec=Math.floor((ms%60000)/1000);el.textContent=(d?d+' д ':'')+String(h).padStart(2,'0')+':'+String(m).padStart(2,'0')+':'+String(sec).padStart(2,'0')})}async function syncRows(){try{const r=await fetch('/catalog/missing-alerts',{cache:'no-store',headers:{'X-Requested-With':'fetch'}});if(!r.ok)return;const j=await r.json();if(!j||!j.ok)return;const active=new Set((j.ids||[]).map(String));document.querySelectorAll('[data-missing-product-id]').forEach(row=>{if(!active.has(String(row.dataset.missingProductId||'')))row.remove()});const list=document.querySelector('.missingProductsList');if(list&&!list.querySelector('[data-missing-product-id]'))list.innerHTML='<div class="center muted" style="padding:30px">Активних відсутніх товарів поки немає</div>';updateMissingBatchCount()}catch(e){}}render();syncRows();updateMissingBatchCount();setInterval(render,1000);setInterval(syncRows,${MISSING_ALERT_POLL_INTERVAL_MS});document.addEventListener('visibilitychange',()=>{if(!document.hidden)syncRows()})})();</script>`;
...
function adminOrderDetailsHtml(o){
  const openUrl=`/admin-order-picking?id=${encodeURIComponent(o.id)}`;
  return `<div class="adminOrderQuickOpen"><a class="btn" href="${openUrl}">Відкрити замовлення</a></div><div class="adminOrderItemsTable"><div class="adminOrderItemsHead"><span>Назва товару</span><span>Замовлена кількість</span></div>${(o.items||[]).map(i=>`<div class="adminOrderItemRow"><div class="adminOrderItemName">${esc(productDisplayName(i))}</div><div class="adminOrderItemQty">${esc(productTotalDisplay(i,i.qty))}</div></div>`).join('')}</div><div class="orderComment"><div class="orderCommentLabel">Коментар магазину:</div>${esc(o.comment||'Без коментаря')}</div><div class="adminOrderActionsGrid adminOrderSummaryActions"><a class="btn secondary" target="_blank" rel="noopener" href="/admin-order-original-print?id=${encodeURIComponent(o.id)}&auto=1">🖨️ <span>Друк замовлення</span></a><a class="btn secondary" download href="/admin-order-original-pdf?id=${encodeURIComponent(o.id)}">⬇️ <span>PDF замовлення</span></a><button type="button" class="secondary" data-copy="${esc(orderCopyText(o))}" onclick="copyOrder(this)">📋 <span>Копіювати</span></button><a class="btn secondary" href="/admin-order-original-export?id=${encodeURIComponent(o.id)}">📊 <span>Excel</span></a></div>`;
}
function adminOrderCard(o, products){
  const itemCount=(o.items||[]).length;
  return `<details class="card order adminOrderGridCard adminOrderSummaryCard adminOrderCollapse" data-order-id="${esc(o.id)}" ontoggle="loadLazyOrderDetails(this)">
  ...
  <div class="adminOrderCollapseBody" data-order-lazy data-url="/admin-orders/details?id=${encodeURIComponent(o.id)}"><div class="muted center" style="padding:18px 0 2px">Відкрийте замовлення, щоб завантажити позиції</div>
  </div></details>`;
}
...
function addProductToOrderPicking(o, product, actualTotal, barcode=''){
  ...
  const item=o.items[idx],key='idx:'+idx,legacyKey=String(item.id||idx),prev=o.pickingItems[key]||o.pickingItems[legacyKey]||{};
  const nextTotal=Math.round((Number(prev.actualTotal||0)+total)*1000)/1000;
  o.pickingItems[key]={...prev,status:'present',actualTotal:nextTotal,productId:String(product.id),updatedAt:warsawTime(),updatedMs:nowMs()};
  o.status='Збирається';
}
...
function appResolvedDepositType(item,db){
  const direct=normalizeDepositType(item&&item.depositType);
  if(direct)return direct;
  const product=(db&&Array.isArray(db.products))?(db.products||[]).find(p=>String(p.id)===String(item&&item.productId||item&&item.id||'')):null;
  return productDepositType(product);
}
function appDepositCount(item,db){
  if(!appResolvedDepositType(item,db))return 0;
  // Kaucja must sum the ACTUAL item quantity shown in the printed row.
  // Example: 24 szt. + 24 szt. = 48 szt.  Do not count rows/packages.
  const unit=quantityUnitCode(item&&item.unitType);
  const total=Number(item&&item.totalUnits);
  if(unit==='szt' && Number.isFinite(total) && total>=0)return Math.max(0,total);
  // Deposit is a container count. For any legacy row that is not stored as szt.,
  // fall back to quantity rather than converting litres/kg/ml.
  const qty=Number(item&&item.quantity);
  if(Number.isFinite(qty)&&qty>=0)return Math.max(0,qty);
  return 0;
}
function appPrintGroups(app,db){
  const rows=[];(app.pallets||[]).forEach(p=>(p.items||[]).forEach(i=>rows.push(i)));
  const orderIndex=new Map(CATEGORIES.map((c,n)=>[c,n]));
  const alcohol=rows.filter(i=>String(i.category||'')==='Алкоголь');
  const supplies=rows.filter(i=>String(i.category||'')==='Забезпечення');
  const middle=rows.filter(i=>!['Алкоголь','Забезпечення'].includes(String(i.category||''))).sort((a,b)=>(orderIndex.get(String(a.category||''))??999)-(orderIndex.get(String(b.category||''))??999));
  const deposits={puszka:0,PET:0};
  for(const i of alcohol){const t=appResolvedDepositType(i,db);if(t)deposits[t]+=appDepositCount(i,db);}
  deposits.puszka=Math.round(deposits.puszka*1000)/1000;deposits.PET=Math.round(deposits.PET*1000)/1000;
  return {alcohol,middle,supplies,deposits,all:[...alcohol,...middle,...supplies]};
}
...
async function appPdf(app, options={}){
  ...
  const groups=appPrintGroups(app,db);
  const pdfRows=groups.all;
  ...
  return new Promise((resolve,reject)=>{
    try{
      ...
      let rowNumber=0, imageIndex=0;
      const drawProductRow=(i,numbered=true)=>{
        if(y+rowH>doc.page.height-70){doc.addPage();y=36;header();}
        const value=appOrderValue(i,db);
        if(numbered)rowNumber++;
        drawCell(x0,y,widths[0],rowH,numbered?rowNumber:'',false,'center');
        ...
        if(lastColumnField==='barcode'){
          ...
          const image=barcodeBuffers[imageIndex++];
        ...
        }else{drawCell(lastX,y,widths[3],rowH,i[lastColumnField]||'',false,'left');imageIndex++;}
        y+=rowH;
      };
      groups.alcohol.forEach(i=>drawProductRow(i,true));
      const depRows=[['Kaucja puszka',groups.deposits.puszka],['Kaucja PET',groups.deposits.PET]].filter(x=>Number(x[1])>0);
      for(const [label,count] of depRows){
        if(y+28>doc.page.height-70){doc.addPage();y=36;header();}
        drawCell(x0,y,widths[0],28,'',false,'center');drawCell(x0+widths[0],y,widths[1],28,label,true);drawCell(x0+widths[0]+widths[1],y,widths[2],28,`${fmtNum(count)} szt.`,true,'center');drawCell(x0+widths[0]+widths[1]+widths[2],y,widths[3],28,'',false,'center');y+=28;
      }
      // Alcohol and the remaining assortment use separate numbering sequences.
      // Supplies stay unnumbered.
      rowNumber=0;
      groups.middle.forEach(i=>drawProductRow(i,true));
      if(groups.supplies.length){
        if(y+rowH>doc.page.height-70){doc.addPage();y=36;header();}
        doc.moveTo(x0,y).lineTo(x0+widths.reduce((a,b)=>a+b,0),y).stroke('#777');
      }
      groups.supplies.forEach(i=>drawProductRow(i,false));
...
</style></head><body><button class="printButton" onclick="window.print()">Drukuj / zapisz PDF</button><header class="documentHead"><img class="documentLogo" src="/taranka-header-logo.png" alt="TARANKA"><div class="meta"><div class="metaRow"><span class="metaLabel">Numer zamówienia:</span><span>${esc(app.number)}</span></div><div class="metaRow"><span class="metaLabel">Data:</span><span>${esc(app.completedAt||app.createdAt)}</span></div><div class="metaRow"><span class="metaLabel">Sklep:</span><span>${esc(app.shopName)}</span></div></div></header><table class="orderTable"><thead><tr><th>Lp.</th><th>Nazwa pozycji</th><th>Ilość</th><th>${esc(lastColumnLabel)}</th></tr></thead><tbody>${rows.length?(()=>{let n=0;const productRow=(i,numbered=true,cls='')=>`<tr class="${cls}"><td>${numbered?++n:''}</td><td>${esc(i.productName)}</td><td>${orderValue(i)}</td><td>${lastCell(i)}</td></tr>`;const dep=[['Kaucja puszka',groups.deposits.puszka],['Kaucja PET',groups.deposits.PET]].filter(x=>Number(x[1])>0).map(([label,count])=>`<tr class="depositSummaryRow"><td></td><td><b>${label}</b></td><td><b>${esc(fmtNum(count))} szt.</b></td><td></td></tr>`).join('');const alcoholRows=groups.alcohol.map(i=>productRow(i,true)).join('');n=0;const middleRows=groups.middle.map(i=>productRow(i,true)).join('');const supplyRows=groups.supplies.map((i,idx)=>productRow(i,false,idx===0?'suppliesStart':'')).join('');return alcoholRows+dep+middleRows+supplyRows;})():'<tr><td class="empty" colspan="4">Brak pozycji w zamówieniu</td></tr>'}</tbody></table><div class="sign"><span>Data odbioru: ____________________</span><span>Podpis osoby uprawnionej do odbioru faktury: ____________________</span></div>${auto?'<script>setTimeout(()=>window.print(),300)</script>':''}</body></html>`;
}
...
function requireWarehouseJson(res,session){
  if(isWarehouse(session)) return true;
  jsonReply(res,{ok:false,error:'Потрібна авторизація складу'},401);
  return false;
}
...
async function handleRequest(req,res){ try{ const url=new URL(req.url, `http://${req.headers.host}`);
  // These routes are intentionally handled before sessions and the JSON database.
  // Product photos use unique filenames, so immutable browser caching remains safe:
  // replacing a photo changes its URL and automatically bypasses the old cache.
  if(req.method==='GET' && url.pathname==='/healthz'){ res.writeHead(200,{'Content-Type':'text/plain'}); return res.end('ok'); }
  if(req.method==='GET' && url.pathname.startsWith('/uploads/products/')){
    const safe=safeProductImagePath(url.pathname);
    const file=safe?productImageDiskPath(safe):null;
    if(!file||!fs.existsSync(file)){ res.writeHead(404,{'Content-Type':'text/plain; charset=utf-8','Cache-Control':'no-store'}); return res.end('Not found'); }
  ...
  let db=readDb();
  if(session.shop && isValidShopInDb(db, session.shop)) schedulePresenceUpdate(db,session);
  ...
  if(req.method==='POST' && url.pathname==='/catalog/favorite'){
    if(!requireShop(req,res,session)) return;
    const b=await body(req), id=String(b.id||''), product=db.products.find(p=>String(p.id)===id);
    if(!product) return sendJson(res,404,{ok:false,error:'Товар не знайдено'});
    const active=String(b.active||'')==='1';
    setShopFavorite(db,session.shop,id,active); writeDb(db);
    return sendJson(res,200,{ok:true,active,count:shopFavoriteIds(db,session.shop).size});
  }
  if(req.method==='GET' && url.pathname==='/catalog/products'){
    if(!requireShop(req,res,session)) return;
    const selection=catalogSelection(db,session,url);
    const pageInfo=catalogPageInfo(selection.products,url);
    const nextOffset=pageInfo.offset+pageInfo.items.length;
    return sendJson(res,200,{ok:true,html:catalogChunkHtml(pageInfo.items,session,db),nextOffset,total:pageInfo.total,hasMore:pageInfo.hasNext,page:pageInfo.page,pageSize:pageInfo.pageSize,totalPages:pageInfo.totalPages,hasPrevious:pageInfo.hasPrevious,hasNext:pageInfo.hasNext,paginationHtml:catalogPaginationHtml(url,pageInfo)});
  }
  if(req.method==='GET' && url.pathname==='/catalog'){
    if(!requireShop(req,res,session)) return;
    let selection=catalogSelection(db,session,url);
    if(selection.onlyNew && (session.shop || session.admin)){
      markRead(db, session, 'newProducts');
      db=readDb();
      selection=catalogSelection(db,session,url);
    }
    const {cat,onlyNew,onlyFavorites,query,favoriteIds,products}=selection;
    const pageInfo=catalogPageInfo(products,url);
    const pageProducts=pageInfo.items;
    const unread=unreadCounts(db, session);
    const emptyText=onlyFavorites&&!query?'Ви ще не додали товари в обране':(query?'Товарів за вашим запитом не знайдено':'У цій категорії немає товарів');
    return send(res, layout('Каталог', `
<a class="floatingCartButton" href="/cart" data-cart-checkout aria-label="Відкрити кошик" title="Кошик">🛒<span class="floatingCartCount" data-cart-count>${(session.cart||[]).reduce((a,i)=>a+Number(i.qty||0),0)}</span></a>
<div class="layout2">
  <aside class="card side catSideNew">
    <div class="catSideHead">
      <a href="/catalog" data-catalog-navigation class="catAllLink ${!cat&&!onlyNew&&!onlyFavorites?'catAllActive':''}">
      ...
      <a href="/catalog?new=1" data-catalog-navigation class="catNewLink ${onlyNew?'catNewActive':''}">
    ...
    <div class="catGridNew">
      ${CATEGORIES.map((c,i)=>`
      <a href="/catalog?cat=${encodeURIComponent(c)}" data-catalog-navigation class="catCardNew ${cat===c?'catCardActive':''}">
      ...
      <a href="/catalog?favorites=1" data-catalog-navigation class="catCardNew ${onlyFavorites?'catCardActive':''}">
        <span class="catIconNew" style="background:rgba(245,158,11,.10);box-shadow:0 3px 10px rgba(245,158,11,.18),inset 0 1px 0 rgba(255,255,255,.75)"><span class="catFavoriteIcon">★</span></span>
        <span class="catCardLbl">Обрані товари</span>
        <span class="catCardCnt" data-favorite-count>${favoriteIds.size}</span>
      </a>
    </div>
    <div class="catSearch">
      <span class="catSearchIcon">🔍</span>
      <input id="search" value="${esc(query)}" oninput="catalogSearchChanged()" placeholder="Пошук товарів..." autocomplete="off">
    </div>
  </aside>
  <section>
    <div class="catalogHeader">
      <h1 class="catalogTitle">${onlyFavorites?'Обрані товари':(onlyNew?'Новинки':(cat?esc(cat):'Каталог товарів'))}</h1>
    ...
    <div id="prodGrid" class="prodGrid" style="display:grid">${catalogChunkHtml(pageProducts,session,db)}</div>
    <div id="catalogEmpty" class="card center" style="padding:36px;${products.length?'display:none':''}"><p class="muted" id="catalogEmptyText">${esc(emptyText)}</p><a class="btn secondary" href="/catalog">Усі товари</a></div>
    <div id="catalogPaginationWrap">${catalogPaginationHtml(url,pageInfo)}</div>
  </section>
</div>
<script>(function(){
  let stopped=false;
  ...
  let missingIds=null;
  let lastMissingSync=0;
  const grid=document.getElementById('prodGrid');
  const paginationWrap=document.getElementById('catalogPaginationWrap');
  const empty=document.getElementById('catalogEmpty');
  const emptyText=document.getElementById('catalogEmptyText');

  function applyMissingProductCards(){
    if(!missingIds)return;
    grid.querySelectorAll('.prodCard[data-product-id]').forEach(el=>el.classList.toggle('is-temporarily-missing',missingIds.has(String(el.dataset.productId||''))));
  }
  function updateCatalogControls(data){
    if(paginationWrap)paginationWrap.innerHTML=data.paginationHtml||'';
    if(empty)empty.style.display=total?'none':'';
  }
  function requestParams(nextPage){
    const params=new URLSearchParams(location.search);
    const q=String((document.getElementById('search')||{}).value||'').trim();
    if(q)params.set('q',q);else params.delete('q');
    params.delete('offset');params.set('page',String(nextPage));
    return params;
  }
  function catalogUrl(params){return location.pathname+(params.toString()?'?'+params.toString():'')}
  async function saveCartBeforeCatalogNavigation(){
    if(typeof cartHasPending==='function'&&cartHasPending()&&typeof flushCartChanges==='function'){
      cartSaveStatus('Збереження…',false);
      await flushCartChanges();
    }
  }
  async function loadCatalogPage(nextPage,options){
    options=options||{};
    try{await saveCartBeforeCatalogNavigation()}catch(error){toast(error.message||'Не вдалося зберегти кошик');return false}
    if(requestController)requestController.abort();
    requestController=new AbortController();
    const currentController=requestController;
    const params=requestParams(nextPage);
    loading=true;
    try{
      const r=await fetch('/catalog/products?'+params.toString(),{cache:'no-store',headers:{'X-Requested-With':'fetch'},signal:currentController.signal});
      const j=await r.json();
      if(!r.ok||!j||!j.ok)throw new Error('catalog');
      grid.innerHTML=j.html||'';
      if(typeof syncOptimisticCartDom==='function')syncOptimisticCartDom();
      currentPage=Math.max(1,Number(j.page)||1);
      total=Number(j.total||0);
      totalPages=Number(j.totalPages||0);
      __catalogSearchItems=null;
      applyMissingProductCards();
      if(emptyText&&options.search)emptyText.textContent='Товарів за вашим запитом не знайдено';
      updateCatalogControls(j);
      params.set('page',String(currentPage));
      const nextUrl=catalogUrl(params);
      if(options.history==='push')history.pushState({catalogPage:currentPage},'',nextUrl);
      else if(options.history==='replace')history.replaceState({catalogPage:currentPage},'',nextUrl);
      if(options.scroll&&grid)grid.scrollIntoView({block:'start',behavior:'auto'});
      return true;
    }catch(e){
      if(e&&e.name!=='AbortError')toast('Не вдалося завантажити сторінку товарів');
      return false;
    }finally{
      if(requestController===currentController)loading=false;
    }
  }
  paginationWrap&&paginationWrap.addEventListener('click',function(event){
    const link=event.target.closest&&event.target.closest('[data-catalog-page]');if(!link)return;
    event.preventDefault();const page=Number(link.dataset.catalogPage);if(Number.isInteger(page)&&page>=1)loadCatalogPage(page,{history:'push',scroll:true});
  });
  document.addEventListener('click',async function(event){
    const link=event.target.closest&&event.target.closest('a[data-catalog-navigation]');if(!link||!(typeof cartHasPending==='function'&&cartHasPending()))return;
    event.preventDefault();try{await saveCartBeforeCatalogNavigation();location.href=link.href}catch(error){toast(error.message||'Не вдалося зберегти кошик')}
  },true);
  window.refreshCatalogProducts=function(){return loadCatalogPage(currentPage,{history:'replace',scroll:false})};
  window.catalogSearchChanged=function(){
    clearTimeout(searchTimer);
    searchTimer=setTimeout(function(){
      loadCatalogPage(1,{history:'replace',scroll:false,search:true});
    },450);
  };
  window.addEventListener('popstate',function(){
    const params=new URLSearchParams(location.search),raw=params.get('page')||'1',page=/^\d+$/.test(raw)?Math.max(1,Number(raw)):1;
    const search=document.getElementById('search');if(search)search.value=String(params.get('q')||'');
    loadCatalogPage(page,{history:'none',scroll:true});
  });
  async function syncMissingProductCards(force){
    const now=Date.now();
    if(stopped||document.hidden||(!force&&now-lastMissingSync<${MISSING_ALERT_POLL_INTERVAL_MS}))return;
    lastMissingSync=now;
    try{
      ...
      missingIds=new Set((j.ids||[]).map(String));
      applyMissingProductCards();
    }catch(e){}
  }
  syncMissingProductCards(true);
  const timer=setInterval(function(){syncMissingProductCards(false)},${MISSING_ALERT_POLL_INTERVAL_MS});
  document.addEventListener('visibilitychange',function(){if(!document.hidden)syncMissingProductCards(false)});
  window.addEventListener('pagehide',function(){stopped=true;clearInterval(timer);clearTimeout(searchTimer);if(requestController)requestController.abort()},{once:true});
})();</script>`, session));
  }
  if(req.method==='POST' && url.pathname==='/cart/add'){
    if(!requireShop(req,res,session)) return;
    const b=await body(req);
    if(b.expectedQty!==undefined||b.targetQty!==undefined||b.operationId!==undefined){
      const result=applyCartTargetUpdate(db,session,b);
      return sendJson(res,result.status,result.data);
    }
    const p=db.products.find(x=>String(x.id)===String(b.id)&&!x.hidden);
    const requestedDelta=Number(b.delta===undefined||b.delta===''?1:b.delta),delta=[1,5,10].includes(requestedDelta)?requestedDelta:1;
    let itemQty=0;
    if(p){const item=session.cart.find(x=>String(x.id)===String(p.id));if(item)item.qty=Number(item.qty||0)+delta;else session.cart.push({...copyProductFields(p),qty:delta});itemQty=(session.cart.find(x=>String(x.id)===String(p.id))||{}).qty||0;saveCart(session);}
    const count=session.cart.reduce((a,i)=>a+Number(i.qty||0),0);
    if(req.headers['x-requested-with']==='fetch'){res.writeHead(200,{'Content-Type':'application/json; charset=utf-8'});return res.end(JSON.stringify({ok:true,count,positions:session.cart.length,id:String(b.id),itemQty,result:productResultText(p,itemQty),catalogTotal:productCatalogTotalText(p,itemQty)}));}
    return redirect(res,req.headers.referer||'/catalog');
  }
  if(req.method==='GET' && url.pathname==='/orders/history-chunk'){
    if(!requireShop(req,res,session)) return;
    const orders=shopOrdersFor(db,session.shop);
    const offset=Math.max(0,Math.min(orders.length,parseInt(url.searchParams.get('offset')||'0',10)||0));
    const chunk=orders.slice(offset,offset+ORDER_PAGE_SIZE),nextOffset=offset+chunk.length;
    return sendJson(res,200,{ok:true,html:chunk.map(shopOrderHistoryCard).join(''),nextOffset,total:orders.length,hasMore:nextOffset<orders.length});
  }
  if(req.method==='GET' && url.pathname==='/orders/history-details'){
    if(!requireShop(req,res,session)) return;
    const order=(db.orders||[]).find(o=>String(o.id)===String(url.searchParams.get('id')||'')&&String(o.shop||'')===String(session.shop||''));
    if(!order) return sendJson(res,404,{ok:false,error:'Замовлення не знайдено'});
    return sendJson(res,200,{ok:true,html:shopOrderDetailsHtml(order)});
  }
  if(req.method==='GET' && url.pathname==='/cart'){ if(!requireShop(req,res,session)) return; ensureOrderNumbers(db,false); const totalQty=session.cart.reduce((a,i)=>a+Number(i.qty||0),0); const historyHtml=shopOrderHistoryHtml(db, session.shop); return send(res, layout('Кошик', `<div class="actions" style="align-items:center;justify-content:space-between;margin-bottom:12px"><h1 style="margin:0">Кошик</h1><a class="btn secondary" href="/catalog">Продовжити покупки</a></div><div data-cart-page>${session.cart.length?`<div class="cartSummary"><div><b>Ваше замовлення</b><div class="muted">У кошику <span data-cart-summary-total>${totalQty}</span> шт. · <span data-cart-summary-positions>${session.cart.length}</span> позицій</div></div><a class="btn" href="/checkout" data-cart-checkout>Оформити замовлення</a></div><div class="listWrap"><table class="listTable cartTable"><thead><tr><th>№</th><th>Назва</th><th>К-сть</th><th>×</th></tr></thead><tbody>${session.cart.map((i,n)=>`<tr data-cart-row="${i.id}"><td class="num">${n+1}</td><td class="mainCell"><span class="name">${esc(productDisplayName(i))}</span></td><td class="qtyCell"><form class="listQty" method="post" action="/cart/qty" data-ajax-cart data-action="qty" onsubmit="event.preventDefault(); return changeQty(this)"><input type="hidden" name="id" value="${i.id}"><input type="hidden" name="delta" value="0"><button type="button" onclick="changeQty(this.form,-1)" class="secondary iconBtn minusBtn" aria-label="Мінус">−</button><div class="qtynum catalogTotalValue" data-row-qty data-item-result="${i.id}" data-quantity-id="${esc(i.id)}" data-quantity="${esc(i.qty)}" data-quantity-factor="${esc(productFormatValue(i))}" data-quantity-unit="${esc(inferResultUnit(i))}">${esc(productResultText(i,i.qty))}</div><button type="button" onclick="changeQty(this.form,1)" class="iconBtn" aria-label="Додати">+</button></form></td><td class="deleteCell"><form method="post" action="/cart/remove" data-ajax-cart data-action="remove" onsubmit="event.preventDefault(); return removeCart(this)"><input type="hidden" name="id" value="${i.id}"><button type="button" class="deleteIcon" title="Видалити позицію" aria-label="Видалити позицію" onclick="removeCart(this.form)">×</button></form></td></tr>`).join('')}</tbody></table></div><form method="post" action="/cart/clear" data-ajax-cart data-action="clear" onsubmit="event.preventDefault(); return clearCart(this)" style="margin-top:10px"><button class="secondary">Очистити кошик</button></form>`:'<section class="card center"><p>Кошик порожній</p><a class="btn" href="/catalog">До каталогу</a></section>'}</div>${historyHtml}`, session)); }
  if(req.method==='POST' && url.pathname==='/cart/qty'){
    if(!requireShop(req,res,session)) return;
    const b=await body(req);
    if(b.expectedQty!==undefined||b.targetQty!==undefined||b.operationId!==undefined){const result=applyCartTargetUpdate(db,session,b);return sendJson(res,result.status,result.data);}
    const product=db.products.find(x=>String(x.id)===String(b.id));
    const item=session.cart.find(x=>String(x.id)===String(b.id));
    const displayItem=item||product;
    const delta=Number(b.delta||0);
    if(!Number.isFinite(delta)||!Number.isInteger(delta)||Math.abs(delta)>1000000)return sendJson(res,400,{ok:false,error:'Некоректна кількість',id:String(b.id),saved:false});
    if(item){const next=Number(item.qty||0)+delta;if(!Number.isFinite(next))return sendJson(res,400,{ok:false,error:'Некоректна кількість',id:String(b.id),saved:false});item.qty=next;if(item.qty<1)session.cart=session.cart.filter(x=>String(x.id)!==String(b.id));}
    saveCart(session);
    const count=session.cart.reduce((a,i)=>a+Number(i.qty||0),0),itemQty=(session.cart.find(x=>String(x.id)===String(b.id))||{}).qty||0;
    if(req.headers['x-requested-with']==='fetch'){res.writeHead(200,{'Content-Type':'application/json; charset=utf-8'});return res.end(JSON.stringify({ok:true,count,positions:session.cart.length,id:String(b.id),itemQty,result:productResultText(displayItem,itemQty),catalogTotal:productCatalogTotalText(displayItem,itemQty)}));}
  ...
  if(req.method==='GET' && url.pathname==='/admin-statistics'){ if(!requireAdmin(req,res,session)) return; res.writeHead(200,{'Content-Type':'text/html; charset=utf-8','Cache-Control':'no-store, no-cache, must-revalidate','Pragma':'no-cache'}); return res.end(layout('Статистика',statisticsPage(db,url),session)); }
  if(req.method==='GET' && url.pathname==='/admin-statistics.xlsx'){ if(!requireAdmin(req,res,session)) return; const xlsx=statisticsXlsx(db,url); res.writeHead(200,{'Content-Type':'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet','Content-Disposition':contentDispositionXlsx(`statystyka-towarow_${exportFileDate()}.xlsx`),'Cache-Control':'no-store'}); return res.end(xlsx); }
  if(req.method==='GET' && url.pathname==='/admin-market'){ if(!requireAdmin(req,res,session))return; return send(res,layout('Ринок',marketActivePage(db,url),session)); }
  if(req.method==='GET' && url.pathname==='/admin-market/history'){ if(!requireAdmin(req,res,session))return; return send(res,layout('Історія ринків',marketHistoryPage(db,url),session)); }
  if(req.method==='POST' && url.pathname==='/admin-market/product-add'){
    if(!requireAdmin(req,res,session))return;const b=await body(req),state=marketState(db),name=String(b.name||'').trim(),unit=marketUnit(b.unit),priceCents=marketParsePriceCents(b.price);
    if(!name||priceCents===null)return redirect(res,'/admin-market?status=invalid');
    const duplicate=state.template.some(p=>String(p.name||'').trim().toLocaleLowerCase('uk')===name.toLocaleLowerCase('uk'));
    if(duplicate)return redirect(res,'/admin-market?status=invalid');
    const item={id:marketUid('product'),name,priceCents,unit,createdAt:warsawTime(),updatedAt:warsawTime(),versions:{name:0,price:0,unit:0}};state.template.push(item);
    if(state.active){const snapshot=marketProductSnapshot(item);state.active.products=Array.isArray(state.active.products)?state.active.products:[];state.active.products.push(snapshot);marketEnsureRows(state.active,state.active.sellerSnapshots||[]);state.active.updatedAt=warsawTime();}
    writeDb(db);return redirect(res,'/admin-market?status=product-added');
  }
  if(req.method==='POST' && url.pathname==='/admin-market/seller-add'){
    if(!requireAdmin(req,res,session))return;const b=await body(req),state=marketState(db),name=String(b.name||'').trim();if(!name)return redirect(res,'/admin-market?status=invalid');
    if(state.sellers.some(s=>String(s.name||'').trim().toLocaleLowerCase('uk')===name.toLocaleLowerCase('uk')))return redirect(res,'/admin-market?status=invalid');
    const seller={id:marketUid('seller'),name,createdAt:warsawTime()};state.sellers.push(seller);
    if(state.active){state.active.sellerSnapshots=Array.isArray(state.active.sellerSnapshots)?state.active.sellerSnapshots:[];state.active.sellerSnapshots.push({id:seller.id,name:seller.name});marketEnsureRows(state.active,state.active.sellerSnapshots);state.active.updatedAt=warsawTime();}
    writeDb(db);return redirect(res,'/admin-market?seller='+encodeURIComponent(seller.id)+'&status=seller-added');
  }
  if(req.method==='POST' && url.pathname==='/admin-market/new'){
    if(!requireAdmin(req,res,session))return;const b=await body(req),state=marketState(db),date=String(b.date||'');if(!/^\d{4}-\d{2}-\d{2}$/.test(date))return redirect(res,'/admin-market?status=invalid');
    if(state.active){state.active.archivedAt=warsawTime();state.active.archivedMs=nowMs();state.history.push(state.active);}
    state.active=marketCreateRecord(state,date);writeDb(db);return redirect(res,'/admin-market?status=created');
  }
  if(req.method==='POST' && url.pathname==='/admin-market/update'){
    if(!requireAdmin(req,res,session))return;const b=await body(req),state=marketState(db),marketId=String(b.marketId||''),record=marketId?marketRecord(state,marketId):null,isActive=!!(record&&state.active&&String(record.id)===String(state.active.id));
    if(!record||String(record.id)!==marketId)return sendJson(res,409,{ok:false,conflict:true,error:'Цей ринок змінено або видалено. Оновіть сторінку.'});
    const sellerId=String(b.sellerId||''),productId=String(b.productId||''),field=String(b.field||''),product=marketProduct(record,productId),seller=marketSeller(record,sellerId);
    if(!product||!seller)return sendJson(res,404,{ok:false,error:'Позицію або продавця не знайдено'});
    const expected=Math.max(0,Number.parseInt(b.expectedVersion||'0',10)||0);let fieldVersion=0,normalized='';
    if(['accepted','returned'].includes(field)){
      const rows=marketSellerRows(record,sellerId),row=rows[productId]||(rows[productId]={accepted:null,returned:null,versions:{accepted:0,returned:0},updatedAt:''});row.versions=row.versions||{accepted:0,returned:0};const current=Math.max(0,Number(row.versions[field])||0);
      if(current!==expected)return sendJson(res,409,{ok:false,conflict:true,error:'Це поле вже змінено в іншій вкладці',currentVersion:current});
      const parsed=marketParseQuantityMilli(b.value,product.unit);if(!parsed.ok)return sendJson(res,400,{ok:false,error:parsed.error});
      const nextAccepted=field==='accepted'?parsed.value:row.accepted,nextReturned=field==='returned'?parsed.value:row.returned;
      if(nextReturned!==null&&nextReturned!==undefined&&nextAccepted!==null&&nextAccepted!==undefined&&nextReturned>nextAccepted)return sendJson(res,400,{ok:false,error:'Здана кількість не може перевищувати прийняту'});
      row[field]=parsed.value;row.versions[field]=current+1;row.updatedAt=warsawTime();fieldVersion=row.versions[field];normalized=marketQuantity(parsed.value,product.unit);
    }else if(['name','price','unit'].includes(field)){
      product.versions=product.versions||{name:0,price:0,unit:0};const current=Math.max(0,Number(product.versions[field])||0);if(current!==expected)return sendJson(res,409,{ok:false,conflict:true,error:'Це поле вже змінено в іншій вкладці',currentVersion:current});
      const template=isActive?state.template.find(p=>String(p.id)===productId):null;
      if(field==='name'){const value=String(b.value||'').trim();if(!value)return sendJson(res,400,{ok:false,error:'Назва товару не може бути порожньою'});product.name=value;if(template)template.name=value;normalized=value;}
      if(field==='price'){const value=marketParsePriceCents(b.value);if(value===null)return sendJson(res,400,{ok:false,error:'Ціна повинна містити не більше двох знаків після коми'});product.priceCents=value;if(template)template.priceCents=value;normalized=marketPriceInput(value);}
      if(field==='unit'){const value=marketUnit(b.value);product.unit=value;if(template)template.unit=value;normalized=value;}
      product.versions[field]=current+1;product.updatedAt=warsawTime();fieldVersion=product.versions[field];if(template){template.updatedAt=warsawTime();template.versions=template.versions||{};template.versions[field]=(Number(template.versions[field])||0)+1;}
    }else return sendJson(res,400,{ok:false,error:'Недозволене поле'});
    record.updatedAt=warsawTime();writeDb(db);const calc=marketCalc(record,sellerId,productId);
    return sendJson(res,200,{ok:true,fieldVersion,normalized,sold:calc.sold===null?'':marketQuantity(calc.sold,product.unit)+' '+product.unit,sum:calc.sumCents===null?'':marketMoney(calc.sumCents),sellerTotal:marketMoney(marketSellerTotal(record,sellerId))});
  }
  if(req.method==='POST' && url.pathname==='/admin-market/product-delete'){
    if(!requireAdmin(req,res,session))return;const b=await body(req),state=marketState(db),id=String(b.id||'');state.template=state.template.filter(p=>String(p.id)!==id);
    if(state.active&&!marketHasRowData(state.active,id)){state.active.products=(state.active.products||[]).filter(p=>String(p.id)!==id);Object.values(state.active.rows||{}).forEach(rows=>{if(rows&&typeof rows==='object')delete rows[id]});}
    writeDb(db);return redirect(res,'/admin-market?status=product-deleted');
  }
  if(req.method==='POST' && url.pathname==='/admin-market/history-delete'){
    if(!requireAdmin(req,res,session))return;const b=await body(req),state=marketState(db),id=String(b.id||'');state.history=state.history.filter(r=>String(r.id)!==id);writeDb(db);return redirect(res,'/admin-market/history?status=deleted');
  }
  if(req.method==='GET' && url.pathname==='/admin-market/print'){
    if(!requireAdmin(req,res,session))return;const state=marketState(db),record=marketRecord(state,String(url.searchParams.get('marketId')||''));if(!record)return notFound(res);
    return send(res,marketPrintPage(record,{mode:'full',scope:url.searchParams.get('scope'),sellerId:url.searchParams.get('seller'),auto:url.searchParams.get('auto')==='1'}));
  }
  if(req.method==='GET' && url.pathname==='/admin-market/pdf'){
    if(!requireAdmin(req,res,session))return;const state=marketState(db),record=marketRecord(state,String(url.searchParams.get('marketId')||''));if(!record)return notFound(res);
    try{const pdf=await marketPdf(record,{mode:'full',scope:url.searchParams.get('scope'),sellerId:url.searchParams.get('seller')}),filename=`market_${String(record.date||'').replace(/[^0-9-]/g,'_')}.pdf`;res.writeHead(200,{'Content-Type':'application/pdf','Content-Disposition':`attachment; filename="${filename}"`,'Content-Length':pdf.length,'Cache-Control':'no-store','X-Content-Type-Options':'nosniff'});return res.end(pdf);}catch(error){console.error('[market-pdf]',error);return send(res,marketPrintPage(record,{mode:'full',scope:url.searchParams.get('scope'),sellerId:url.searchParams.get('seller')}),500);}
  }
  ...
  if(req.method==='POST' && url.pathname==='/admin/order-edit'){ if(!requireAdmin(req,res,session)) return; const b=await body(req); const o=(db.orders||[]).find(x=>String(x.id)===String(b.id)); if(!o) return notFound(res); if(o.invoicePrintedAt) return send(res,layout('Редагування недоступне',`<div class="adminShell">${adminMenu()}<section class="card center" style="padding:36px"><h1>Зміни не збережено</h1><p class="muted">Накладну вже надруковано.</p><a class="btn" href="/admin-orders">Повернутися</a></section></div>`,session),409); try{ const raw=JSON.parse(String(b.itemsJson||'[]')); const items=raw.map(i=>{ const p=(db.products||[]).find(x=>String(x.id)===String(i.id)); if(!p)return null; return {...copyProductFields(p),qty:Math.max(1,Number(i.qty||1))}; }).filter(Boolean); if(!items.length) return send(res,layout('Редагувати замовлення',`<div class="adminShell">${adminMenu()}<section>${adminOrderEditPage(db,o,'Додайте хоча б один товар.')}</section></div>`,session),400); if(o.pickingFinalizedAt){reverseOrderStock(db,o);delete o.pickingFinalizedAt;delete o.pickingFinalizedMs;o.pickingItems={};} o.items=items; o.comment=String(b.comment||'').trim(); o.kegItems=captureOrderKegItems(db,items); o.updatedAt=warsawTime(); o.updatedMs=nowMs(); o.status='Оновлено складом'; writeDb(db); return redirect(res,'/admin-orders'); }catch(e){ return send(res,layout('Редагувати замовлення',`<div class="adminShell">${adminMenu()}<section>${adminOrderEditPage(db,o,'Не вдалося зберегти зміни. Спробуйте ще раз.')}</section></div>`,session),400); } }
  if(req.method==='GET' && url.pathname==='/admin-orders/chunk'){
    if(!requireAdmin(req,res,session)) return;
    ensureOrderNumbers(db);
    const orders=(db.orders||[]).slice().sort((a,b)=>(Number(b.id)||0)-(Number(a.id)||0));
    const offset=Math.max(0,Math.min(orders.length,parseInt(url.searchParams.get('offset')||'0',10)||0));
    const chunk=orders.slice(offset,offset+ORDER_PAGE_SIZE),nextOffset=offset+chunk.length;
    return sendJson(res,200,{ok:true,html:chunk.map(o=>adminOrderCard(o,db.products)).join(''),nextOffset,total:orders.length,hasMore:nextOffset<orders.length});
  }
  if(req.method==='GET' && url.pathname==='/admin-orders/details'){
    if(!requireAdmin(req,res,session)) return;
    const order=(db.orders||[]).find(o=>String(o.id)===String(url.searchParams.get('id')||''));
    if(!order) return sendJson(res,404,{ok:false,error:'Замовлення не знайдено'});
    return sendJson(res,200,{ok:true,html:adminOrderDetailsHtml(order)});
  }
  if(req.method==='GET' && url.pathname==='/admin-orders'){
    if(!requireAdmin(req,res,session)) return;
    ensureOrderNumbers(db);
    const orders=(db.orders||[]).slice().sort((a,b)=>(Number(b.id)||0)-(Number(a.id)||0));
    const first=orders.slice(0,ORDER_PAGE_SIZE),hasMore=first.length<orders.length;
    const content=`<div class="adminShell">${adminMenu()}<section><h1>Замовлення</h1><div id="adminOrdersList">${first.length?first.map(o=>adminOrderCard(o,db.products)).join(''):'<div class="card center"><p class="muted">Замовлень поки немає</p></div>'}</div><div class="center" data-order-load-wrap style="padding:10px 0 20px;${hasMore?'':'display:none'}"><button type="button" class="btn secondary" data-url="/admin-orders/chunk" data-target="adminOrdersList" data-offset="${first.length}" onclick="loadMoreOrderCards(this)">Показати ще 20 замовлень</button><div class="muted" data-order-progress style="font-size:12px;margin-top:8px">Показано ${first.length} із ${orders.length}</div></div></section></div>`;
    return send(res,layout('Склад — Замовлення',content,session));
  }
  ...
  if(req.method==='POST' && url.pathname==='/admin-missing-products/in-stock-batch'){ if(!requireAdmin(req,res,session)) return; const b=await body(req); const raw=b.productIds; const ids=(Array.isArray(raw)?raw:[raw]).filter(Boolean).map(String); for(const productId of ids) clearMissingProductAlert(db,productId); writeDb(db); return redirect(res,'/admin-missing-products'); }
  ...
  if(req.method==='GET' && url.pathname==='/admin-products/chunk'){
    if(!requireAdmin(req,res,session)) return;
    const selection=adminProductSelection(db,url);
    const offset=Math.max(0,Number.parseInt(url.searchParams.get('offset')||'0',10)||0);
    const chunk=selection.products.slice(offset,offset+ADMIN_PRODUCT_PAGE_SIZE);
    const nextOffset=offset+chunk.length;
    return sendJson(res,200,{ok:true,html:adminProductRowsHtml(chunk,offset),nextOffset,total:selection.products.length,hasMore:nextOffset<selection.products.length});
  }
  if(req.method==='GET' && url.pathname==='/admin-products'){
    if(!requireAdmin(req,res,session)) return;
    return send(res,layout('Склад — Товари',adminProductsPage(db,url),session));
  }
...
// Одночасні запити читання не блокують один одного. Запити, що можуть змінити
// JSON-базу, виконуються ексклюзивно й послідовно, щоб два записи не могли
// перезаписати дані один одного.
let activeDbReaders=0,activeDbWriter=false;
const dbAccessQueue=[];
function drainDbAccessQueue(){
  if(activeDbWriter||!dbAccessQueue.length) return;
  if(dbAccessQueue[0].write){
    if(activeDbReaders>0) return;
    activeDbWriter=true;
    const entry=dbAccessQueue.shift();
    entry.resolve(()=>{activeDbWriter=false;drainDbAccessQueue()});
    return;
  }
  while(dbAccessQueue.length&&!dbAccessQueue[0].write&&!activeDbWriter){
    activeDbReaders++;
    const entry=dbAccessQueue.shift();
    entry.resolve(()=>{activeDbReaders=Math.max(0,activeDbReaders-1);drainDbAccessQueue()});
  }
}
function acquireDbAccess(write){
  return new Promise(resolve=>{dbAccessQueue.push({write:!!write,resolve});drainDbAccessQueue()});
}
async function runWithDbAccess(write,task){
  const release=await acquireDbAccess(write);
  try{return await requestDbContext.run({db:null},task)}finally{release()}
}
function enqueueDbWrite(task){
  const run=runWithDbAccess(true,task);
  run.catch(error=>console.error('[database write queue]',error));
  return run;
}
const MUTATING_GET_PATHS=new Set([
  '/shop-logout','/admin-logout','/messages','/messages/list','/admin-messages',
  '/admin-notifications','/notifications','/about','/chat/messages',
  '/admin-order-export','/admin-order-original-export','/admin-order-original-print',
  '/admin-order-original-pdf','/admin-order-print','/admin-order-pdf','/admin-order-picking'
]);
function requestNeedsDbWrite(req){
  if(String(req.method||'GET').toUpperCase()!=='GET') return true;
  const url=new URL(req.url,`http://${req.headers.host||'localhost'}`);
  if(url.pathname==='/catalog'&&url.searchParams.get('new')==='1') return true;
  return MUTATING_GET_PATHS.has(url.pathname);
}
function requestUsesStaticFastPath(req){
  if(String(req.method||'GET').toUpperCase()!=='GET') return false;
  const url=new URL(req.url,`http://${req.headers.host||'localhost'}`);
  return url.pathname==='/healthz'
    || url.pathname==='/taranka-header-logo.png'
    || url.pathname==='/taranka-logo.png'
    || url.pathname==='/logo.png'
    || url.pathname.startsWith('/uploads/products/');
}
function handler(req,res){
  if(requestUsesStaticFastPath(req)) return requestDbContext.run({db:null},()=>handleRequest(req,res));
  return runWithDbAccess(requestNeedsDbWrite(req),()=>handleRequest(req,res));
}
