...
(()=>{const marketId=${JSON.stringify(String(marketId))},sellerId=${JSON.stringify(String(sellerId))},timers=new Map(),queued=new Map(),pending=new Set(),status=document.getElementById('marketSaveStatus');
...
async function save(input){const row=input.closest('[data-market-row]');if(!row)return;const key=row.dataset.productId+':'+input.dataset.field;queued.delete(key);setStatus('saving','Збереження…');const data=new URLSearchParams({marketId,sellerId,productId:row.dataset.productId,field:input.dataset.field,value:input.value,expectedVersion:input.dataset.version||'0'});const promise=fetch('/admin-market/update',{method:'POST',headers:{'Content-Type':'application/x-www-form-urlencoded','X-Requested-With':'fetch'},body:data.toString()}).then(async response=>{const out=await response.json();if(!response.ok)throw Object.assign(new Error(out.error||'Не вдалося зберегти'),{conflict:out.conflict});input.dataset.version=String(out.fieldVersion||0);if(out.normalized!==undefined&&document.activeElement!==input)input.value=out.normalized;if(input.dataset.field==='unit')row.dataset.unit=out.normalized;row.querySelector('[data-sold]').textContent=out.sold||'';row.querySelector('[data-sum]').textContent=out.sum||'';const total=document.querySelector('[data-seller-total]');if(total)total.textContent=out.sellerTotal;setStatus('saved','Збережено')}).catch(error=>{setStatus('error',(error.conflict?'Дані змінено в іншій вкладці. Оновіть сторінку.':'Помилка збереження. Повторити'));input.classList.add('inputError')}).finally(()=>pending.delete(promise));pending.add(promise);return promise}
document.querySelectorAll('[data-market-input]').forEach(input=>{input.addEventListener('input',()=>{input.classList.remove('inputError');const row=input.closest('[data-market-row]');updateLocal(row);const key=row.dataset.productId+':'+input.dataset.field;clearTimeout(timers.get(key));queued.set(key,input);timers.set(key,setTimeout(()=>save(input),400))});input.addEventListener('blur',()=>{const row=input.closest('[data-market-row]'),key=row.dataset.productId+':'+input.dataset.field;if(queued.has(key)){clearTimeout(timers.get(key));save(input)}})});
window.marketFlush=async()=>{for(const [key,input] of [...queued]){clearTimeout(timers.get(key));await save(input)}if(pending.size)await Promise.allSettled([...pending]);if(status&&status.classList.contains('error'))throw new Error(status.textContent)};
